			...
			for (int k=0;k<toPaint.size();k++){
				if (view.getCell() instanceof DefaultEdge){
					...
					if (view.getCell() instanceof DefaultGraphCell){
						...
						DefaultGraphCell dgc = ((DefaultGraphCell)view.getCell());
						if (dgc.getChildCount()>0){
							DefaultPort port = (DefaultPort) dgc.getFirstChild();
							...
							while (edges.hasNext() && !found){
								...
								found=found
								|| 
								((DefaultPort)(next.getTarget())).getParent().equals(toPaint.elementAt(k))||
								((DefaultPort)(next.getSource())).getParent().equals(toPaint.elementAt(k));
