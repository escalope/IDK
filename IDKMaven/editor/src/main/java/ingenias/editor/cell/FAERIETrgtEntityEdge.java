...
public class FAERIETrgtEntityEdge extends NAryEdge {

  // Constructor with data content
  public FAERIETrgtEntityEdge(ingenias.editor.entities.FAERIETrgtEntity userObject) {
    super(userObject);
    
     this.addRole("FAERIETrgtEntitysource");
     this.setArity("FAERIETrgtEntitysource",true,1);
     this.setArity("FAERIETrgtEntitysource",false,1);
     
this.addClass("FAERIETrgtEntitysource","ingenias.editor.entities.FAERIECtxtRelationship");
     
this.addClass("FAERIETrgtEntitysource","ingenias.editor.entities.FAERIECtxtEntity");
     
    
     this.addRole("FAERIETrgtEntitytarget");
     this.setArity("FAERIETrgtEntitytarget",true,1);
     this.setArity("FAERIETrgtEntitytarget",false,2147483647);
     
this.addClass("FAERIETrgtEntitytarget","ingenias.editor.entities.FAERIECtxtEntity");
     
this.addClass("FAERIETrgtEntitytarget","ingenias.editor.entities.FAERIECtxtRelationship");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof FAERIETrgtEntityEdge)))
      return false;

    // n-edge to be considered.
    FAERIETrgtEntityEdge edge = new FAERIETrgtEntityEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof FAERIETrgtEntityEdge)
      edge = (FAERIETrgtEntityEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("FAERIETrgtEntitysource")){
 	 edges.add( new DefaultEdge( new FAERIETrgtEntitysourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("FAERIETrgtEntitytarget")){
 	 edges.add( new DefaultEdge( new FAERIETrgtEntitytargetRole() ) );
	 }
