...
public class CDUsesCodeEdge extends NAryEdge {

  // Constructor with data content
  public CDUsesCodeEdge(ingenias.editor.entities.CDUsesCode userObject) {
    super(userObject);
    
     this.addRole("CDUsesCodesource");
     this.setArity("CDUsesCodesource",true,1);
     this.setArity("CDUsesCodesource",false,1);
     
this.addClass("CDUsesCodesource","ingenias.editor.entities.INGENIASObject");
     
    
     this.addRole("CDUsesCodetarget");
     this.setArity("CDUsesCodetarget",true,1);
     this.setArity("CDUsesCodetarget",false,1);
     
this.addClass("CDUsesCodetarget","ingenias.editor.entities.INGENIASCodeComponent");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof CDUsesCodeEdge)))
      return false;

    // n-edge to be considered.
    CDUsesCodeEdge edge = new CDUsesCodeEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof CDUsesCodeEdge)
      edge = (CDUsesCodeEdge) selectedEdge;
