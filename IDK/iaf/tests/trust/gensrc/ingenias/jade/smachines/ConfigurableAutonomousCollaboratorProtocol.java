...
public class ConfigurableAutonomousCollaboratorProtocol implements AgentProtocols{

public ConfigurableAutonomousCollaboratorProtocol(){};		
	...
	public ActiveConversation initialiseProtocols(String agentName,RuntimeConversation conv, 
        ...
        // Adds a protocol to the protocol queue
		   ...
		   if (conv.getInteraction().getId().equals("PerformProposal") && conv.getPlayedRole().equals("CollaboratorRole")){
		   DefaultCommControl dcc=new CollaboratorRolePerformProposalDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new CollaboratorRolePerformProposalStateBehavior(
			agentName,
				msr,msu,
					conv,                           		
				   conv.getPlayedRole(),
         ...
         throw new WrongInteraction("Agent "+agentName+" does not know any interaction protocol of type "+conv.getInteraction().getId()+" where the " +
	        ...
	        public AgentExternalDescription[] getInteractionActors(String interaction, YellowPages yp) throws ingenias.jade.exception.NoAgentsFound{
              AgentExternalDescription[] result=null;
               ...
               if (interaction.equals("PerformProposal")){
                Vector<AgentExternalDescription> cols=new Vector<AgentExternalDescription>();
                 
                 try {
                 String cardinality="";
		 DFAgentDescription[] agents=yp.getAgents("SupervisorRole");
                   if (agents==null || agents.length<=0)
                      throw new ingenias.jade.exception.NoAgentsFound("Could not find an agent playing the role SupervisorRole");
                   if (cardinality.equals("1") || cardinality.equals(""))
                    cols.add(new AgentExternalDescription(agents[0].getName(),"SupervisorRole"));
                    else
                	   if (cardinality.equals("1__*"))
						   for (int k=0;k<agents.length;k++)
                			  cols.add(new AgentExternalDescription(agents[k].getName(),"SupervisorRole"));
