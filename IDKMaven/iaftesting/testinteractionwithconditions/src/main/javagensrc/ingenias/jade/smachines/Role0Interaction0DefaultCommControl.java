...
import ingenias.exception.NotFound;
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    lw.addDeletionLockExpectedType("FrameFact0");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("InteractionUnit0") &&
         ...
         Vector<MentalEntity> fact1=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"FrameFact0");
         allexist=allexist && (fact1 !=null) && !fact1.isEmpty();
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"FrameFact0").isEmpty();
         ...
         if (allexist && true){
           ...
           sb.removeState("InteractionUnit0");
	   	   ...
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"FrameFact0");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("FrameFact0");
	       lockProcessed("FrameFact0");
           //MainInteractionManager.log("Removing lock FrameFact0",this.getAgent().getLocalName()+"-"+sb.getConversationID());
