...
public class UseCasePursuesEdge extends NAryEdge {

  // Constructor with data content
  public UseCasePursuesEdge(ingenias.editor.entities.UseCasePursues userObject) {
    super(userObject);
    
     this.addRole("UseCasePursuessource");
     this.setArity("UseCasePursuessource",true,1);
     this.setArity("UseCasePursuessource",false,1);
     
this.addClass("UseCasePursuessource","ingenias.editor.entities.UseCase");
     
    
     this.addRole("UseCasePursuestarget");
     this.setArity("UseCasePursuestarget",true,1);
     this.setArity("UseCasePursuestarget",false,2147483647);
     
this.addClass("UseCasePursuestarget","ingenias.editor.entities.Goal");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof UseCasePursuesEdge)))
      return false;

    // n-edge to be considered.
    UseCasePursuesEdge edge = new UseCasePursuesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof UseCasePursuesEdge)
      edge = (UseCasePursuesEdge) selectedEdge;
