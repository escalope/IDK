...
public class InitiatorAgentJADEAgent
		 extends JADEAgent {         
 
		 public InitiatorAgentJADEAgent(){
		 super(new InitiatorAgentProtocol(),new InitiatorAgentInteractionLocks());
		 }
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
		...
		expectedInput=this.getMSM().getMentalEntityByType("Request");
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
				nonExistingInputs.add("Request");
			 } else {
			    addExpectedInputs(tobject, "Request","1",expectedInput);
             	addConsumedInput(to,"Request",expectedInput);
			 }
         ...
         if (tobject.getType().equals("InteractionLaunchingTask") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
	 ...
	 /*    
			 ...
			 expectedInt.setInteraction(new Interaction("SampleInteraction"));
	      ...
	      /**/	      
 			...
 			if (RuntimeConversation.class.isAssignableFrom(expectedOutputRuntimeConversation.getClass())){
				...
				try {
					...
					m.invoke(expectedOutputRuntimeConversation, new Interaction("SampleInteraction")) ;	  
            ...
            to.add(new OutputEntity(expectedOutputRuntimeConversation,TaskOperations.CreateMS));
 			...
 			{
 			Request expectedOutputRequest=
 				new Request(MentalStateManager.generateMentalEntityID());
 			if (RuntimeConversation.class.isAssignableFrom(expectedOutputRequest.getClass())){
 			    java.lang.reflect.Method m;
				try {
					m = expectedOutputRequest.getClass().getMethod("setInteraction", new Class[]{Interaction.class});
					m.invoke(expectedOutputRequest, new Interaction("")) ;	  
				} catch (SecurityException e) {
            ...
            to.add(new OutputEntity(expectedOutputRequest,TaskOperations.CreateMS));
            }
	...
	private boolean initialiseConversationalTask(RuntimeConversation conversation, Task tobject) {
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		expectedInput=this.getMSM().obtainConversationalMentalEntityByType(conversation,"Request");
		if (this.getLM().canBeDeleted(expectedInput)){                          
             if (expectedInput.size()==0){
				nonExistingInputs.add("Request");
			 } else {
		...
		validConversationType=validConversationType||
				conversation.getInteraction().getId().equalsIgnoreCase("SampleInteraction");
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	repeatedOutputs.clear();
  	   	boolean correctRole=conversation.getPlayedRole().equals ("Initiator");
       	...
       	if (tobject.getType().equals("TaskProcessingTheAnswer") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
		...
		boolean allEntitiesExist=true;
            ...
            expectedInput=this.getMSM().obtainConversationalMentalEntityByType(conversation,"Answer");
			if (expectedInput.size()==0 && !("1".equals("0..n")))
				nonExistingInputs.add("Answer");
			else {
			    addExpectedInputs(tobject, "Answer","1",expectedInput);
			    addConsumedInput(to, "1", expectedInput);
			}
	      allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
	...
	public Vector tasksThatSatisfyGoal(String goalname){
	     ...
	     typesOfConversation.add("SampleInteraction");
		 
         
         if (goalname.equals("Collaborate")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new TaskProcessingTheAnswerTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal Collaborate",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
					}
		 ...
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new InteractionLaunchingTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("TaskProcessingTheAnswer");					
         
         
         
         
         
         ttypes.add("InteractionLaunchingTask");		         
   ...
   getCM().addKnownProtocol("SampleInteraction");
   ...
   sg= new ingenias.editor.entities.StateGoal("Collaborate");
   sg.setState("pending");
      try {
   ...
   java.awt.event.ActionListener ifPressed=null;
     ...
     final JADEAgent _jaInitiatorSampleInteraction=this;
     ifPressed=new java.awt.event.ActionListener() {
       public void actionPerformed(ActionEvent e) {
         _jaInitiatorSampleInteraction.addBehaviour(
           ...
           public void action() {
			 ...
			 if (allexist){
              ...
              if (getGraphics()!=null)
                getGraphics().runtimeWarning(" Mental conditions required for starting InitiatorSampleInteraction "+
     ...
     if (getGraphics()!=null)
      getGraphics().addInteraction(this.getName(), "Role: Initiator - Int: SampleInteraction", ifPressed);
     getCM().addInitiatorRoles("SampleInteraction","Initiator");
