...
public class UIColaboratesEdge extends NAryEdge {

  // Constructor with data content
  public UIColaboratesEdge(ingenias.editor.entities.UIColaborates userObject) {
    super(userObject);
    
     this.addRole("UIColaboratessource");
     this.setArity("UIColaboratessource",true,1);
     this.setArity("UIColaboratessource",false,1);
     
this.addClass("UIColaboratessource","ingenias.editor.entities.Interaction");
     
this.addClass("UIColaboratessource","ingenias.editor.entities.InteractionUnit");
     
    
     this.addRole("UIColaboratestarget");
     this.setArity("UIColaboratestarget",true,1);
     this.setArity("UIColaboratestarget",false,1);
     
this.addClass("UIColaboratestarget","ingenias.editor.entities.Agent");
     
this.addClass("UIColaboratestarget","ingenias.editor.entities.Role");
     
    
     this.addRole("UIExecutestarget");
     this.setArity("UIExecutestarget",true,0);
     this.setArity("UIExecutestarget",false,2147483647);
     
this.addClass("UIExecutestarget","ingenias.editor.entities.Task");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof UIColaboratesEdge)))
      return false;

    // n-edge to be considered.
    UIColaboratesEdge edge = new UIColaboratesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof UIColaboratesEdge)
      edge = (UIColaboratesEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("UIColaboratessource")){
 	 edges.add( new DefaultEdge( new UIColaboratessourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("UIColaboratestarget")){
 	 edges.add( new DefaultEdge( new UIColaboratestargetRole() ) );
	 }
