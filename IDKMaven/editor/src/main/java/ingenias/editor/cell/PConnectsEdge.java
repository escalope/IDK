...
public class PConnectsEdge extends NAryEdge {

  // Constructor with data content
  public PConnectsEdge(ingenias.editor.entities.PConnects userObject) {
    super(userObject);
    
     this.addRole("PConnectssource");
     this.setArity("PConnectssource",true,1);
     this.setArity("PConnectssource",false,1);
     
this.addClass("PConnectssource","ingenias.editor.entities.Task");
     
    
     this.addRole("PConnectstarget");
     this.setArity("PConnectstarget",true,1);
     this.setArity("PConnectstarget",false,2147483647);
     
this.addClass("PConnectstarget","ingenias.editor.entities.Task");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof PConnectsEdge)))
      return false;

    // n-edge to be considered.
    PConnectsEdge edge = new PConnectsEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof PConnectsEdge)
      edge = (PConnectsEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("PConnectssource")){
 	 edges.add( new DefaultEdge( new PConnectssourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("PConnectstarget")){
 	 edges.add( new DefaultEdge( new PConnectstargetRole() ) );
	 }
