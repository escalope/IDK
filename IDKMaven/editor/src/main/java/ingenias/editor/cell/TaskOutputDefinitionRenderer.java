...
public class TaskOutputDefinitionRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("TaskOutputDefinition",
	index,"/rendererxml/TaskOutputDefinitionUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("TaskOutputDefinition",
	index,"/rendererxml/TaskOutputDefinitionINGENIASPanel.xml");
     ...
     */
    public TaskOutputDefinitionRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("TaskOutputDefinition",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("TaskOutputDefinition",current);
    }
  ...
  public static void setEntity(TaskOutputDefinition ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("TaskOutputDefinition",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("TaskOutputDefinition",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("TaskOutputDefinition",current);
	}
