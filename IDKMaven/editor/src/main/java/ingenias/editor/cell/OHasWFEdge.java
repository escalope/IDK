...
public class OHasWFEdge extends NAryEdge {

  // Constructor with data content
  public OHasWFEdge(ingenias.editor.entities.OHasWF userObject) {
    super(userObject);
    
     this.addRole("OHasWFsource");
     this.setArity("OHasWFsource",true,1);
     this.setArity("OHasWFsource",false,1);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof OHasWFEdge)))
      return false;

    // n-edge to be considered.
    OHasWFEdge edge = new OHasWFEdge( null);
