...
public class GTDecomposesOREdge extends NAryEdge {

  // Constructor with data content
  public GTDecomposesOREdge(ingenias.editor.entities.GTDecomposesOR userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof GTDecomposesOREdge)))
      return false;

    // n-edge to be considered.
    GTDecomposesOREdge edge = new GTDecomposesOREdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof GTDecomposesOREdge)
      edge = (GTDecomposesOREdge) selectedEdge;
