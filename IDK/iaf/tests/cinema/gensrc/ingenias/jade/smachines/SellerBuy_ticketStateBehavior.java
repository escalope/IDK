...
private MentalStateReader msr=null;
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "Buy_ticket","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("I_do_not_want_it") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      this.notifyStateTransitionExecuted("I_do_not_want_it", "waiting for I_do_not_want_it");
  } 
  ...
  if (this.isState("I_want_it") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      this.notifyStateTransitionExecuted("I_want_it", "waiting for I_want_it");
  } 
  ...
  if (this.isState("ObtainTicket") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      this.notifyStateTransitionExecuted("ObtainTicket", "waiting for ObtainTicket");
  } 
  ...
  if (this.isState("TicketRequestRefused")) {
     ...
     try {
      ...
      if (this.getDCC().notifyMessageSent("TicketRequestRefused",optionsA,this)){
           ...
           getTimeout().stop();
            this.notifyStateTransitionExecuted("TicketRequestRefused", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("TicketRequestRefused", "ABORTED");
    	  } else  {
  ...
  if (this.isState("TicketExists")) {
     ...
     try {
      ...
      if (this.getDCC().notifyMessageSent("TicketExists",optionsA,this)){
           ...
           getTimeout().stop();
            this.notifyStateTransitionExecuted("TicketExists", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("TicketExists", "ABORTED");
    	  } else  {
    		  if (!getTimeout().isStarted())
    		  getTimeout().start(0);
    	  }
  ...
  if (this.isState("SellTicket")) {
     ...
     try {
      ...
      if (this.getDCC().notifyMessageSent("SellTicket",optionsA,this)){
           ...
           getTimeout().stop();
            this.notifyStateTransitionExecuted("SellTicket", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("SellTicket", "ABORTED");
    	  } else  {
    		  if (!getTimeout().isStarted())
    		  getTimeout().start(0);
    	  }
