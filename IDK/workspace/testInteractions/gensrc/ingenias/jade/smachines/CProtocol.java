...
public class CProtocol implements AgentProtocols{
	...
	public ActiveConversation initialiseProtocols(String agentName,RuntimeConversation conv, 
        ...
        // Adds a protocol to the protocol queue
          StateBehavior   sb=null;
		   ...
		   if (conv.getInteraction().getId().equals("IntExample") && conv.getPlayedRole().equals("Participant2")){
		   	DefaultCommControl dcc=new Participant2IntExampleDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new Participant2IntExampleStateBehavior(agentName,
				msr, msu,
				conv,                           		
				conv.getPlayedRole(),
			   	actors,
