...
public class SupervisorRoleAlphaQualityInspectionRequestStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public SupervisorRoleAlphaQualityInspectionRequestStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "AlphaQualityInspectionRequestIU");
			    
			    
			      // Receiving a message    
			      smf.add("AlphaQualityDegreeIU", "waiting for AlphaQualityDegreeIU");
			      
			      // Next states after receiving "AlphaQualityDegreeIU"
			      
			       smf.add("waiting for AlphaQualityDegreeIU","endAlphaQualityDegreeIU");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("AlphaQualityInspectionRequestIU", "AlphaQualityDegreeIU");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
  	try {  	  
      String[] options=new String[]{"AlphaQualityInspectionRequestIU"};
      AID[] actors=null;
      Vector<AID> actorsv=new Vector<AID>();
      Vector<String> rolesv=new Vector<String>();
      
      actorsv.addAll(this.getActor("SourcesAlphaQualityInspectorRole"));
      {      
       Vector<AID> receivers=this.getActor("SourcesAlphaQualityInspectorRole");      
       for (AID aid:receivers){
        rolesv.add("SourcesAlphaQualityInspectorRole");
       }
      }
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","AlphaQualityInspectionRequest",completelistInvolvedActors);
  ...
  if (this.isState("AlphaQualityDegreeIU") ) {  // State changed by other agent and upted
    //in the parent class
         
      Vector options=new Vector();
      
      options.add("endAlphaQualityDegreeIU");
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"AlphaQualityDegreeIU",
                   "AlphaQualityInspectionRequest",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("AlphaQualityDegreeIU");
      this.addState("waiting for AlphaQualityDegreeIU");
      this.notifyStateTransitionExecuted("AlphaQualityDegreeIU", "waiting for AlphaQualityDegreeIU");
  } 
  ...
  if (this.isState("AlphaQualityInspectionRequestIU")) {
     ...
     try {
      ...
      options.add("AlphaQualityDegreeIU");      
      
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("AlphaQualityInspectionRequestIU",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "AlphaQualityInspectionRequestIU","AlphaQualityInspectionRequest",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("AlphaQualityInspectionRequestIU", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("AlphaQualityInspectionRequestIU", "ABORTED");
    	  } else  {
  ...
  if (this.isState("endAlphaQualityDegreeIU")) {
