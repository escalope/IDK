...
public class MemberDplmntEdge extends NAryEdge {

  // Constructor with data content
  public MemberDplmntEdge(ingenias.editor.entities.MemberDplmnt userObject) {
    super(userObject);
    
     this.addRole("MemberDplmntsource");
     this.setArity("MemberDplmntsource",true,1);
     this.setArity("MemberDplmntsource",false,1);
     
this.addClass("MemberDplmntsource","ingenias.editor.entities.GroupDeploymentUnit");
     
    
     this.addRole("MemberDplmnttarget");
     this.setArity("MemberDplmnttarget",true,1);
     this.setArity("MemberDplmnttarget",false,2147483647);
     
this.addClass("MemberDplmnttarget","ingenias.editor.entities.DeploymentUnitByType");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof MemberDplmntEdge)))
      return false;

    // n-edge to be considered.
    MemberDplmntEdge edge = new MemberDplmntEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof MemberDplmntEdge)
      edge = (MemberDplmntEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("MemberDplmntsource")){
 	 edges.add( new DefaultEdge( new MemberDplmntsourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("MemberDplmnttarget")){
 	 edges.add( new DefaultEdge( new MemberDplmnttargetRole() ) );
	 }
