...
public class GroupDeploymentUnitRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("GroupDeploymentUnit",
	index,"/rendererxml/GroupDeploymentUnitUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("GroupDeploymentUnit",
	index,"/rendererxml/GroupDeploymentUnitINGENIASPanel.xml");
     ...
     */
    public GroupDeploymentUnitRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("GroupDeploymentUnit",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("GroupDeploymentUnit",current);
    }
  ...
  public static void setEntity(GroupDeploymentUnit ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("GroupDeploymentUnit",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
      ...
      if (currentMap.get("InstGroup")!=null){
	   if (ent!=null && ent.getInstGroup()!=null){
		if (currentMap.get("InstGroup") instanceof javax.swing.JLabel){
		((javax.swing.JLabel)( currentMap).get("InstGroup")).setText(ent.getInstGroup().toString());
		} else {
		 if (currentMap.get("InstGroup") instanceof javax.swing.text.JTextComponent)
		 ((javax.swing.text.JTextComponent)( currentMap).get("InstGroup")).setText(ent.getInstGroup().toString());
		
	      } 
	   } else  {
	     if (currentMap.get("InstGroup") instanceof javax.swing.JLabel)
	     ((javax.swing.JLabel)( currentMap).get("InstGroup")).setText("");
	     else {
		if (!(currentMap.get("InstGroup") instanceof ingenias.editor.rendererxml.CollectionPanel)) 
		((javax.swing.text.JTextComponent)( currentMap).get("InstGroup")).setText("");
	     }
         }
	  }
	...
	{
	return RenderComponentManager.retrievePanel("GroupDeploymentUnit",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("GroupDeploymentUnit",current);
	}
