...
public class AMIContextRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("AMIContext",
	index,"/rendererxml/AMIContextUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("AMIContext",
	index,"/rendererxml/AMIContextINGENIASPanel.xml");
     ...
     */
    public AMIContextRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("AMIContext",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("AMIContext",current);
    }
  ...
  public static void setEntity(AMIContext ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("AMIContext",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("AMIContext",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("AMIContext",current);
	}
