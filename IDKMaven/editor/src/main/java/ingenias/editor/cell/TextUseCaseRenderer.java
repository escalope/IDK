...
public class TextUseCaseRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("TextUseCase",
	index,"/rendererxml/TextUseCaseUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("TextUseCase",
	index,"/rendererxml/TextUseCaseINGENIASPanel.xml");
     ...
     */
    public TextUseCaseRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("TextUseCase",ent.getPrefs(map).getView());
  ...
  public static void setEntity(TextUseCase ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("TextUseCase",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("TextUseCase",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("TextUseCase",current);
	}
