...
import ingenias.exception.NotFound;
  ...
  public class BuyerBuyerAssignmentDefaultCommControl extends DefaultCommControl{
  ...
  public BuyerBuyerAssignmentDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("Accept_assistency_proposal");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("Reject_assistency_proposal");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("Accept_assistency_proposal");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("Reject_assistency_proposal");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("AcceptBecomingAssistant") &&
    	 sb.isState("AcceptBecomingAssistant")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Accept_assistency_proposal").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("AcceptBecomingAssistant");
    ...
    if (stateToEvaluate.equals("RejectBecomingAssistant") &&
    	 sb.isState("RejectBecomingAssistant")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Reject_assistency_proposal").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("RejectBecomingAssistant");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for RequestBeingAssistant")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("RequestBeingAssistant")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for RequestBeingAssistant");           
    	   //try {
