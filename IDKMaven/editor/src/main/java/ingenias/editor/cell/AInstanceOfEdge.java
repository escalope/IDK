...
public class AInstanceOfEdge extends NAryEdge {

  // Constructor with data content
  public AInstanceOfEdge(ingenias.editor.entities.AInstanceOf userObject) {
    super(userObject);
    
     this.addRole("AInstanceOfsource");
     this.setArity("AInstanceOfsource",true,1);
     this.setArity("AInstanceOfsource",false,2147483647);
     
this.addClass("AInstanceOfsource","ingenias.editor.entities.Agent");
     
this.addClass("AInstanceOfsource","ingenias.editor.entities.Role");
     
    
     this.addRole("AInstanceOftarget");
     this.setArity("AInstanceOftarget",true,1);
     this.setArity("AInstanceOftarget",false,1);
     
this.addClass("AInstanceOftarget","ingenias.editor.entities.AutonomousEntityQuery");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AInstanceOfEdge)))
      return false;

    // n-edge to be considered.
    AInstanceOfEdge edge = new AInstanceOfEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AInstanceOfEdge)
      edge = (AInstanceOfEdge) selectedEdge;
