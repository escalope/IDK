...
public class AGORelationshipGroupEdge extends NAryEdge {

  // Constructor with data content
  public AGORelationshipGroupEdge(ingenias.editor.entities.AGORelationshipGroup userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AGORelationshipGroupEdge)))
      return false;

    // n-edge to be considered.
    AGORelationshipGroupEdge edge = new AGORelationshipGroupEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AGORelationshipGroupEdge)
      edge = (AGORelationshipGroupEdge) selectedEdge;
