...
public class ColAgent2JADEAgent
		 extends JADEAgent {         
 
		 public ColAgent2JADEAgent(){
		 super(new ColAgent2Protocol(),new ColAgent2InteractionLocks());
		 }
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
		...
		expectedInput=this.getMSM().getMentalEntityByType("Answer");
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
			    ...
			    addExpectedInputs(tobject, "Answer","1",expectedInput);
             	addConsumedInput(to,"Answer",expectedInput);
			 }
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	...
  	   	boolean correctRole=conversation.getPlayedRole().equals ("CustomColaborator1");
       	...
       	if (tobject.getType().equals("CustomizedTaskProcessingTheRequest") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
        ...
        /*     
	     ...
	     boolean alreadyExists=true;
		    ...
		    {Answer expectedOutputAnswer=		    
		     new Answer(MentalStateManager.generateMentalEntityID());			
             to.add(new OutputEntity(expectedOutputAnswer,TaskOperations.CreateWF));
            }
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	repeatedOutputs.clear();
  	   	boolean correctRole=conversation.getPlayedRole().equals ("Colaborator");
  	   	// Now all ascendant roles are verified, to enable tasks belonging to roles specializing a more
  	   	// generic one involved in an interaction
       	...
       	if (tobject.getType().equals("TaskProcessingTheRequest") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
         ...
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new CustomizedTaskProcessingTheRequestTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
         ...
         if (goalname.equals("Collaborate")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				boolean canbescheduled=false;
				for (int k=0;k<conversations.size();k++){
					tobject=new TaskProcessingTheRequestTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal Collaborate",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("CustomizedTaskProcessingTheRequest");					
         
         
                  
         
                   
         ttypes.add("TaskProcessingTheRequest");					
