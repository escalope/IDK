...
@Test
  public void testDemo() throws TimeOut{			
			...
			MentalStateManager msm = MSMRepository.getInstance().waitFor("A_0"); // provides access to the
			// mental state of the agent
			MentalStateProcessor msp = MSPRepository.getInstance().waitFor("A_0"); 
			TestUtils.waitForAgentInitialised(msp);
			...
			assertTrue("There should be two SingleInputTaskFact entities",frameFactEntitiesBefore.size()==2);
