...
public class ContributeEdge extends NAryEdge {

  // Constructor with data content
  public ContributeEdge(ingenias.editor.entities.Contribute userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof ContributeEdge)))
      return false;

    // n-edge to be considered.
    ContributeEdge edge = new ContributeEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof ContributeEdge)
      edge = (ContributeEdge) selectedEdge;
