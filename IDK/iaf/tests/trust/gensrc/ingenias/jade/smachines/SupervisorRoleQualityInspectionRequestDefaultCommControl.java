...
import ingenias.exception.NotFound;
  ...
  public class SupervisorRoleQualityInspectionRequestDefaultCommControl extends DefaultCommControl{
  ...
  public SupervisorRoleQualityInspectionRequestDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("InspectQualitySource");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("InspectQualitySource");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("QualityInspectionRequestIU") &&
    	 sb.isState("QualityInspectionRequestIU")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"InspectQualitySource").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("QualityInspectionRequestIU");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"InspectQualitySource");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("InspectQualitySource");
	       lockProcessed("InspectQualitySource");
           //MainInteractionManager.log("Removing lock InspectQualitySource",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("QualityDegreeIU");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for QualityDegreeIU")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("QualityDegreeIU")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for QualityDegreeIU");           
    	   //try {
		   ...
		   futureStates.add("endQualityDegreeIU");
