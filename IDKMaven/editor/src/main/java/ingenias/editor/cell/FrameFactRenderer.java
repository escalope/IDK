...
public class FrameFactRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("FrameFact",
	index,"/rendererxml/FrameFactUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("FrameFact",
	index,"/rendererxml/FrameFactINGENIASPanel.xml");
     ...
     */
    public FrameFactRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("FrameFact",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("FrameFact",current);
    }
  ...
  public static void setEntity(FrameFact ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("FrameFact",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("FrameFact",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("FrameFact",current);
	}
