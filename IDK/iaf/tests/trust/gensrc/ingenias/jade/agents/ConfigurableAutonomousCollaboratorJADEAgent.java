...
public class ConfigurableAutonomousCollaboratorJADEAgent
		 extends JADEAgent {         
 
		 public ConfigurableAutonomousCollaboratorJADEAgent(){
		 super(new ConfigurableAutonomousCollaboratorProtocol(),new ConfigurableAutonomousCollaboratorInteractionLocks());
		 }
		...
		if (!allEntitiesExist){
		   ...
		   for (int j=0;j<nonExistingInputs.size();j++){
			...
			EventManager.getInstance().conversationalInitializationOfTaskFailed(getLocalName(), 
												"ConfigurableAutonomousCollaborator", 
												tobject, nonexisting);
		}
		return initialised;	       
	      }
         ...
         nonExistingInputs.clear();
  		 repeatedOutputs.clear();
         if (tobject.getType().equals("ConfigureStartupSequence") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("PreConfiguredInformationSourceList");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("PreConfiguredInformationSourceList");
			 } else {
			    addExpectedInputs(tobject, "PreConfiguredInformationSourceList","1",expectedInput);
             	addConsumedInput(to,"1",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
	     ...
	     expectedApp=(ingenias.jade.components.Application)getAM().getApplication("NewCycleEventGenerator");
             tobject.addApplication("NewCycleEventGenerator",expectedApp);
	     ...
	     tobject.addOutput(to);
     	      ...
     	      if (!allEntitiesExist){
				 ...
				 EventManager.getInstance().conversationalInitializationOfTaskFailed(
				 			getLocalName(), "ConfigurableAutonomousCollaborator", 
												tobject, nonexisting);
	...
	public Vector tasksThatSatisfyGoal(String goalname){
		 ...
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new ConfigureStartupSequenceTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
				 canbescheduled=initialiseNonConversationalTask(tobject);
			 	 if (canbescheduled){
					//MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal EnrichTheSystemWithGoodInfoSources",getLocalName()+"-"+tobject.getType());
					tasks.add(tobject);
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("ConfigureStartupSequence");		         
   ...
   sg.setState("pending");
	...
	Vector evetns=null;
     ...
     app=NewCycleEventGeneratorInit.createInstance(this);
	 //app.registerOwner(this);
	 	    
     this.getAM().addApplication("NewCycleEventGenerator",app);        
	 events=new Vector();
	 actions=new Vector();
	 ...
	 */ 
	 ...
	 if (getGraphics()!=null)
	  getGraphics().addApplication("NewCycleEventGenerator", events,actions);    
   ...
   java.awt.event.ActionListener ifPressed=null;
	...
	public DFAgentDescription[ ] getDescription() {
		...
		sd.setType("ConfigurableCollaboratorRole");
