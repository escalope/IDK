...
public class AUMLPortRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("AUMLPort",
	index,"/rendererxml/AUMLPortUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("AUMLPort",
	index,"/rendererxml/AUMLPortINGENIASPanel.xml");
     ...
     */
    public AUMLPortRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("AUMLPort",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("AUMLPort",current);
    }
  ...
  public static void setEntity(AUMLPort ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("AUMLPort",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("AUMLPort",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("AUMLPort",current);
	}
