...
public class GeneralizesEdge extends NAryEdge {

  // Constructor with data content
  public GeneralizesEdge(ingenias.editor.entities.Generalizes userObject) {
    super(userObject);
    
     this.addRole("Includessource");
     this.setArity("Includessource",true,1);
     this.setArity("Includessource",false,1);
     
this.addClass("Includessource","ingenias.editor.entities.UseCase");
     
    
     this.addRole("Includestarget");
     this.setArity("Includestarget",true,1);
     this.setArity("Includestarget",false,2147483647);
     
this.addClass("Includestarget","ingenias.editor.entities.UseCase");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof GeneralizesEdge)))
      return false;

    // n-edge to be considered.
    GeneralizesEdge edge = new GeneralizesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof GeneralizesEdge)
      edge = (GeneralizesEdge) selectedEdge;
