...
public class SourcesAlfaQualityInspectorRoleSolicitudInspeccionAlfaCalidadStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public SourcesAlfaQualityInspectorRoleSolicitudInspeccionAlfaCalidadStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","SolicitudInspeccionAlfaCalidadUI");
			    
			    
			    
			      // Receiving a message    
			      smf.add("SolicitudInspeccionAlfaCalidadUI", "waiting for SolicitudInspeccionAlfaCalidadUI");
			      
			      // Next states after receiving "SolicitudInspeccionAlfaCalidadUI"
			      
			       smf.add("waiting for SolicitudInspeccionAlfaCalidadUI","GradoCalidadCuarentenaListoUI");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("GradoCalidadCuarentenaListoUI", "endGradoCalidadCuarentenaListoUI");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "SolicitudInspeccionAlfaCalidad","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("SolicitudInspeccionAlfaCalidadUI") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"SolicitudInspeccionAlfaCalidadUI",
                   "SolicitudInspeccionAlfaCalidad",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("SolicitudInspeccionAlfaCalidadUI");
      this.addState("waiting for SolicitudInspeccionAlfaCalidadUI");
      this.notifyStateTransitionExecuted("SolicitudInspeccionAlfaCalidadUI", "waiting for SolicitudInspeccionAlfaCalidadUI");
  } 
  ...
  if (this.isState("GradoCalidadCuarentenaListoUI")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("GradoCalidadCuarentenaListoUI",optionsA,this)){
           ...
           "GradoCalidadCuarentenaListoUI","SolicitudInspeccionAlfaCalidad",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("GradoCalidadCuarentenaListoUI", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("GradoCalidadCuarentenaListoUI", "ABORTED");
    	  } else  {
