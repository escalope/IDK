...
public class GTOrDependsEdge extends NAryEdge {

  // Constructor with data content
  public GTOrDependsEdge(ingenias.editor.entities.GTOrDepends userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof GTOrDependsEdge)))
      return false;

    // n-edge to be considered.
    GTOrDependsEdge edge = new GTOrDependsEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof GTOrDependsEdge)
      edge = (GTOrDependsEdge) selectedEdge;
