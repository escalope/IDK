...
private MentalStateReader msr=null;
  public SupervisorRolePerformProposalStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","PropuestaIU");
			    
			    
			    
			      // Receiving a message    
			      smf.add("PropuestaIU", "waiting for PropuestaIU");
			      
			      // Next states after receiving "PropuestaIU"
			      
			       smf.add("waiting for PropuestaIU","PropuestaRechazadaIU");
			      
			       smf.add("waiting for PropuestaIU","PropuestaAceptadaIU");
  ...
  if (this.isState("PropuestaIU") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"PropuestaIU",
                   "PerformProposal",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("PropuestaIU");
      this.addState("waiting for PropuestaIU");
      this.notifyStateTransitionExecuted("PropuestaIU", "waiting for PropuestaIU");
  } 
  ...
  if (this.isState("PropuestaAceptadaIU")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("PropuestaAceptadaIU",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "PropuestaAceptadaIU","PerformProposal",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("PropuestaAceptadaIU", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("PropuestaAceptadaIU", "ABORTED");
    	  } else  {
    		  if (!getTimeout().isStarted())
    		  getTimeout().start(0);
    	  }
