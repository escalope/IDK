...
public class TimedOutCommunicationBothParticipant2 {
@Test
  public void testDemo() throws TimeOut{			
	...
	TestUtils.waitForAgentInitialised(mspC3);
	...
	TestUtils.waitForAgentInitialised(mspC4);
	TestUtils.snapShot(msmC4,"snapC4Initial");
	
			
	assertTrue("There should be once instance of  FailAllParticipant2",msmA
			.getMentalEntityByType("FailAllParticipant2").size()==1);
							
	MainInteractionManager.goAutomatic(); // tells the agents to start working
    TestUtils.doNothing(6000); // waits for 2 second and the interaction should be finished by then
	...
	TestUtils.checkNOExistenceMEWithinMS(msmA, "InitialFact", "ATimeOut");
	TestUtils.checkNOExistenceMEWithinConv(conv, "InitialFact", "ATimeOut");
	...
	conv = TestUtils.checkFirstConversation(msmC3,"C_3","aborted");
...
private void checkAgent(MentalStateManager msmC0,
	...
	TestUtils.checkNOExistenceMEWithinConv(conv, "MultipleAnswer", agentName);
