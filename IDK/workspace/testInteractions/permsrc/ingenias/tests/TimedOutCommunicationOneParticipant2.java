...
public class TimedOutCommunicationOneParticipant2 {
@Test
  public void testDemo() throws TimeOut{			
	...
	assertTrue("There should be once instance of  FailOneParticipan2",msmA
			.getMentalEntityByType("FailOneParticipan2").size()==1);
	...
	ingenias.editor.entities.RuntimeConversation  conv = TestUtils.checkFirstConversation(msmA,"ATimeOut","FINISHED");
	...
	TestUtils.checkExistenceMEWithinConv(conv, "SingleQuestion", "ATimeOut",1);

	TestUtils.checkNOExistenceMEWithinMS(msmA, "FailOneParticipan2", "ATimeOut");
	TestUtils.checkNOExistenceMEWithinConv(conv, "FailOneParticipan2", "ATimeOut");
	
	//*************************************************
	// Agent B1
	//*************************************************
	
	conv= TestUtils.checkFirstConversation(msmB1,"B_1","FINISHED");						
	TestUtils.checkExistenceMEWithinConv(conv, "SingleAnswer", "B_1",1);			
	...
	assertTrue("Agent C_0 should remain the same and I found the following differences "+differencesC0+
			". Original mental state was "+TestUtils.getSnapshotString("snapC0Initial"),differencesC0.size()==0);
	
	//*************************************************
	// Agent C1
	//*************************************************
	Vector<MentalEntity> differencesC1 = TestUtils.compareCurrentStateAgainstSnapshot(msmC1, "snapC1Initial");
	assertTrue("Agent C_1 should remain the same and I found the following differences "+differencesC1+
			". Original mental state was "+TestUtils.getSnapshotString("snapC1Initial"),differencesC1.size()==0);
	
	//*************************************************
	// Agent C2
	//*************************************************
	conv = TestUtils.checkFirstConversation(msmC2,"C_2","aborted");
	checkAgent(msmC2, conv,"C_2");
	TestUtils.checkNOExistenceMEWithinMS(msmC2, "MultipleAnswer", "C_2");
	TestUtils.checkNOExistenceMEWithinConv(conv, "MultipleAnswer", "C_2");
	...
	TestUtils.checkNOExistenceMEWithinMS(msmC2, "MultipleAnswer", "C_3");
	TestUtils.checkExistenceMEWithinConv(conv, "MultipleAnswer", "C_3",1);
	...
	assertTrue("Agent C_4 should remain the same and I found the following differences "+differencesC4
			+". Original mental state was "+TestUtils.getSnapshotString("snapC4Initial"),differencesC4.size()==0);  	
	}
