...
public class CtxtNotifiesEdge extends NAryEdge {

  // Constructor with data content
  public CtxtNotifiesEdge(ingenias.editor.entities.CtxtNotifies userObject) {
    super(userObject);
    
     this.addRole("CtxtNotifiessource");
     this.setArity("CtxtNotifiessource",true,1);
     this.setArity("CtxtNotifiessource",false,1);
     
this.addClass("CtxtNotifiessource","ingenias.editor.entities.AMIContext");
     
this.addClass("CtxtNotifiessource","ingenias.editor.entities.FAERIECtxtElement");
     
    
     this.addRole("CtxtNotifiestarget");
     this.setArity("CtxtNotifiestarget",true,1);
     this.setArity("CtxtNotifiestarget",false,2147483647);
     
this.addClass("CtxtNotifiestarget","ingenias.editor.entities.Agent");
     
this.addClass("CtxtNotifiestarget","ingenias.editor.entities.Role");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof CtxtNotifiesEdge)))
      return false;

    // n-edge to be considered.
    CtxtNotifiesEdge edge = new CtxtNotifiesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof CtxtNotifiesEdge)
      edge = (CtxtNotifiesEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("CtxtNotifiessource")){
 	 edges.add( new DefaultEdge( new CtxtNotifiessourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("CtxtNotifiestarget")){
 	 edges.add( new DefaultEdge( new CtxtNotifiestargetRole() ) );
	 }
