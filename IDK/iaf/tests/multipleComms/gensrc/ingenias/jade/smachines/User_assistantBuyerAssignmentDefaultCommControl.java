...
import ingenias.exception.NotFound;
  ...
  public class User_assistantBuyerAssignmentDefaultCommControl extends DefaultCommControl{
  ...
  public User_assistantBuyerAssignmentDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("BuyerAssignmentProposal");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("BuyerAssignmentProposal");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("RequestBeingAssistant") &&
    	 sb.isState("RequestBeingAssistant")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"BuyerAssignmentProposal").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("RequestBeingAssistant");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"BuyerAssignmentProposal");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("BuyerAssignmentProposal");
	       lockProcessed("BuyerAssignmentProposal");
           //MainInteractionManager.log("Removing lock BuyerAssignmentProposal",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("RejectBecomingAssistant");
                  
		   futureStates.add("AcceptBecomingAssistant");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for RejectBecomingAssistant")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("RejectBecomingAssistant")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for RejectBecomingAssistant");           
    	   //try {
		   ...
		   futureStates.add("endRejectBecomingAssistant");
          
          processed = true;
      	 }      	 
    } 
   
    if (sb.isState("waiting for AcceptBecomingAssistant")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("AcceptBecomingAssistant")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for AcceptBecomingAssistant");           
    	   //try {
                    Vector toAdd=new Vector();
					for (ACLMessage singleMessage:multipleMessages){
						 String content=singleMessage.getContent();    	 				
		   ...
		   futureStates.add("endAcceptBecomingAssistant");
