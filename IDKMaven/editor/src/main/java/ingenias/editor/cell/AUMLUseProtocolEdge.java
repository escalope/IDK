...
public class AUMLUseProtocolEdge extends NAryEdge {

  // Constructor with data content
  public AUMLUseProtocolEdge(ingenias.editor.entities.AUMLUseProtocol userObject) {
    super(userObject);
    
     this.addRole("AUMLUseProtocolsource");
     this.setArity("AUMLUseProtocolsource",true,1);
     this.setArity("AUMLUseProtocolsource",false,1);
     
this.addClass("AUMLUseProtocolsource","ingenias.editor.entities.AUMLPort");
     
    
     this.addRole("AUMLUseProtocoltarget");
     this.setArity("AUMLUseProtocoltarget",true,1);
     this.setArity("AUMLUseProtocoltarget",false,1);
     
this.addClass("AUMLUseProtocoltarget","ingenias.editor.entities.AUMLPort");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AUMLUseProtocolEdge)))
      return false;

    // n-edge to be considered.
    AUMLUseProtocolEdge edge = new AUMLUseProtocolEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AUMLUseProtocolEdge)
      edge = (AUMLUseProtocolEdge) selectedEdge;
