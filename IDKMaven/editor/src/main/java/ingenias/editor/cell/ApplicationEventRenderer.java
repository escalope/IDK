...
public class ApplicationEventRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("ApplicationEvent",
	index,"/rendererxml/ApplicationEventUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("ApplicationEvent",
	index,"/rendererxml/ApplicationEventINGENIASPanel.xml");
     ...
     */
    public ApplicationEventRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("ApplicationEvent",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("ApplicationEvent",current);
    }
  ...
  public static void setEntity(ApplicationEvent ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("ApplicationEvent",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("ApplicationEvent",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("ApplicationEvent",current);
	}
