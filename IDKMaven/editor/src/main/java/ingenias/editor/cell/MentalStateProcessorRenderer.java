...
public class MentalStateProcessorRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("MentalStateProcessor",
	index,"/rendererxml/MentalStateProcessorUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("MentalStateProcessor",
	index,"/rendererxml/MentalStateProcessorINGENIASPanel.xml");
     ...
     */
    public MentalStateProcessorRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("MentalStateProcessor",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("MentalStateProcessor",current);
    }
  ...
  public static void setEntity(MentalStateProcessor ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("MentalStateProcessor",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
        ...
        if (currentMap.get("ProcessorDescription")!=null && 
            currentMap.get("ProcessorDescription") instanceof ingenias.editor.rendererxml.CollectionPanel) {
          try {
            ( (ingenias.editor.rendererxml.CollectionPanel) currentMap.get(
                "ProcessorDescription")).setCollection("ProcessorDescription",ent.ProcessorDescription, ent.ProcessorDescription.getType());
          }
	...
	{
	return RenderComponentManager.retrievePanel("MentalStateProcessor",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("MentalStateProcessor",current);
	}
