...
public class AGORelationshipOrgEdge extends NAryEdge {

  // Constructor with data content
  public AGORelationshipOrgEdge(ingenias.editor.entities.AGORelationshipOrg userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AGORelationshipOrgEdge)))
      return false;

    // n-edge to be considered.
    AGORelationshipOrgEdge edge = new AGORelationshipOrgEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AGORelationshipOrgEdge)
      edge = (AGORelationshipOrgEdge) selectedEdge;
