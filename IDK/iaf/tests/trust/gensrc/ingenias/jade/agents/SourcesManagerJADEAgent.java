...
public class SourcesManagerJADEAgent
		 extends JADEAgent {         
 
		 public SourcesManagerJADEAgent(){
		 super(new SourcesManagerProtocol(),new SourcesManagerInteractionLocks());
		 }
	...
	private boolean initialiseConversationalTask(RuntimeConversation conversation, Task tobject) {
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		validConversationType=validConversationType||
				conversation.getInteraction().getId().equalsIgnoreCase("RequestToAddSource");
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	...
  	   	boolean correctRole=conversation.getPlayedRole().equals ("GestorFuentesRole");
       	...
       	if (tobject.getType().equals("ProcesarPeticionIntroducirFuenteTask") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
	...
	public Vector tasksThatSatisfyGoal(String goalname){
	     ...
	     typesOfConversation.add("RequestToAddSource");
		 
         
         if (goalname.equals("DarServicioGestionFuentes")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new ProcesarPeticionIntroducirFuenteTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal DarServicioGestionFuentes",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
					}
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("ProcesarPeticionIntroducirFuenteTask");					
   ...
   sg= new ingenias.editor.entities.StateGoal("DarServicioGestionFuentes");
   sg.setState("pending");
      try {
