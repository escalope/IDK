...
public class ActivityFinalRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("ActivityFinal",
	index,"/rendererxml/ActivityFinalUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("ActivityFinal",
	index,"/rendererxml/ActivityFinalINGENIASPanel.xml");
     ...
     */
    public ActivityFinalRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("ActivityFinal",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("ActivityFinal",current);
    }
  ...
  public static void setEntity(ActivityFinal ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("ActivityFinal",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("ActivityFinal",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("ActivityFinal",current);
	}
