...
public class MoveSubProtocol
  ...
  public MoveSubProtocol(JGraph graph) {
  ...
  private Rectangle computeSubProtocolSize(SubProtocol sp,
                                              ingenias.editor.Model model) {
    ...
    for (int k=0;k<2;k++){
      Enumeration enumeration = sp.getChildrenElements();
  ...
  private void moveAllSubProtocol(ingenias.editor.Model model) {

    for (int k = 0; k < model.getRootCount(); k++) {
      Object current = model.getRootAt(k);
      if (current instanceof SubProtocolCell) {
        this.moveSubProtocol( (SubProtocolCell) current, model);
      }

    }
  }

  private void moveSubProtocol(SubProtocolCell lc,
                               ingenias.editor.Model model) {
    SubProtocol ab = (SubProtocol) lc.getUserObject();
    Enumeration enumeration = ab.getChildrenElements();
    Map elems = new Hashtable();
    Rectangle bounds = this.computeSubProtocolSize(ab, model);
  ...
  public void graphChanged(org.jgraph.event.GraphModelEvent gme) {
    ...
    if (enabled && this.workingObject == null) {
      workingObject = "hello";
      this.moveAllSubProtocol( (ingenias.editor.Model) gme.getSource());
      workingObject = null;
    }
