...
public class GTDestroysEdge extends NAryEdge {

  // Constructor with data content
  public GTDestroysEdge(ingenias.editor.entities.GTDestroys userObject) {
    super(userObject);
    
     this.addRole("GTDestroyssource");
     this.setArity("GTDestroyssource",true,1);
     this.setArity("GTDestroyssource",false,1);
     
this.addClass("GTDestroyssource","ingenias.editor.entities.Task");
     
    
     this.addRole("GTDestroystarget");
     this.setArity("GTDestroystarget",true,1);
     this.setArity("GTDestroystarget",false,2147483647);
     
this.addClass("GTDestroystarget","ingenias.editor.entities.MentalEntity");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof GTDestroysEdge)))
      return false;

    // n-edge to be considered.
    GTDestroysEdge edge = new GTDestroysEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof GTDestroysEdge)
      edge = (GTDestroysEdge) selectedEdge;
