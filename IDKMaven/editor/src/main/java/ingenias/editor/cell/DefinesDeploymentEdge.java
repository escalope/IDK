...
public class DefinesDeploymentEdge extends NAryEdge {

  // Constructor with data content
  public DefinesDeploymentEdge(ingenias.editor.entities.DefinesDeployment userObject) {
    super(userObject);
    
     this.addRole("DefinesDeploymentsource");
     this.setArity("DefinesDeploymentsource",true,1);
     this.setArity("DefinesDeploymentsource",false,1);
...
this.addClass("DefinesDeploymentsource","ingenias.editor.entities.AMIContextInstantiation");
     ...
     this.setArity("DefinesDeploymenttarget",false,1);
     
this.addClass("DefinesDeploymenttarget","ingenias.editor.entities.DeploymentPackage");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof DefinesDeploymentEdge)))
      return false;

    // n-edge to be considered.
    DefinesDeploymentEdge edge = new DefinesDeploymentEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof DefinesDeploymentEdge)
      edge = (DefinesDeploymentEdge) selectedEdge;
