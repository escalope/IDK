...
public class GroupDplmntEdge extends NAryEdge {

  // Constructor with data content
  public GroupDplmntEdge(ingenias.editor.entities.GroupDplmnt userObject) {
    super(userObject);
    
     this.addRole("GroupDplmntsource");
     this.setArity("GroupDplmntsource",true,1);
     this.setArity("GroupDplmntsource",false,1);
     
this.addClass("GroupDplmntsource","ingenias.editor.entities.GroupDeploymentUnit");
     
    
     this.addRole("GroupDplmnttarget");
     this.setArity("GroupDplmnttarget",true,1);
     this.setArity("GroupDplmnttarget",false,2147483647);
     
this.addClass("GroupDplmnttarget","ingenias.editor.entities.GroupDeploymentUnit");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof GroupDplmntEdge)))
      return false;

    // n-edge to be considered.
    GroupDplmntEdge edge = new GroupDplmntEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof GroupDplmntEdge)
      edge = (GroupDplmntEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("GroupDplmntsource")){
 	 edges.add( new DefaultEdge( new GroupDplmntsourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("GroupDplmnttarget")){
 	 edges.add( new DefaultEdge( new GroupDplmnttargetRole() ) );
	 }
