...
public class DeploymentUnitByTypeRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("DeploymentUnitByType",
	index,"/rendererxml/DeploymentUnitByTypeUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("DeploymentUnitByType",
	index,"/rendererxml/DeploymentUnitByTypeINGENIASPanel.xml");
     ...
     */
    public DeploymentUnitByTypeRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("DeploymentUnitByType",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("DeploymentUnitByType",current);
    }
  ...
  public static void setEntity(DeploymentUnitByType ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("DeploymentUnitByType",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("DeploymentUnitByType",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("DeploymentUnitByType",current);
	}
