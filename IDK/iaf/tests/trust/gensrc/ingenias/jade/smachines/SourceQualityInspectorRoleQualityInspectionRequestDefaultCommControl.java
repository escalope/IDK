...
import ingenias.exception.NotFound;
  ...
  public class SourceQualityInspectorRoleQualityInspectionRequestDefaultCommControl extends DefaultCommControl{
  
  	  private Vector<String> previous=new Vector<String>();
  	  
  public SourceQualityInspectorRoleQualityInspectionRequestDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  };
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("GradoCalidadFuente");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("GradoCalidadListaUI") &&
    	 sb.isState("GradoCalidadListaUI")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"GradoCalidadFuente").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("GradoCalidadListaUI");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"GradoCalidadFuente");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLock(mfcontent);
	       lockProcessed("GradoCalidadFuente");
           //MainInteractionManager.log("Removing lock GradoCalidadFuente",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("endGradoCalidadListaUI");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for SolicitudInspeccionCalidadUI")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("SolicitudInspeccionCalidadUI")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for SolicitudInspeccionCalidadUI");           
    	   try {
		   ...
		   futureStates.add("GradoCalidadListaUI");
