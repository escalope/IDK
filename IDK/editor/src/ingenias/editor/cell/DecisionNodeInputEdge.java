...
public class DecisionNodeInputEdge extends NAryEdge {

  // Constructor with data content
  public DecisionNodeInputEdge(DecisionNodeInput userObject) {
    super(userObject);
    
     this.addRole("DecisionNodesource");
     this.setArity("DecisionNodesource",true,1);
     this.setArity("DecisionNodesource",false,1);
     
this.addClass("DecisionNodesource","ingenias.editor.entities.ActionUML");
     
this.addClass("DecisionNodesource","ingenias.editor.entities.InitialNode");
     
this.addClass("DecisionNodesource","ingenias.editor.entities.DecisionNode");
     
this.addClass("DecisionNodesource","ingenias.editor.entities.JoinNode");
     
this.addClass("DecisionNodesource","ingenias.editor.entities.MergeNode");
     
    
     this.addRole("DecisionNodetarget");
     this.setArity("DecisionNodetarget",true,1);
     this.setArity("DecisionNodetarget",false,1);
     
this.addClass("DecisionNodetarget","ingenias.editor.entities.DecisionNode");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof DecisionNodeInputEdge)))
      return false;

    // n-edge to be considered.
    DecisionNodeInputEdge edge = new DecisionNodeInputEdge( null);
    // If the connection involves an existing n-edge its assignations has to be considered.
    if (nAryEdgesNum == 1 && selectedEdge instanceof DecisionNodeInputEdge)
      edge = (DecisionNodeInputEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("DecisionNodesource")){
 	 edges.add( new DefaultEdge( new DecisionNodesourceRole("") ) );
	 }
      
	if (roles[i].equalsIgnoreCase("DecisionNodetarget")){
 	 edges.add( new DefaultEdge( new DecisionNodetargetRole("") ) );
	 }
