...
public class ForkNodeRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("ForkNode",
	index,"/rendererxml/ForkNodeUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("ForkNode",
	index,"/rendererxml/ForkNodeINGENIASPanel.xml");
     ...
     */
    public ForkNodeRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("ForkNode",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("ForkNode",current);
    }
  ...
  public static void setEntity(ForkNode ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("ForkNode",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("ForkNode",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("ForkNode",current);
	}
