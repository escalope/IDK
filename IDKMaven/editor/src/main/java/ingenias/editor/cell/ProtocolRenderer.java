...
public class ProtocolRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("Protocol",
	index,"/rendererxml/ProtocolUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("Protocol",
	index,"/rendererxml/ProtocolINGENIASPanel.xml");
    ...
    public ProtocolRenderer() {
	  ...
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("Protocol",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("Protocol",current);
    }
  ...
  public static void setEntity(Protocol ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("Protocol",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("Protocol",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("Protocol",current);
	}
