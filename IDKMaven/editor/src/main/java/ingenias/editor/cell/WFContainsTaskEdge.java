...
public class WFContainsTaskEdge extends NAryEdge {

  // Constructor with data content
  public WFContainsTaskEdge(ingenias.editor.entities.WFContainsTask userObject) {
    super(userObject);
    
     this.addRole("WFContainsTasksource");
     this.setArity("WFContainsTasksource",true,1);
     this.setArity("WFContainsTasksource",false,1);
     
this.addClass("WFContainsTasksource","ingenias.editor.entities.Workflow");
     
    
     this.addRole("WFContainsTasktarget");
     this.setArity("WFContainsTasktarget",true,1);
     this.setArity("WFContainsTasktarget",false,2147483647);
     
this.addClass("WFContainsTasktarget","ingenias.editor.entities.Task");
     
this.addClass("WFContainsTasktarget","ingenias.editor.entities.Workflow");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFContainsTaskEdge)))
      return false;

    // n-edge to be considered.
    WFContainsTaskEdge edge = new WFContainsTaskEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof WFContainsTaskEdge)
      edge = (WFContainsTaskEdge) selectedEdge;
