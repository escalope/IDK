...
public class Participant1IntExampleStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public Participant1IntExampleStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","Simple_deliver");
			    
			    
			    
			      // Receiving a message    
			      smf.add("Simple_deliver", "waiting for Simple_deliver");
			      
			      // Next states after receiving "Simple_deliver"
			      
			       smf.add("waiting for Simple_deliver","Simple_answer");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("Simple_answer", "endSimple_answer");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     String[] options=new String[]{"Simple_deliver"};
  ...
  if (this.isState("Simple_deliver") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"Simple_deliver",
                   "IntExample",optionsA,this,cardinality,2000);
      }	 
      this.removeState("Simple_deliver");
      this.addState("waiting for Simple_deliver");
  } 
  ...
  if (this.isState("Simple_answer")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("Simple_answer",optionsA,this)){
           ...
           "Simple_answer","IntExample",this.getContentForNextMessage());
           getTimeout().stop();
      } else {
