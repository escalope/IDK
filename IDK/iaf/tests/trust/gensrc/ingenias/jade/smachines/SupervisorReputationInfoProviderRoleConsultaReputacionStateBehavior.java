...
public class SupervisorReputationInfoProviderRoleConsultaReputacionStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public SupervisorReputationInfoProviderRoleConsultaReputacionStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","ConsultaReputacionUI");
			    
			    
			    
			      // Receiving a message    
			      smf.add("ConsultaReputacionUI", "waiting for ConsultaReputacionUI");
			     ...
			     // States involved in message deliver
			      ...
			      smf.add("RespuetaReputacion", "endRespuetaReputacion");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "ConsultaReputacion","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("ConsultaReputacionUI") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"ConsultaReputacionUI",
                   "ConsultaReputacion",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("ConsultaReputacionUI");
      this.addState("waiting for ConsultaReputacionUI");
      this.notifyStateTransitionExecuted("ConsultaReputacionUI", "waiting for ConsultaReputacionUI");
  } 
  ...
  if (this.isState("RespuetaReputacion")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("RespuetaReputacion",optionsA,this)){
           ...
           "RespuetaReputacion","ConsultaReputacion",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("RespuetaReputacion", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("RespuetaReputacion", "ABORTED");
    	  } else  {
