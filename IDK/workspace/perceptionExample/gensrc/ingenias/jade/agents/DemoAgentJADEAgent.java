...
public class DemoAgentJADEAgent
		 extends JADEAgent {         
 
		 public DemoAgentJADEAgent(){
		 super(new DemoAgentProtocol(),new DemoAgentInteractionLocks());
		 }
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
	    ...
	    if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
         ...
         if (tobject.getType().equals("AnotherTask")){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("DemoFact");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("DemoFact");
			 } else {
			    addExpectedInputs(tobject, "DemoFact","1",expectedInput);
	 ...
	 /*    
		 ...
		 " to achieve goal AttendUser because a previous outcome exists:"+poutcomes,getLocalName()+"-"+tobject.getType());
	      return false;
		} else */
		if (!allEntitiesExist){
			StringBuffer nonexisting=new StringBuffer();
			...
			MainInteractionManager.log("Non conversational initialisation discarded task "+tobject.getType()+
				" to achieve goal AttendUser because did not have all preconditions. Missing elements are:"+nonexisting+". Currently, the MS has the following elements:"+this.getMSM().getAllMentalEntities(),getLocalName()+"-"+tobject.getType());
		}
		return initialised;	       
	      }
         ...
         nonExistingInputs.clear();
  		 repeatedOutputs.clear();
         if (tobject.getType().equals("HelloWorldTask")){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("UserPressedButton");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("UserPressedButton");
			 } else {
			    addExpectedInputs(tobject, "UserPressedButton","1",expectedInput);
             	addConsumedInput(to,"1",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
	 ...
	 /*    
	      ...
	      */	  
	      ...
	      /**/	      
 			...
 			{
 			DemoFact expectedOutputDemoFact=
 				new DemoFact(MentalStateManager.generateMentalEntityID());
 			if (RuntimeConversation.class.isAssignableFrom(expectedOutputDemoFact.getClass())){
 			    java.lang.reflect.Method m;
				try {
					m = expectedOutputDemoFact.getClass().getMethod("setInteraction", new Class[]{Interaction.class});
					m.invoke(expectedOutputDemoFact, new Interaction("")) ;	  
            ...
            to.add(new OutputEntity(expectedOutputDemoFact,TaskOperations.CreateMS));
            }
	...
	public Vector tasksThatSatisfyGoal(String goalname){
         ...
         if (goalname.equals("AttendUser")){
         
         {
         boolean canbescheduled=false;
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new AnotherTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
				 canbescheduled=initialiseNonConversationalTask(tobject);
			 	 if (canbescheduled){
					MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal AttendUser",getLocalName()+"-"+tobject.getType());
					tasks.add(tobject);
				 } 			
	     }
		 ...
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new HelloWorldTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
				 canbescheduled=initialiseNonConversationalTask(tobject);
			 	 if (canbescheduled){
					MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal AttendUser",getLocalName()+"-"+tobject.getType());
					tasks.add(tobject);
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("AnotherTask");		         
                
         
         
         ttypes.add("HelloWorldTask");		         
   ...
   sg= new ingenias.editor.entities.StateGoal("AttendUser");
   sg.setState("pending");
      try {
	...
	Vector evetns=null;
	 ...
	 event= new UserPressedButton();
	 /* 
	 */ 
	 events.add(event);
	 actions.add(generateActionListener(UserPressedButton.class));		
