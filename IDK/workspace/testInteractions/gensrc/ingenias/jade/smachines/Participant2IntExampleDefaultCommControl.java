...
import ingenias.exception.NotFound;
  ...
  public class Participant2IntExampleDefaultCommControl extends DefaultCommControl{
  
  	  private Vector<String> previous=new Vector<String>();
  	  
  public Participant2IntExampleDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  };
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLock("MultipleAnswer");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("Multiple_retrieval") &&
    	 sb.isState("Multiple_retrieval")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"MultipleAnswer").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("Multiple_retrieval");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"MultipleAnswer");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLock("MultipleAnswer");
	       lockProcessed("MultipleAnswer");
           //MainInteractionManager.log("Removing lock MultipleAnswer",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("endMultiple_retrieval");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for Multiple_deliver")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("Multiple_deliver")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for Multiple_deliver");           
    	   try {
