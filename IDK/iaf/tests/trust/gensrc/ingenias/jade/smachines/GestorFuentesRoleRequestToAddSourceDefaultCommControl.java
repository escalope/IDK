...
import ingenias.exception.NotFound;
  ...
  public class GestorFuentesRoleRequestToAddSourceDefaultCommControl extends DefaultCommControl{
  
  	  private Vector<String> previous=new Vector<String>();
  	  
  public GestorFuentesRoleRequestToAddSourceDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  };
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("InformeResultadoPeticionUI") &&
    	 sb.isState("InformeResultadoPeticionUI")&& options.length>0) {         
         ...
         if (allexist && true){
           ...
           sb.removeState("InformeResultadoPeticionUI");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for PeticionIntroducirFuenteUI")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("PeticionIntroducirFuenteUI")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for PeticionIntroducirFuenteUI");           
    	   try {
