...
public class HasCtxModelEdge extends NAryEdge {

  // Constructor with data content
  public HasCtxModelEdge(ingenias.editor.entities.HasCtxModel userObject) {
    super(userObject);
    
     this.addRole("HasCtxModelsource");
     this.setArity("HasCtxModelsource",true,1);
     this.setArity("HasCtxModelsource",false,1);
     
this.addClass("HasCtxModelsource","ingenias.editor.entities.AMIContext");
     
    
     this.addRole("HasCtxModeltarget");
     this.setArity("HasCtxModeltarget",true,1);
     this.setArity("HasCtxModeltarget",false,2147483647);
     
this.addClass("HasCtxModeltarget","ingenias.editor.entities.AMICtxtModel");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof HasCtxModelEdge)))
      return false;

    // n-edge to be considered.
    HasCtxModelEdge edge = new HasCtxModelEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof HasCtxModelEdge)
      edge = (HasCtxModelEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("HasCtxModelsource")){
 	 edges.add( new DefaultEdge( new HasCtxModelsourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("HasCtxModeltarget")){
 	 edges.add( new DefaultEdge( new HasCtxModeltargetRole() ) );
	 }
