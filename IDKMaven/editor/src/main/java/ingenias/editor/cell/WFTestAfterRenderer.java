...
public class WFTestAfterRenderer extends VertexRenderer implements CellViewRenderer{
...
private static ViewPreferences.ViewType current = ViewPreferences.ViewType.INGENIAS;
  ...
  static {
   try {
	...
	RenderComponentManager.loadRenderFile("WFTestAfter",
	index,"/rendererxml/WFTestAfterNOICONPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("WFTestAfter",
	index,"/rendererxml/WFTestAfterINGENIASPanel.xml");
	
	index=ViewPreferences.ViewType.LABEL;
	RenderComponentManager.loadRenderFile("WFTestAfter",
	index,"/rendererxml/WFTestAfterLABELPanel.xml");
     ...
     */
     public WFTestAfterRenderer() {
    }

    public Dimension getSize() {
    return RenderComponentManager.getSize("WFTestAfter",current);
    }
 ...
 public static void setEntity(ingenias.editor.entities.WFTestAfter ent, Map attributes){
 Map currentMap=(Map)  RenderComponentManager.retrieveIDs("WFTestAfter",ent.getPrefs(attributes).getView());
current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("WFTestAfter",this.current);
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("WFTestAfter",c);
	}
