...
public class UMLCommentRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("UMLComment",
	index,"/rendererxml/UMLCommentUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("UMLComment",
	index,"/rendererxml/UMLCommentINGENIASPanel.xml");
     ...
     */
    public UMLCommentRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("UMLComment",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("UMLComment",current);
    }
  ...
  public static void setEntity(UMLComment ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("UMLComment",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("UMLComment",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("UMLComment",current);
	}
