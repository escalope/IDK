...
public class GTDependsEdge extends NAryEdge {

  // Constructor with data content
  public GTDependsEdge(ingenias.editor.entities.GTDepends userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof GTDependsEdge)))
      return false;

    // n-edge to be considered.
    GTDependsEdge edge = new GTDependsEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof GTDependsEdge)
      edge = (GTDependsEdge) selectedEdge;
