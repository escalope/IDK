...
import ingenias.exception.NotFound;
  ...
  public class User_assistantAsking_for_a_ticketDefaultCommControl extends DefaultCommControl{
  ...
  public User_assistantAsking_for_a_ticketDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("Find_a_movie");
     
     lw.addDeletionLockExpectedType("Movie");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("RequestOrder") &&
    	 sb.isState("RequestOrder")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Find_a_movie").isEmpty();
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Movie").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("RequestOrder");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Find_a_movie");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("Find_a_movie");
	       lockProcessed("Find_a_movie");
           //MainInteractionManager.log("Removing lock Find_a_movie",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Movie");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("Movie");
	       lockProcessed("Movie");
           //MainInteractionManager.log("Removing lock Movie",this.getAgent().getLocalName()+"-"+sb.getConversationID());
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for ImpossibleToGiveTicket")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("ImpossibleToGiveTicket")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for ImpossibleToGiveTicket");           
    	   //try {
    ...
    if (sb.isState("waiting for GiveTicket")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("GiveTicket")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for GiveTicket");           
    	   //try {
                    Vector toAdd=new Vector();
					for (ACLMessage singleMessage:multipleMessages){
						 String content=singleMessage.getContent();    	 				
