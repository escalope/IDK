...
public class GestorFuentesRoleRequestToAddSourceStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public GestorFuentesRoleRequestToAddSourceStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","PeticionIntroducirFuenteUI");
			    
			    
			    
			      // Receiving a message    
			      smf.add("PeticionIntroducirFuenteUI", "waiting for PeticionIntroducirFuenteUI");
			      
			      // Next states after receiving "PeticionIntroducirFuenteUI"
			      
			       smf.add("waiting for PeticionIntroducirFuenteUI","InformeResultadoPeticionUI");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("InformeResultadoPeticionUI", "endInformeResultadoPeticionUI");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "RequestToAddSource","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("PeticionIntroducirFuenteUI") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"PeticionIntroducirFuenteUI",
                   "RequestToAddSource",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("PeticionIntroducirFuenteUI");
      this.addState("waiting for PeticionIntroducirFuenteUI");
      this.notifyStateTransitionExecuted("PeticionIntroducirFuenteUI", "waiting for PeticionIntroducirFuenteUI");
  } 
  ...
  if (this.isState("InformeResultadoPeticionUI")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("InformeResultadoPeticionUI",optionsA,this)){
           ...
           "InformeResultadoPeticionUI","RequestToAddSource",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("InformeResultadoPeticionUI", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("InformeResultadoPeticionUI", "ABORTED");
    	  } else  {
