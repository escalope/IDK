...
public class SupervisorRoleRequestToAddSourceStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public SupervisorRoleRequestToAddSourceStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "PeticionIntroducirFuenteUI");
			    
			    
			      // Receiving a message    
			      smf.add("InformeResultadoPeticionUI", "waiting for InformeResultadoPeticionUI");
			      
			      // Next states after receiving "InformeResultadoPeticionUI"
			      
			       smf.add("waiting for InformeResultadoPeticionUI","endInformeResultadoPeticionUI");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("PeticionIntroducirFuenteUI", "InformeResultadoPeticionUI");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
  	try {  	  
      String[] options=new String[]{"PeticionIntroducirFuenteUI"};
      AID[] actors=null;
      Vector<AID> actorsv=new Vector<AID>();
      Vector<String> rolesv=new Vector<String>();
      
      actorsv.addAll(this.getActor("GestorFuentesRole"));
      {      
       Vector<AID> receivers=this.getActor("GestorFuentesRole");      
       for (AID aid:receivers){
        rolesv.add("GestorFuentesRole");
       }
      }
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","RequestToAddSource",completelistInvolvedActors);
  ...
  if (this.isState("InformeResultadoPeticionUI") ) {  // State changed by other agent and upted
    //in the parent class
         
      Vector options=new Vector();
      
      options.add("endInformeResultadoPeticionUI");
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"InformeResultadoPeticionUI",
                   "RequestToAddSource",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("InformeResultadoPeticionUI");
      this.addState("waiting for InformeResultadoPeticionUI");
      this.notifyStateTransitionExecuted("InformeResultadoPeticionUI", "waiting for InformeResultadoPeticionUI");
  } 
  ...
  if (this.isState("PeticionIntroducirFuenteUI")) {
     ...
     try {
      ...
      options.add("InformeResultadoPeticionUI");      
      
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("PeticionIntroducirFuenteUI",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "PeticionIntroducirFuenteUI","RequestToAddSource",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("PeticionIntroducirFuenteUI", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("PeticionIntroducirFuenteUI", "ABORTED");
    	  } else  {
  ...
  if (this.isState("endInformeResultadoPeticionUI")) {
