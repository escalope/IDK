...
import ingenias.exception.NotFound;
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("PeticionIntroducirFuenteUI") &&
    	 sb.isState("PeticionIntroducirFuenteUI")&& options.length>0) {         
         ...
         if (allexist && true){
           ...
           sb.removeState("PeticionIntroducirFuenteUI");
		   ...
		   futureStates.add("InformeResultadoPeticionUI");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for InformeResultadoPeticionUI")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("InformeResultadoPeticionUI")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for InformeResultadoPeticionUI");           
    	   try {
		   ...
		   futureStates.add("endInformeResultadoPeticionUI");
