...
public class AutonomousColaboratorJADEAgent
		 extends JADEAgent {         
 
		 public AutonomousColaboratorJADEAgent(){
		 super(new AutonomousColaboratorProtocol(),new AutonomousColaboratorInteractionLocks());
		 }
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	 ...
	 /*    
		...
		return initialised;	       
         ...
         nonExistingInputs.clear();
  		 repeatedOutputs.clear();
         if (tobject.getType().equals("BuscarFuenteTask") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("NuevoCicloEvento");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("NuevoCicloEvento");
			 } else {
			    addExpectedInputs(tobject, "NuevoCicloEvento","1",expectedInput);
             	addConsumedInput(to,"1",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
	     ...
	     tobject.addOutput(to);
     	      ...
     	      if (!allEntitiesExist){
				 ...
				 EventManager.getInstance().conversationalInitializationOfTaskFailed(
				 			getLocalName(), "AutonomousColaborator", 
												tobject, nonexisting);
	...
	public Vector tasksThatSatisfyGoal(String goalname){
		 ...
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new BuscarFuenteTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
				 canbescheduled=initialiseNonConversationalTask(tobject);
			 	 if (canbescheduled){
					//MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal SistemaEnriquecidoConBuenasFuentes",getLocalName()+"-"+tobject.getType());
					tasks.add(tobject);
   ...
   sg.setState("pending");
	...
	Vector evetns=null;
     ...
     app=TimerCiclosAppInit.getInstance(this);
     //app.registerOwner(this);
	
     	    
     this.getAM().addApplication("TimerCiclosApp",app);        
	 events=new Vector();
	 actions=new Vector();
		
	 event= new NuevoCicloEvento();
	 /*
	 slot=new Slot("227");
	 slot.setName("data");
	 ...
	 */ 
	 events.add(event);
	 actions.add(generateActionListener(NuevoCicloEvento.class));		

	 if (getGraphics()!=null)
	  getGraphics().addApplication("TimerCiclosApp", events,actions);    
   ...
   java.awt.event.ActionListener ifPressed=null;
	...
	public DFAgentDescription[ ] getDescription() {
		...
		sd.setType("ColaboradorArtificialRole");
