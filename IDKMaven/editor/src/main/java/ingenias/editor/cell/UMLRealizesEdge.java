...
public class UMLRealizesEdge extends NAryEdge {

  // Constructor with data content
  public UMLRealizesEdge(ingenias.editor.entities.UMLRealizes userObject) {
    super(userObject);
    
     this.addRole("UMLRealizessource");
     this.setArity("UMLRealizessource",true,1);
     this.setArity("UMLRealizessource",false,1);
     
this.addClass("UMLRealizessource","ingenias.editor.entities.INGENIASObject");
     
    
     this.addRole("UMLRealizestarget");
     this.setArity("UMLRealizestarget",true,1);
     this.setArity("UMLRealizestarget",false,1);
     
this.addClass("UMLRealizestarget","ingenias.editor.entities.INGENIASComponent");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof UMLRealizesEdge)))
      return false;

    // n-edge to be considered.
    UMLRealizesEdge edge = new UMLRealizesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof UMLRealizesEdge)
      edge = (UMLRealizesEdge) selectedEdge;
