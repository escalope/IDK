...
public class GTOrDependsRenderer extends VertexRenderer implements CellViewRenderer{
...
private static ViewPreferences.ViewType current = ViewPreferences.ViewType.INGENIAS;
  ...
  static {
   try {
	...
	RenderComponentManager.loadRenderFile("GTOrDepends",
	index,"/rendererxml/GTOrDependsNOICONPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("GTOrDepends",
	index,"/rendererxml/GTOrDependsINGENIASPanel.xml");
	
	index=ViewPreferences.ViewType.LABEL;
	RenderComponentManager.loadRenderFile("GTOrDepends",
	index,"/rendererxml/GTOrDependsLABELPanel.xml");
     ...
     */
     public GTOrDependsRenderer() {
    }

    public Dimension getSize() {
    return RenderComponentManager.getSize("GTOrDepends",current);
    }
 ...
 public static void setEntity(ingenias.editor.entities.GTOrDepends ent, Map attributes){
 Map currentMap=(Map)  RenderComponentManager.retrieveIDs("GTOrDepends",ent.getPrefs(attributes).getView());
current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("GTOrDepends",this.current);
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("GTOrDepends",c);
	}
