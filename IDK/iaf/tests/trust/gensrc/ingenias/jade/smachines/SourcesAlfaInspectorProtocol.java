...
public SourcesAlfaInspectorProtocol(){};		
	...
	public ActiveConversation initialiseProtocols(String agentName,RuntimeConversation conv, 
        ...
        // Adds a protocol to the protocol queue
		   ...
		   if (conv.getInteraction().getId().equals("SolicitudInspeccionAlfaCalidad") && conv.getPlayedRole().equals("SourcesAlfaQualityInspectorRole")){
		   	DefaultCommControl dcc=new SourcesAlfaQualityInspectorRoleSolicitudInspeccionAlfaCalidadDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new SourcesAlfaQualityInspectorRoleSolicitudInspeccionAlfaCalidadStateBehavior(agentName,
				msr, msu,
				conv,                           		
				conv.getPlayedRole(),
			   	actors,
