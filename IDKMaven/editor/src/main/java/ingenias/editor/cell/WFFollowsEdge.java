...
public class WFFollowsEdge extends NAryEdge {

  // Constructor with data content
  public WFFollowsEdge(ingenias.editor.entities.WFFollows userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFFollowsEdge)))
      return false;

    // n-edge to be considered.
    WFFollowsEdge edge = new WFFollowsEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof WFFollowsEdge)
      edge = (WFFollowsEdge) selectedEdge;
