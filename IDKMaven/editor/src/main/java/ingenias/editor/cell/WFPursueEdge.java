...
public class WFPursueEdge extends NAryEdge {

  // Constructor with data content
  public WFPursueEdge(ingenias.editor.entities.WFPursue userObject) {
    super(userObject);
    
     this.addRole("WFPursuesource");
     this.setArity("WFPursuesource",true,1);
     this.setArity("WFPursuesource",false,1);
     
this.addClass("WFPursuesource","ingenias.editor.entities.Agent");
     
this.addClass("WFPursuesource","ingenias.editor.entities.Role");
     
    
     this.addRole("WFPursuetarget");
     this.setArity("WFPursuetarget",true,1);
     this.setArity("WFPursuetarget",false,2147483647);
     
this.addClass("WFPursuetarget","ingenias.editor.entities.Goal");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFPursueEdge)))
      return false;

    // n-edge to be considered.
    WFPursueEdge edge = new WFPursueEdge( null);
    // If the connection involves an existing n-edge its assignations has to be considered.
    if (nAryEdgesNum == 1 && selectedEdge instanceof WFPursueEdge)
      edge = (WFPursueEdge) selectedEdge;
