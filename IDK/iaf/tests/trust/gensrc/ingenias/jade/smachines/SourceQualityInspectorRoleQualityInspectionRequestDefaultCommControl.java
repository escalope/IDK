...
import ingenias.exception.NotFound;
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("SourceQualityDegree");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("QualityDegreeIU") &&
    	 sb.isState("QualityDegreeIU")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"SourceQualityDegree").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("QualityDegreeIU");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for QualityInspectionRequestIU")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("QualityInspectionRequestIU")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for QualityInspectionRequestIU");           
    	   //try {
