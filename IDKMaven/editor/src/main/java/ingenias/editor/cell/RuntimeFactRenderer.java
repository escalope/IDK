...
public class RuntimeFactRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("RuntimeFact",
	index,"/rendererxml/RuntimeFactUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("RuntimeFact",
	index,"/rendererxml/RuntimeFactINGENIASPanel.xml");
     ...
     */
    public RuntimeFactRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("RuntimeFact",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("RuntimeFact",current);
    }
  ...
  public static void setEntity(RuntimeFact ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("RuntimeFact",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("RuntimeFact",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("RuntimeFact",current);
	}
