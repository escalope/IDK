...
public class IncludesEdge extends NAryEdge {

  // Constructor with data content
  public IncludesEdge(ingenias.editor.entities.Includes userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof IncludesEdge)))
      return false;

    // n-edge to be considered.
    IncludesEdge edge = new IncludesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof IncludesEdge)
      edge = (IncludesEdge) selectedEdge;
