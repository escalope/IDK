...
public class MultipleInputTaskTest extends BasicMASTest {
@Test
  public void testDemo() throws TimeOut{			
  			
	MentalStateManager msm = MSMRepository.getInstance().waitFor("A_0DeploymentUnitByTypeEnumInitMS1");
	...
	assertTrue("There should be no any AFrameFact entities after execution  and there were " +frameFactEntitiesAfter.size()+" "+tasks.size()+" "+tasks,
			frameFactEntitiesAfter.size() == 0);
	assertTrue("There should be no any AnotherFrameFact entities after execution  and there were " +anotherFrameFactEntitiesAfter.size(),
