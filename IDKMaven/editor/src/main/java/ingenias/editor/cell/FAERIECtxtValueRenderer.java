...
public class FAERIECtxtValueRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("FAERIECtxtValue",
	index,"/rendererxml/FAERIECtxtValueUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("FAERIECtxtValue",
	index,"/rendererxml/FAERIECtxtValueINGENIASPanel.xml");
     ...
     */
    public FAERIECtxtValueRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("FAERIECtxtValue",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("FAERIECtxtValue",current);
    }
  ...
  public static void setEntity(FAERIECtxtValue ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("FAERIECtxtValue",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("FAERIECtxtValue",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("FAERIECtxtValue",current);
	}
