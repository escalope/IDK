...
public class AHasMSManagerEdge extends NAryEdge {

  // Constructor with data content
  public AHasMSManagerEdge(ingenias.editor.entities.AHasMSManager userObject) {
    super(userObject);
    
     this.addRole("AHasMSManagersource");
     this.setArity("AHasMSManagersource",true,1);
     this.setArity("AHasMSManagersource",false,1);
     
this.addClass("AHasMSManagersource","ingenias.editor.entities.Agent");
     
    
     this.addRole("AHasMSManagertarget");
     this.setArity("AHasMSManagertarget",true,1);
     this.setArity("AHasMSManagertarget",false,1);
     
this.addClass("AHasMSManagertarget","ingenias.editor.entities.MentalStateManager");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AHasMSManagerEdge)))
      return false;

    // n-edge to be considered.
    AHasMSManagerEdge edge = new AHasMSManagerEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AHasMSManagerEdge)
      edge = (AHasMSManagerEdge) selectedEdge;
