...
public class InternalApplicationRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("InternalApplication",
	index,"/rendererxml/InternalApplicationUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("InternalApplication",
	index,"/rendererxml/InternalApplicationINGENIASPanel.xml");
     ...
     */
    public InternalApplicationRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("InternalApplication",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("InternalApplication",current);
    }
  ...
  public static void setEntity(InternalApplication ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("InternalApplication",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("InternalApplication",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("InternalApplication",current);
	}
