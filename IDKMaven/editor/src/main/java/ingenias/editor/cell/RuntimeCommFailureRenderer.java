...
public class RuntimeCommFailureRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("RuntimeCommFailure",
	index,"/rendererxml/RuntimeCommFailureUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("RuntimeCommFailure",
	index,"/rendererxml/RuntimeCommFailureINGENIASPanel.xml");
     ...
     */
    public RuntimeCommFailureRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("RuntimeCommFailure",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("RuntimeCommFailure",current);
    }
  ...
  public static void setEntity(RuntimeCommFailure ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("RuntimeCommFailure",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("RuntimeCommFailure",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("RuntimeCommFailure",current);
	}
