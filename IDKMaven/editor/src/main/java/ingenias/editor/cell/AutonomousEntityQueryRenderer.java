...
public class AutonomousEntityQueryRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("AutonomousEntityQuery",
	index,"/rendererxml/AutonomousEntityQueryUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("AutonomousEntityQuery",
	index,"/rendererxml/AutonomousEntityQueryINGENIASPanel.xml");
     ...
     */
    public AutonomousEntityQueryRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("AutonomousEntityQuery",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("AutonomousEntityQuery",current);
    }
  ...
  public static void setEntity(AutonomousEntityQuery ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("AutonomousEntityQuery",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("AutonomousEntityQuery",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("AutonomousEntityQuery",current);
	}
