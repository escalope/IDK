...
public class AUMLSelectionEdge extends NAryEdge {

  // Constructor with data content
  public AUMLSelectionEdge(ingenias.editor.entities.AUMLSelection userObject) {
    super(userObject);
    
     this.addRole("AUMLSelectionsource");
     this.setArity("AUMLSelectionsource",true,1);
     this.setArity("AUMLSelectionsource",false,1);
     
this.addClass("AUMLSelectionsource","ingenias.editor.entities.AUMLPort");
     
    
     this.addRole("AUMLSelectiontarget");
     this.setArity("AUMLSelectiontarget",true,1);
     this.setArity("AUMLSelectiontarget",false,1);
     
this.addClass("AUMLSelectiontarget","ingenias.editor.entities.AUMLPort");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AUMLSelectionEdge)))
      return false;

    // n-edge to be considered.
    AUMLSelectionEdge edge = new AUMLSelectionEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AUMLSelectionEdge)
      edge = (AUMLSelectionEdge) selectedEdge;
