...
public class FAERIEHasValueEdge extends NAryEdge {

  // Constructor with data content
  public FAERIEHasValueEdge(ingenias.editor.entities.FAERIEHasValue userObject) {
    super(userObject);
    
     this.addRole("FAERIEHasValuesource");
     this.setArity("FAERIEHasValuesource",true,1);
     this.setArity("FAERIEHasValuesource",false,1);
     
this.addClass("FAERIEHasValuesource","ingenias.editor.entities.FAERIECtxtAttribute");
     
    
     this.addRole("FAERIEHasValuetarget");
     this.setArity("FAERIEHasValuetarget",true,1);
     this.setArity("FAERIEHasValuetarget",false,2147483647);
     
this.addClass("FAERIEHasValuetarget","ingenias.editor.entities.FAERIECtxtValue");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof FAERIEHasValueEdge)))
      return false;

    // n-edge to be considered.
    FAERIEHasValueEdge edge = new FAERIEHasValueEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof FAERIEHasValueEdge)
      edge = (FAERIEHasValueEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("FAERIEHasValuesource")){
 	 edges.add( new DefaultEdge( new FAERIEHasValuesourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("FAERIEHasValuetarget")){
 	 edges.add( new DefaultEdge( new FAERIEHasValuetargetRole() ) );
	 }
