...
import ingenias.editor.cell.DefaultPanel;
...
public class BasicGraphUI extends GraphUI implements Serializable {
	...
	public static final boolean DNDPREVIEW = System.getProperty("java.version")
			...
			final static BasicStroke thickline =
				...
				if (view != null && bounds != null) {
					...
					for (int k=0;k<toPaint.size();k++){
						if (view.getCell() instanceof DefaultEdge){
							found =found || 
									((DefaultPort)(((DefaultEdge)view.getCell()).getTarget())).getParent().equals(toPaint.elementAt(k))||
									((DefaultPort)(((DefaultEdge)view.getCell()).getSource())).getParent().equals(toPaint.elementAt(k));
							if (!found){
								if (((DefaultPort)(((DefaultEdge)view.getCell()).getTarget())).getParent() instanceof NAryEdge){
									DefaultGraphCell dgcevaluatewith=(DefaultGraphCell) ((DefaultPort)(((DefaultEdge)view.getCell()).getTarget())).getParent();
									found=found||evaluateIfDefaultGraphCellHasToBeHighlighted(dgcevaluatewith, found, toPaint.elementAt(k));
								} else {
									if (((DefaultPort)(((DefaultEdge)view.getCell()).getSource())).getParent() instanceof NAryEdge){
										DefaultGraphCell dgcevaluatewith=(DefaultGraphCell) ((DefaultPort)(((DefaultEdge)view.getCell()).getSource())).getParent();
										found=found||evaluateIfDefaultGraphCellHasToBeHighlighted(dgcevaluatewith, found, toPaint.elementAt(k));
									}
								}						
							}
							...
							if (view.getCell() instanceof DefaultGraphCell){						
								found = evaluateIfDefaultGraphCellHasToBeHighlighted(
										(DefaultGraphCell)(view.getCell()), found, toPaint.elementAt(k));
							}
						}
					...
					if (found){
						...
						//	setFontRecursive(new Component[]{component}, oldfont.deriveFont(Font.BOLD));
					...
					if (cell instanceof DefaultGraphCell){
						...
						if (this.graph.getModel().getAttributes(currentCell)!=null){
							this.graph.getModel().getAttributes(
									currentCell).put("subcomponentbounds", subcomponentBounds);
						}			
					}
				}
				// Then Paint Children
			...
			private boolean evaluateIfDefaultGraphCellHasToBeHighlighted(DefaultGraphCell dgc,
					boolean found, DefaultGraphCell toPaint) {
					...
					for (Object objectport:dgc.getChildren()){
						if (objectport instanceof DefaultPort){
							...
							while (edges.hasNext() && !found){								
								...
								found=found
										|| 
										((DefaultPort)(next.getTarget())).getParent().equals(toPaint)||
										((DefaultPort)(next.getSource())).getParent().equals(toPaint);
							}
						}
					}
				}
				...
				if (ports != null) {
					...
					for (int i = 0; i < ports.length; i++) {
						if (ports[i] != null) {
							...
							Component parentRenderer = ports[i].getParentView().getRendererComponent(graph, false, false,false);
							boolean withoutports=false;
							if ((parentRenderer instanceof DefaultPanel) && 
									(((ingenias.editor.cell.DefaultPanel)parentRenderer).getPanelInside() instanceof WithoutPortsPanel) )
								withoutports=true;
