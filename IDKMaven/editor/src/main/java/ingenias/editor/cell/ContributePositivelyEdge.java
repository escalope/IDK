...
public class ContributePositivelyEdge extends NAryEdge {

  // Constructor with data content
  public ContributePositivelyEdge(ingenias.editor.entities.ContributePositively userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof ContributePositivelyEdge)))
      return false;

    // n-edge to be considered.
    ContributePositivelyEdge edge = new ContributePositivelyEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof ContributePositivelyEdge)
      edge = (ContributePositivelyEdge) selectedEdge;
