...
public class Provider2JADEAgent
		 extends JADEAgent {         
 
		 public Provider2JADEAgent(){
		 super(new Provider2Protocol(),new Provider2InteractionLocks());
		 }
	...
	private boolean initialiseConversationalTask(RuntimeConversation conversation, Task tobject) {
	   ...
	   boolean validConversationType=false;
		...
		if (validConversationType){
       	...
       	if (tobject.getType().equals("GenerateProposal2")){
	        Vector<MentalEntity> expectedInput=null;
        ...
        /*     
	     ...
	     boolean alreadyExists=true;
		    ...
		    {TicketProposal expectedOutputTicketProposal=		    
		     new TicketProposal(MentalStateManager.generateMentalEntityID());			
         ...
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new GenerateProposal2Task(ingenias.jade.MentalStateManager.generateMentalEntityID());
