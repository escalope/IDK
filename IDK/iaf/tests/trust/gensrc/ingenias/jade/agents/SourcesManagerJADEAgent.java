...
public class SourcesManagerJADEAgent
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	...
  	   	boolean correctRole=conversation.getPlayedRole().equals ("SourcesManagerRole");
       	...
       	if (tobject.getType().equals("ProcessAddingSourceIntoSystemRequestTask") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
	...
	public Vector tasksThatSatisfyGoal(String goalname){
         ...
         if (goalname.equals("SourcesManagement")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new ProcessAddingSourceIntoSystemRequestTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal SourcesManagement",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
					}
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("ProcessAddingSourceIntoSystemRequestTask");					
   ...
   sg= new ingenias.editor.entities.StateGoal("SourcesManagement");
   sg.setState("pending");
      try {
