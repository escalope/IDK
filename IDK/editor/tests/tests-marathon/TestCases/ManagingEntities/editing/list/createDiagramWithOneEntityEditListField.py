...
def test():
	...
	if window('INGENIAS Development Kit'):
		...
		select('valueListMethods', '[n1]')
		assert_content('valueListMethods', [ ['n1']])
		...
		select('valueListMethods1', '[n1]')
		assert_content('valueListMethods1', [ ['n1']])		
		#Remove the current list member
		select('valueListMethods1', '[n1]')
		rightclick('valueListMethods1', 'n1')
		select_menu('Remove selected element1')
		select('valueListMethods1', '')
		#Verify now there is none
		assert_p('valueListMethods1', 'ItemCount', '0')		
		#exit the edit dialog
		click('Accept1')
		...
		assert_p('valueListMethods2', 'ItemCount', '0')
		click('Accept2')		
	close()
