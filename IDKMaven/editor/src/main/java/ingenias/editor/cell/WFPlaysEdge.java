...
public class WFPlaysEdge extends NAryEdge {

  // Constructor with data content
  public WFPlaysEdge(ingenias.editor.entities.WFPlays userObject) {
    super(userObject);
    
     this.addRole("WFPlayssource");
     this.setArity("WFPlayssource",true,1);
     this.setArity("WFPlayssource",false,2147483647);
     
this.addClass("WFPlayssource","ingenias.editor.entities.Agent");
     
this.addClass("WFPlayssource","ingenias.editor.entities.AutonomousEntityQuery");
     
    
     this.addRole("WFPlaystarget");
     this.setArity("WFPlaystarget",true,1);
     this.setArity("WFPlaystarget",false,2147483647);
     
this.addClass("WFPlaystarget","ingenias.editor.entities.Role");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFPlaysEdge)))
      return false;

    // n-edge to be considered.
    WFPlaysEdge edge = new WFPlaysEdge( null);
