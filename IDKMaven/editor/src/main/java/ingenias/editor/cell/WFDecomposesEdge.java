...
public class WFDecomposesEdge extends NAryEdge {

  // Constructor with data content
  public WFDecomposesEdge(ingenias.editor.entities.WFDecomposes userObject) {
    super(userObject);
    
     this.addRole("WFDecomposessource");
     this.setArity("WFDecomposessource",true,1);
     this.setArity("WFDecomposessource",false,1);
     
this.addClass("WFDecomposessource","ingenias.editor.entities.Task");
     
    
     this.addRole("WFDecomposestarget");
     this.setArity("WFDecomposestarget",true,1);
     this.setArity("WFDecomposestarget",false,2147483647);
     
this.addClass("WFDecomposestarget","ingenias.editor.entities.Task");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFDecomposesEdge)))
      return false;

    // n-edge to be considered.
    WFDecomposesEdge edge = new WFDecomposesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof WFDecomposesEdge)
      edge = (WFDecomposesEdge) selectedEdge;
