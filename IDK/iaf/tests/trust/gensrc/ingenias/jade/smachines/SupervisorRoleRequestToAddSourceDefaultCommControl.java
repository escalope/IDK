...
import ingenias.exception.NotFound;
  ...
  public class SupervisorRoleRequestToAddSourceDefaultCommControl extends DefaultCommControl{
  ...
  public SupervisorRoleRequestToAddSourceDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("NewSource");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("AddingSourceRequestIU") &&
    	 sb.isState("AddingSourceRequestIU")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"NewSource").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("AddingSourceRequestIU");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"NewSource");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("NewSource");
	       lockProcessed("NewSource");
           //MainInteractionManager.log("Removing lock NewSource",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("AddingSourceResponseIU");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for AddingSourceResponseIU")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("AddingSourceResponseIU")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for AddingSourceResponseIU");           
    	   //try {
		   ...
		   futureStates.add("endAddingSourceResponseIU");
