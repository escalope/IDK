...
public class AContainsMEEdge extends NAryEdge {

  // Constructor with data content
  public AContainsMEEdge(ingenias.editor.entities.AContainsME userObject) {
    super(userObject);
    
     this.addRole("AContainsMEsource");
     this.setArity("AContainsMEsource",true,1);
     this.setArity("AContainsMEsource",false,1);
     
this.addClass("AContainsMEsource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("AContainsMEtarget");
     this.setArity("AContainsMEtarget",true,1);
     this.setArity("AContainsMEtarget",false,2147483647);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AContainsMEEdge)))
      return false;

    // n-edge to be considered.
    AContainsMEEdge edge = new AContainsMEEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AContainsMEEdge)
      edge = (AContainsMEEdge) selectedEdge;
