...
public class EndNodeRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("EndNode",
	index,"/rendererxml/EndNodeUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("EndNode",
	index,"/rendererxml/EndNodeINGENIASPanel.xml");
    ...
    public EndNodeRenderer() {
	  ...
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("EndNode",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("EndNode",current);
    }
  ...
  public static void setEntity(EndNode ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("EndNode",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("EndNode",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("EndNode",current);
	}
