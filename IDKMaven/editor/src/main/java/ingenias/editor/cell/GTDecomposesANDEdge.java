...
public class GTDecomposesANDEdge extends NAryEdge {

  // Constructor with data content
  public GTDecomposesANDEdge(ingenias.editor.entities.GTDecomposesAND userObject) {
    super(userObject);
    
     this.addRole("GTDecomposessource");
     this.setArity("GTDecomposessource",true,1);
     this.setArity("GTDecomposessource",false,1);
     
this.addClass("GTDecomposessource","ingenias.editor.entities.Goal");
     
    
     this.addRole("GTDecomposestarget");
     this.setArity("GTDecomposestarget",true,1);
     this.setArity("GTDecomposestarget",false,1);
     
this.addClass("GTDecomposestarget","ingenias.editor.entities.Goal");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof GTDecomposesANDEdge)))
      return false;

    // n-edge to be considered.
    GTDecomposesANDEdge edge = new GTDecomposesANDEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof GTDecomposesANDEdge)
      edge = (GTDecomposesANDEdge) selectedEdge;
