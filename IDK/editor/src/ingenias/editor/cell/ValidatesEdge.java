...
public class ValidatesEdge extends NAryEdge {

  // Constructor with data content
  public ValidatesEdge(Validates userObject) {
    super(userObject);
    
     this.addRole("Validatessource");
     this.setArity("Validatessource",true,1);
     this.setArity("Validatessource",false,1);
     
this.addClass("Validatessource","ingenias.editor.entities.Test");
     
    
     this.addRole("Validatestarget");
     this.setArity("Validatestarget",true,1);
     this.setArity("Validatestarget",false,2147483647);
     
this.addClass("Validatestarget","ingenias.editor.entities.Goal");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof ValidatesEdge)))
      return false;

    // n-edge to be considered.
    ValidatesEdge edge = new ValidatesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof ValidatesEdge)
      edge = (ValidatesEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("Validatessource")){
 	 edges.add( new DefaultEdge( new ValidatessourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("Validatestarget")){
 	 edges.add( new DefaultEdge( new ValidatestargetRole() ) );
	 }
