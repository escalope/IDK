...
public class ConversationRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	index,"/rendererxml/ConversationUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("Conversation",
	index,"/rendererxml/ConversationINGENIASPanel.xml");
    ...
    public ConversationRenderer() {
	  ...
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("Conversation",ent.getPrefs(map).getView());
  ...
  public static void setEntity(Conversation ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("Conversation",ent.getPrefs(attributes).getView());
