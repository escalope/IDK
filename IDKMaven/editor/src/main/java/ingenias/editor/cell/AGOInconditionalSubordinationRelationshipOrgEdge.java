...
public class AGOInconditionalSubordinationRelationshipOrgEdge extends NAryEdge {

  // Constructor with data content
  public AGOInconditionalSubordinationRelationshipOrgEdge(ingenias.editor.entities.AGOInconditionalSubordinationRelationshipOrg userObject) {
    super(userObject);
    
     this.addRole("AGORelationship1source");
     this.setArity("AGORelationship1source",true,1);
     this.setArity("AGORelationship1source",false,1);
     
this.addClass("AGORelationship1source","ingenias.editor.entities.Organization");
     
    
     this.addRole("AGORelationship1target");
     this.setArity("AGORelationship1target",true,1);
     this.setArity("AGORelationship1target",false,1);
     
this.addClass("AGORelationship1target","ingenias.editor.entities.Organization");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AGOInconditionalSubordinationRelationshipOrgEdge)))
      return false;

    // n-edge to be considered.
    AGOInconditionalSubordinationRelationshipOrgEdge edge = new AGOInconditionalSubordinationRelationshipOrgEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AGOInconditionalSubordinationRelationshipOrgEdge)
      edge = (AGOInconditionalSubordinationRelationshipOrgEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("AGORelationship1source")){
 	 edges.add( new DefaultEdge( new AGORelationship1sourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("AGORelationship1target")){
 	 edges.add( new DefaultEdge( new AGORelationship1targetRole() ) );
	 }
