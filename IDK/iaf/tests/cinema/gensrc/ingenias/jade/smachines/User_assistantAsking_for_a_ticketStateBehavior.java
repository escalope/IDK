...
public class User_assistantAsking_for_a_ticketStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public User_assistantAsking_for_a_ticketStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "RequestOrder");
			    
			    
			      // Receiving a message    
			      smf.add("ImpossibleToGiveTicket", "waiting for ImpossibleToGiveTicket");
			      
			      // Next states after receiving "ImpossibleToGiveTicket"
			      
			       smf.add("waiting for ImpossibleToGiveTicket","endImpossibleToGiveTicket");
			      
			    
			      // Receiving a message    
			      smf.add("GiveTicket", "waiting for GiveTicket");
			      
			      // Next states after receiving "GiveTicket"
			      
			       smf.add("waiting for GiveTicket","endGiveTicket");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("RequestOrder", "GiveTicket");
			      
			      smf.add("RequestOrder", "ImpossibleToGiveTicket");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","Asking_for_a_ticket",completelistInvolvedActors);
  ...
  if (this.isState("ImpossibleToGiveTicket") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"ImpossibleToGiveTicket",
                   "Asking_for_a_ticket",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("ImpossibleToGiveTicket");
      this.addState("waiting for ImpossibleToGiveTicket");
      this.notifyStateTransitionExecuted("ImpossibleToGiveTicket", "waiting for ImpossibleToGiveTicket");
  } 
  
  // Receives a message and a synchronization command
  if (this.isState("GiveTicket") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"GiveTicket",
                   "Asking_for_a_ticket",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("GiveTicket");
      this.addState("waiting for GiveTicket");
      this.notifyStateTransitionExecuted("GiveTicket", "waiting for GiveTicket");
  } 
  ...
  if (this.isState("RequestOrder")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("RequestOrder",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "RequestOrder","Asking_for_a_ticket",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("RequestOrder", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("RequestOrder", "ABORTED");
    	  } else  {
