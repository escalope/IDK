...
public class EPerceivesEdge extends NAryEdge {

  // Constructor with data content
  public EPerceivesEdge(ingenias.editor.entities.EPerceives userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof EPerceivesEdge)))
      return false;

    // n-edge to be considered.
    EPerceivesEdge edge = new EPerceivesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof EPerceivesEdge)
      edge = (EPerceivesEdge) selectedEdge;
