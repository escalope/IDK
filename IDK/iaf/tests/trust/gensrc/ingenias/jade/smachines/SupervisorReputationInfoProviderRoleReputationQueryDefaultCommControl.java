...
import ingenias.exception.NotFound;
  ...
  public class SupervisorReputationInfoProviderRoleReputationQueryDefaultCommControl extends DefaultCommControl{
  
  	  private Vector<String> previous=new Vector<String>();
  	  
  public SupervisorReputationInfoProviderRoleReputationQueryDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  };
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("AgentReputation");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("ReputationQueryResponse") &&
    	 sb.isState("ReputationQueryResponse")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"AgentReputation").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("ReputationQueryResponse");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for ReputationQueryIU")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("ReputationQueryIU")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for ReputationQueryIU");           
    	   try {
