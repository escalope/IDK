...
public class SimulationPursuesEdge extends NAryEdge {

  // Constructor with data content
  public SimulationPursuesEdge(ingenias.editor.entities.SimulationPursues userObject) {
    super(userObject);
    
     this.addRole("simulationpursuessource");
     this.setArity("simulationpursuessource",true,1);
     this.setArity("simulationpursuessource",false,1);
     
this.addClass("simulationpursuessource","ingenias.editor.entities.SimulationPackage");
     
    
     this.addRole("simulationpursuestarget");
     this.setArity("simulationpursuestarget",true,1);
     this.setArity("simulationpursuestarget",false,2147483647);
     
this.addClass("simulationpursuestarget","ingenias.editor.entities.Goal");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof SimulationPursuesEdge)))
      return false;

    // n-edge to be considered.
    SimulationPursuesEdge edge = new SimulationPursuesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof SimulationPursuesEdge)
      edge = (SimulationPursuesEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("simulationpursuessource")){
 	 edges.add( new DefaultEdge( new simulationpursuessourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("simulationpursuestarget")){
 	 edges.add( new DefaultEdge( new simulationpursuestargetRole() ) );
	 }
