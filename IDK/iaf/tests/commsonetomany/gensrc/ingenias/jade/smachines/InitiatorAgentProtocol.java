...
public class InitiatorAgentProtocol implements AgentProtocols{

public InitiatorAgentProtocol(){};		
	...
	public ActiveConversation initialiseProtocols(String agentName,RuntimeConversation conv, 
        ...
        // Adds a protocol to the protocol queue
		   ...
		   if (conv.getInteraction().getId().equals("SampleInteraction") && conv.getPlayedRole().equals("Initiator")){
		   DefaultCommControl dcc=new InitiatorSampleInteractionDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new InitiatorSampleInteractionStateBehavior(
			agentName,
				msr,msu,
					conv,                           		
				   conv.getPlayedRole(),
         ...
         throw new WrongInteraction("Agent "+agentName+" does not know any interaction protocol of type "+conv.getInteraction().getId()+" where the " +
	        ...
	        public AgentExternalDescription[] getInteractionActors(String interaction, YellowPages yp) throws ingenias.jade.exception.NoAgentsFound{
              AgentExternalDescription[] result=null;
               ...
               if (interaction.equals("SampleInteraction")){
                Vector<AgentExternalDescription> cols=new Vector<AgentExternalDescription>();
                 
                 try {
                 String cardinality="1__*";
		 DFAgentDescription[] agents=yp.getAgents("Colaborator");
                   if (agents==null || agents.length<=0)
                      throw new ingenias.jade.exception.NoAgentsFound("Could not find an agent playing the role Colaborator");
                   if (cardinality.equals("1") || cardinality.equals(""))
                    cols.add(new AgentExternalDescription(agents[0].getName(),"Colaborator"));
                    else
                	   if (cardinality.equals("1__*"))
						   for (int k=0;k<agents.length;k++)
                			  cols.add(new AgentExternalDescription(agents[k].getName(),"Colaborator"));
	       ...
	       Vector playedRoles=new Vector();
	        DFAgentDescription dfd=null;
                ...
                if (protocol.equals("SampleInteraction")){
		dfd.setName(agentID);
