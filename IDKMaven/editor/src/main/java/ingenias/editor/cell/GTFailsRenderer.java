...
private static ViewPreferences.ViewType current = ViewPreferences.ViewType.INGENIAS;
  ...
  static {
   try {
	...
	RenderComponentManager.loadRenderFile("GTFails",
	index,"/rendererxml/GTFailsNOICONPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("GTFails",
	index,"/rendererxml/GTFailsINGENIASPanel.xml");
	
	index=ViewPreferences.ViewType.LABEL;
	RenderComponentManager.loadRenderFile("GTFails",
	index,"/rendererxml/GTFailsLABELPanel.xml");
 ...
 public static void setEntity(ingenias.editor.entities.GTFails ent, Map attributes){
 Map currentMap=(Map)  RenderComponentManager.retrieveIDs("GTFails",ent.getPrefs(attributes).getView());
      ...
      if (currentMap.get("FailureCondition")!=null){
	   if (ent!=null && ent.getFailureCondition()!=null){
		...
		((javax.swing.JLabel)( currentMap).get("FailureCondition")).setText(ent.getFailureCondition().toString());
		 ...
		 if (currentMap.get("FailureCondition") instanceof javax.swing.text.JTextComponent)
		 ((javax.swing.text.JTextComponent)( currentMap).get("FailureCondition")).setText(ent.getFailureCondition().toString());
		
	      } 
	   } else  {
	     if (currentMap.get("FailureCondition") instanceof javax.swing.JLabel)
	     ((javax.swing.JLabel)( currentMap).get("FailureCondition")).setText("");
	     else {
		if (!(currentMap.get("FailureCondition") instanceof ingenias.editor.rendererxml.CollectionPanel)) 
		((javax.swing.text.JTextComponent)( currentMap).get("FailureCondition")).setText("");
	     }
         }
	  }
