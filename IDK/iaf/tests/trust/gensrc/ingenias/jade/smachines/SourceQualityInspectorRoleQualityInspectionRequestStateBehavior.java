...
private MentalStateReader msr=null;
  public SourceQualityInspectorRoleQualityInspectionRequestStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","SolicitudInspeccionCalidadUI");
			    
			    
			    
			      // Receiving a message    
			      smf.add("SolicitudInspeccionCalidadUI", "waiting for SolicitudInspeccionCalidadUI");
			      
			      // Next states after receiving "SolicitudInspeccionCalidadUI"
			      
			       smf.add("waiting for SolicitudInspeccionCalidadUI","GradoCalidadListaUI");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("GradoCalidadListaUI", "endGradoCalidadListaUI");
  ...
  if (this.isState("SolicitudInspeccionCalidadUI") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"SolicitudInspeccionCalidadUI",
                   "QualityInspectionRequest",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("SolicitudInspeccionCalidadUI");
      this.addState("waiting for SolicitudInspeccionCalidadUI");
      this.notifyStateTransitionExecuted("SolicitudInspeccionCalidadUI", "waiting for SolicitudInspeccionCalidadUI");
  } 
  ...
  if (this.isState("GradoCalidadListaUI")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("GradoCalidadListaUI",optionsA,this)){
           ...
           "GradoCalidadListaUI","QualityInspectionRequest",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("GradoCalidadListaUI", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("GradoCalidadListaUI", "ABORTED");
    	  } else  {
