...
public class ConsumesEdge extends NAryEdge {

  // Constructor with data content
  public ConsumesEdge(ingenias.editor.entities.Consumes userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof ConsumesEdge)))
      return false;

    // n-edge to be considered.
    ConsumesEdge edge = new ConsumesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof ConsumesEdge)
      edge = (ConsumesEdge) selectedEdge;
