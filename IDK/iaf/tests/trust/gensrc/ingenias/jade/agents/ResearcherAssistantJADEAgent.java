...
public class ResearcherAssistantJADEAgent
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
		...
		if (this.getLM().canBeDeleted(expectedInput)){             
         ...
         if (tobject.getType().equals("ProcessHumanProposalTask") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("NewIntroducedProposalEvent");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("NewIntroducedProposalEvent");
			 } else {
			    addExpectedInputs(tobject, "NewIntroducedProposalEvent","1",expectedInput);
	 ...
	 /*    
		...
		return initialised;	       
         ...
         nonExistingInputs.clear();
  		 repeatedOutputs.clear();
         if (tobject.getType().equals("UIInitializationTask") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("FirstExecution");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("FirstExecution");
			 } else {
			    addExpectedInputs(tobject, "FirstExecution","1",expectedInput);
             	addConsumedInput(to,"1",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
	     ...
	     expectedApp=(ingenias.jade.components.Application)getAM().getApplication("VisualizationApp");
             tobject.addApplication("VisualizationApp",expectedApp);
	...
	public Vector tasksThatSatisfyGoal(String goalname){
		 ...
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new ProcessHumanProposalTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
         ...
         if (goalname.equals("ResearcherAssistance")){
         
         {
         boolean canbescheduled=false;
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new UIInitializationTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
				 canbescheduled=initialiseNonConversationalTask(tobject);
			 	 if (canbescheduled){
					//MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal ResearcherAssistance",getLocalName()+"-"+tobject.getType());
					tasks.add(tobject);
				 } 			
	     }
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("ProcessHumanProposalTask");		         
         ...
         ttypes.add("UIInitializationTask");		         
   ...
   sg= new ingenias.editor.entities.StateGoal("ResearcherAssistance");
   sg.setState("pending");
      try {
	   this.getMSM().addMentalEntity(sg);
   } catch (InvalidEntity e1) {
   ...
   /* */
	...
	Vector evetns=null;
     ...
     app=VisualizationAppInit.getInstance(this);
     //app.registerOwner(this);
	
     	    
     this.getAM().addApplication("VisualizationApp",app);        
	 events=new Vector();
	 actions=new Vector();
		
	 event= new NewIntroducedProposalEvent();
	 /*
	 slot=new Slot("228");
	 slot.setName("data");
	 ...
	 */ 
	 events.add(event);
	 actions.add(generateActionListener(NewIntroducedProposalEvent.class));		

	 if (getGraphics()!=null)
	  getGraphics().addApplication("VisualizationApp", events,actions);    
   ...
   java.awt.event.ActionListener ifPressed=null;
	...
	public DFAgentDescription[ ] getDescription() {
		...
		sd = new ServiceDescription();
		sd.setName(getLocalName() + "-sub-df");
		sd.setType("HumanCollaboratorRole");
		sd.setOwnership("JADE");
		dfd.addServices(sd);
                playedRoles.add(dfd);
