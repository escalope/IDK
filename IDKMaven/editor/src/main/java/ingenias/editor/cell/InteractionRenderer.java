...
public class InteractionRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("Interaction",
	index,"/rendererxml/InteractionUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("Interaction",
	index,"/rendererxml/InteractionINGENIASPanel.xml");
    ...
    public InteractionRenderer() {
	  ...
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("Interaction",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("Interaction",current);
    }
  ...
  public static void setEntity(Interaction ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("Interaction",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
      ...
      if (currentMap.get("Nature")!=null){
	   if (ent!=null && ent.getNature()!=null){
		if (currentMap.get("Nature") instanceof javax.swing.JLabel){
		((javax.swing.JLabel)( currentMap).get("Nature")).setText(ent.getNature().toString());
		} else {
		 if (currentMap.get("Nature") instanceof javax.swing.text.JTextComponent)
		 ((javax.swing.text.JTextComponent)( currentMap).get("Nature")).setText(ent.getNature().toString());
		
	      } 
	   } else  {
	     if (currentMap.get("Nature") instanceof javax.swing.JLabel)
	     ((javax.swing.JLabel)( currentMap).get("Nature")).setText("");
	     else {
		if (!(currentMap.get("Nature") instanceof ingenias.editor.rendererxml.CollectionPanel)) 
		((javax.swing.text.JTextComponent)( currentMap).get("Nature")).setText("");
	     }
         }
	  }
	...
	{
	return RenderComponentManager.retrievePanel("Interaction",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("Interaction",current);
	}
