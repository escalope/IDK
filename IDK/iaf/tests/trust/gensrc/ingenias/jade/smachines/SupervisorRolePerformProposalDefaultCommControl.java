 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("DeniedProposalIU") &&
		 ...
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"DeniedProposal").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("DeniedProposalIU");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for ProposalIU")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("ProposalIU")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for ProposalIU");           
    	   //try {
