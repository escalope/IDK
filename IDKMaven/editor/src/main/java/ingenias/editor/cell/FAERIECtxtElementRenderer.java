...
public class FAERIECtxtElementRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("FAERIECtxtElement",
	index,"/rendererxml/FAERIECtxtElementUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("FAERIECtxtElement",
	index,"/rendererxml/FAERIECtxtElementINGENIASPanel.xml");
     ...
     */
    public FAERIECtxtElementRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("FAERIECtxtElement",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("FAERIECtxtElement",current);
    }
  ...
  public static void setEntity(FAERIECtxtElement ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("FAERIECtxtElement",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("FAERIECtxtElement",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("FAERIECtxtElement",current);
	}
