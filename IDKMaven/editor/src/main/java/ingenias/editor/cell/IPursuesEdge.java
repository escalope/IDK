...
public class IPursuesEdge extends NAryEdge {

  // Constructor with data content
  public IPursuesEdge(ingenias.editor.entities.IPursues userObject) {
    super(userObject);
    
     this.addRole("IPursuessource");
     this.setArity("IPursuessource",true,1);
     this.setArity("IPursuessource",false,1);
     
this.addClass("IPursuessource","ingenias.editor.entities.Interaction");
     
    
     this.addRole("IPursuestarget");
     this.setArity("IPursuestarget",true,1);
     this.setArity("IPursuestarget",false,2147483647);
     
this.addClass("IPursuestarget","ingenias.editor.entities.Goal");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof IPursuesEdge)))
      return false;

    // n-edge to be considered.
    IPursuesEdge edge = new IPursuesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof IPursuesEdge)
      edge = (IPursuesEdge) selectedEdge;
