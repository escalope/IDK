	...
	private boolean initialiseNonConversationalTask(Task tobject) {
		...
		if (this.getLM().canBeDeleted(expectedInput)){             
         ...
         if (tobject.getType().equals("Task0") ){
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("ApplicationEventSlots0");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("ApplicationEventSlots0");
			 } else {
			    addExpectedInputs(tobject, "ApplicationEventSlots0","1",expectedInput);
	 ...
	 /*    
		...
		return initialised;	       
         ...
         nonExistingInputs.clear();
  		 repeatedOutputs.clear();
         if (tobject.getType().equals("Task1") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
	 ...
	 /*    
	      ...
	      */	  
	      ...
	      /**/	      
		    ...
		    {fake_Task1_output_for_task_Task2 expectedOutputfake_Task1_output_for_task_Task2=		    
		     new fake_Task1_output_for_task_Task2(MentalStateManager.generateMentalEntityID());			
             to.add(new OutputEntity(expectedOutputfake_Task1_output_for_task_Task2,TaskOperations.CreateWF));
            }
		...
		return initialised;	       
         ...
         nonExistingInputs.clear();
  		 repeatedOutputs.clear();
         if (tobject.getType().equals("Task2") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("fake_Task1_output_for_task_Task2");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("fake_Task1_output_for_task_Task2");
			 } else {
			    addExpectedInputs(tobject, "fake_Task1_output_for_task_Task2","1",expectedInput);
             	addConsumedInput(to,"1",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
