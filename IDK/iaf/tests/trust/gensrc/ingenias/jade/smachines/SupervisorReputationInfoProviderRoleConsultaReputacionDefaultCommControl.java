...
import ingenias.exception.NotFound;
  ...
  public class SupervisorReputationInfoProviderRoleConsultaReputacionDefaultCommControl extends DefaultCommControl{
  
  	  private Vector<String> previous=new Vector<String>();
  	  
  public SupervisorReputationInfoProviderRoleConsultaReputacionDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  };
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("RespuetaReputacion") &&
		 ...
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"ReputacionAgente").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("RespuetaReputacion");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"ReputacionAgente");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLock(mfcontent);
	       lockProcessed("ReputacionAgente");
           //MainInteractionManager.log("Removing lock ReputacionAgente",this.getAgent().getLocalName()+"-"+sb.getConversationID());
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for ConsultaReputacionUI")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("ConsultaReputacionUI")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for ConsultaReputacionUI");           
    	   try {
