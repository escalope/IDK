...
public class IInitiatesEdge extends NAryEdge {

  // Constructor with data content
  public IInitiatesEdge(ingenias.editor.entities.IInitiates userObject) {
    super(userObject);
    
     this.addRole("IInitiatessource");
     this.setArity("IInitiatessource",true,1);
     this.setArity("IInitiatessource",false,1);
     
this.addClass("IInitiatessource","ingenias.editor.entities.Interaction");
     
    
     this.addRole("IInitiatestarget");
     this.setArity("IInitiatestarget",true,1);
     this.setArity("IInitiatestarget",false,1);
     
this.addClass("IInitiatestarget","ingenias.editor.entities.Agent");
     
this.addClass("IInitiatestarget","ingenias.editor.entities.Role");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof IInitiatesEdge)))
      return false;

    // n-edge to be considered.
    IInitiatesEdge edge = new IInitiatesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof IInitiatesEdge)
      edge = (IInitiatesEdge) selectedEdge;
