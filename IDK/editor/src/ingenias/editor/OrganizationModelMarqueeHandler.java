...
public class OrganizationModelMarqueeHandler extends MarqueeHandler  implements java.io.Serializable {

	



        public OrganizationModelMarqueeHandler(ModelJGraph graph){
	super(graph);
        }
	 ...
	 protected Vector<AbstractAction> createChangeViewActions(final DefaultGraphCell cell) {
		...
		nobjects.add(
			new AbstractAction("Insert OrganizationNetwork") {
				public void actionPerformed(ActionEvent ev) {
					try {
						getGraph().insert(pt, "OrganizationNetwork");
					} catch (InvalidEntity e) {						
						e.printStackTrace();
						JOptionPane.showMessageDialog(IDE.ide, "Object type OrganizationNetwork is not allowed in this diagram",
                                "Warning", JOptionPane.WARNING_MESSAGE);
					}
				}
			});
