...
public class GTInheritsEdge extends NAryEdge {

  // Constructor with data content
  public GTInheritsEdge(ingenias.editor.entities.GTInherits userObject) {
    super(userObject);
    
     this.addRole("GTInheritssource");
     this.setArity("GTInheritssource",true,1);
     this.setArity("GTInheritssource",false,2147483647);
     
this.addClass("GTInheritssource","ingenias.editor.entities.MentalEntity");
     
    
     this.addRole("GTInheritstarget");
     this.setArity("GTInheritstarget",true,1);
     this.setArity("GTInheritstarget",false,1);
     
this.addClass("GTInheritstarget","ingenias.editor.entities.MentalEntity");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof GTInheritsEdge)))
      return false;

    // n-edge to be considered.
    GTInheritsEdge edge = new GTInheritsEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof GTInheritsEdge)
      edge = (GTInheritsEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("GTInheritssource")){
 	 edges.add( new DefaultEdge( new GTInheritssourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("GTInheritstarget")){
 	 edges.add( new DefaultEdge( new GTInheritstargetRole() ) );
	 }
