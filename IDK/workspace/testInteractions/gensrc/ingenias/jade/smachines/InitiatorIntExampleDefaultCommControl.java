...
import ingenias.exception.NotFound;
  ...
  public class InitiatorIntExampleDefaultCommControl extends DefaultCommControl{
  
  	  private Vector<String> previous=new Vector<String>();
  	  
  public InitiatorIntExampleDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  };
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLock("SingleQuestion");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLock("InitialQuestion");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("Simple_deliver") &&
    	 sb.isState("Simple_deliver")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"SingleQuestion").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("Simple_deliver");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"SingleQuestion");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLock("SingleQuestion");
	       lockProcessed("SingleQuestion");
           //MainInteractionManager.log("Removing lock SingleQuestion",this.getAgent().getLocalName()+"-"+sb.getConversationID());
    ...
    if (stateToEvaluate.equals("Multiple_deliver") &&
    	 sb.isState("Multiple_deliver")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"InitialQuestion").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("Multiple_deliver");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"InitialQuestion");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLock("InitialQuestion");
	       lockProcessed("InitialQuestion");
           //MainInteractionManager.log("Removing lock InitialQuestion",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("Multiple_retrieval");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for Simple_answer")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("Simple_answer")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for Simple_answer");           
    	   try {
    ...
    if (sb.isState("waiting for Multiple_retrieval")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("Multiple_retrieval")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for Multiple_retrieval");           
    	   try {
                    Vector toAdd=new Vector();
					for (ACLMessage singleMessage:multipleMessages){
						toAdd.addAll((Vector)singleMessage.getContentObject());						
		   ...
		   futureStates.add("Simple_deliver");
