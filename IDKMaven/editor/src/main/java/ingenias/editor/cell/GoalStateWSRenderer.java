...
public class GoalStateWSRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("GoalStateWS",
	index,"/rendererxml/GoalStateWSUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("GoalStateWS",
	index,"/rendererxml/GoalStateWSINGENIASPanel.xml");
     ...
     */
    public GoalStateWSRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("GoalStateWS",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("GoalStateWS",current);
    }
  ...
  public static void setEntity(GoalStateWS ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("GoalStateWS",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("GoalStateWS",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("GoalStateWS",current);
	}
