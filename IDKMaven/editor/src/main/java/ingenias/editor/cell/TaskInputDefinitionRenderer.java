...
public class TaskInputDefinitionRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("TaskInputDefinition",
	index,"/rendererxml/TaskInputDefinitionUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("TaskInputDefinition",
	index,"/rendererxml/TaskInputDefinitionINGENIASPanel.xml");
     ...
     */
    public TaskInputDefinitionRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("TaskInputDefinition",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("TaskInputDefinition",current);
    }
  ...
  public static void setEntity(TaskInputDefinition ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("TaskInputDefinition",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("TaskInputDefinition",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("TaskInputDefinition",current);
	}
