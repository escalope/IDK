...
public class ContextUseTaskRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("ContextUseTask",
	index,"/rendererxml/ContextUseTaskUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("ContextUseTask",
	index,"/rendererxml/ContextUseTaskINGENIASPanel.xml");
     ...
     */
    public ContextUseTaskRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("ContextUseTask",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("ContextUseTask",current);
    }
  ...
  public static void setEntity(ContextUseTask ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("ContextUseTask",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("ContextUseTask",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("ContextUseTask",current);
	}
