...
public class AMIContextBindingDataRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("AMIContextBindingData",
	index,"/rendererxml/AMIContextBindingDataUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("AMIContextBindingData",
	index,"/rendererxml/AMIContextBindingDataINGENIASPanel.xml");
     ...
     */
    public AMIContextBindingDataRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("AMIContextBindingData",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("AMIContextBindingData",current);
    }
  ...
  public static void setEntity(AMIContextBindingData ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("AMIContextBindingData",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("AMIContextBindingData",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("AMIContextBindingData",current);
	}
