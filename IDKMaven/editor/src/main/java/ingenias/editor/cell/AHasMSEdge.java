...
public class AHasMSEdge extends NAryEdge {

  // Constructor with data content
  public AHasMSEdge(ingenias.editor.entities.AHasMS userObject) {
    super(userObject);
    
     this.addRole("AHasMSsource");
     this.setArity("AHasMSsource",true,1);
     this.setArity("AHasMSsource",false,1);
     
this.addClass("AHasMSsource","ingenias.editor.entities.Agent");
     
this.addClass("AHasMSsource","ingenias.editor.entities.AutonomousEntityQuery");
     
    
     this.addRole("AHasMStarget");
     this.setArity("AHasMStarget",true,1);
     this.setArity("AHasMStarget",false,2147483647);
     
this.addClass("AHasMStarget","ingenias.editor.entities.MentalState");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AHasMSEdge)))
      return false;

    // n-edge to be considered.
    AHasMSEdge edge = new AHasMSEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("AHasMSsource")){
 	 edges.add( new DefaultEdge( new AHasMSsourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("AHasMStarget")){
 	 edges.add( new DefaultEdge( new AHasMStargetRole() ) );
	 }
