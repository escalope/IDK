...
public class AMICtxtModelRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("AMICtxtModel",
	index,"/rendererxml/AMICtxtModelUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("AMICtxtModel",
	index,"/rendererxml/AMICtxtModelINGENIASPanel.xml");
     ...
     */
    public AMICtxtModelRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("AMICtxtModel",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("AMICtxtModel",current);
    }
  ...
  public static void setEntity(AMICtxtModel ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("AMICtxtModel",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("AMICtxtModel",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("AMICtxtModel",current);
	}
