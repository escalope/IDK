...
public class SourcesManagerRoleRequestToAddSourceStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public SourcesManagerRoleRequestToAddSourceStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","AddingSourceRequestIU");
			    
			    
			    
			      // Receiving a message    
			      smf.add("AddingSourceRequestIU", "waiting for AddingSourceRequestIU");
			      
			      // Next states after receiving "AddingSourceRequestIU"
			      
			       smf.add("waiting for AddingSourceRequestIU","AddingSourceResponseIU");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("AddingSourceResponseIU", "endAddingSourceResponseIU");
  ...
  if (this.isState("AddingSourceRequestIU") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"AddingSourceRequestIU",
                   "RequestToAddSource",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("AddingSourceRequestIU");
      this.addState("waiting for AddingSourceRequestIU");
      this.notifyStateTransitionExecuted("AddingSourceRequestIU", "waiting for AddingSourceRequestIU");
  } 
  ...
  if (this.isState("AddingSourceResponseIU")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("AddingSourceResponseIU",optionsA,this)){
           ...
           "AddingSourceResponseIU","RequestToAddSource",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("AddingSourceResponseIU", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("AddingSourceResponseIU", "ABORTED");
    	  } else  {
