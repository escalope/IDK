...
public class PConnectsRenderer extends VertexRenderer implements CellViewRenderer{
...
private static ViewPreferences.ViewType current = ViewPreferences.ViewType.INGENIAS;
  ...
  static {
   try {
	...
	RenderComponentManager.loadRenderFile("PConnects",
	index,"/rendererxml/PConnectsNOICONPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("PConnects",
	index,"/rendererxml/PConnectsINGENIASPanel.xml");
	
	index=ViewPreferences.ViewType.LABEL;
	RenderComponentManager.loadRenderFile("PConnects",
	index,"/rendererxml/PConnectsLABELPanel.xml");
     ...
     */
     public PConnectsRenderer() {
    }

    public Dimension getSize() {
    return RenderComponentManager.getSize("PConnects",current);
    }
 ...
 public static void setEntity(ingenias.editor.entities.PConnects ent, Map attributes){
 Map currentMap=(Map)  RenderComponentManager.retrieveIDs("PConnects",ent.getPrefs(attributes).getView());
current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("PConnects",this.current);
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("PConnects",c);
	}
