...
public class ContextBindingTaskRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("ContextBindingTask",
	index,"/rendererxml/ContextBindingTaskUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("ContextBindingTask",
	index,"/rendererxml/ContextBindingTaskINGENIASPanel.xml");
     ...
     */
    public ContextBindingTaskRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("ContextBindingTask",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("ContextBindingTask",current);
    }
  ...
  public static void setEntity(ContextBindingTask ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("ContextBindingTask",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("ContextBindingTask",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("ContextBindingTask",current);
	}
