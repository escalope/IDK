...
public class IHasSpecEdge extends NAryEdge {

  // Constructor with data content
  public IHasSpecEdge(ingenias.editor.entities.IHasSpec userObject) {
    super(userObject);
    
     this.addRole("IHasSpecsource");
     this.setArity("IHasSpecsource",true,1);
     this.setArity("IHasSpecsource",false,1);
     
this.addClass("IHasSpecsource","ingenias.editor.entities.Interaction");
     
    
     this.addRole("IHasSpectarget");
     this.setArity("IHasSpectarget",true,1);
     this.setArity("IHasSpectarget",false,1);
     
this.addClass("IHasSpectarget","ingenias.editor.entities.Specification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof IHasSpecEdge)))
      return false;

    // n-edge to be considered.
    IHasSpecEdge edge = new IHasSpecEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof IHasSpecEdge)
      edge = (IHasSpecEdge) selectedEdge;
