...
public class WFSpecifiesExecutionEdge extends NAryEdge {

  // Constructor with data content
  public WFSpecifiesExecutionEdge(ingenias.editor.entities.WFSpecifiesExecution userObject) {
    super(userObject);
    
     this.addRole("WFSpecifiesExecutionsource");
     this.setArity("WFSpecifiesExecutionsource",true,1);
     this.setArity("WFSpecifiesExecutionsource",false,1);
     
this.addClass("WFSpecifiesExecutionsource","ingenias.editor.entities.Task");
     
    
     this.addRole("WFSpecifiesExecutiontarget");
     this.setArity("WFSpecifiesExecutiontarget",true,1);
     this.setArity("WFSpecifiesExecutiontarget",false,2147483647);
     
this.addClass("WFSpecifiesExecutiontarget","ingenias.editor.entities.Interaction");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFSpecifiesExecutionEdge)))
      return false;

    // n-edge to be considered.
    WFSpecifiesExecutionEdge edge = new WFSpecifiesExecutionEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof WFSpecifiesExecutionEdge)
      edge = (WFSpecifiesExecutionEdge) selectedEdge;
