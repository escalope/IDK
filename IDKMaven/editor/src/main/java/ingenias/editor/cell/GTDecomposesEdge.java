...
public class GTDecomposesEdge extends NAryEdge {

  // Constructor with data content
  public GTDecomposesEdge(ingenias.editor.entities.GTDecomposes userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof GTDecomposesEdge)))
      return false;

    // n-edge to be considered.
    GTDecomposesEdge edge = new GTDecomposesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof GTDecomposesEdge)
      edge = (GTDecomposesEdge) selectedEdge;
