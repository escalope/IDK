...
public class ConcreteAgentRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("ConcreteAgent",
	index,"/rendererxml/ConcreteAgentUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("ConcreteAgent",
	index,"/rendererxml/ConcreteAgentINGENIASPanel.xml");
     ...
     */
    public ConcreteAgentRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("ConcreteAgent",ent.getPrefs(map).getView());
  ...
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("ConcreteAgent",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("ConcreteAgent",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("ConcreteAgent",current);
	}
