...
import ingenias.exception.NotFound;
  ...
  public class RequesteeRFPDefaultCommControl extends DefaultCommControl{
  
  	  private Vector<String> previous=new Vector<String>();
  	  
  public RequesteeRFPDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  };
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLock("TicketProposal");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("AnswerDeliver") &&
    	 sb.isState("AnswerDeliver")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"TicketProposal").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("AnswerDeliver");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"TicketProposal");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLock("TicketProposal");
	       lockProcessed("TicketProposal");
           //MainInteractionManager.log("Removing lock TicketProposal",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("endAnswerDeliver");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for SendRequest")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("SendRequest")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for SendRequest");           
    	   try {
		   ...
		   futureStates.add("AnswerDeliver");
