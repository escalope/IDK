...
public class StateGoalRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("StateGoal",
	index,"/rendererxml/StateGoalUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("StateGoal",
	index,"/rendererxml/StateGoalINGENIASPanel.xml");
    ...
    public StateGoalRenderer() {
	  ...
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("StateGoal",ent.getPrefs(map).getView());
  ...
  public static void setEntity(StateGoal ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("StateGoal",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
      ...
      if (currentMap.get("LinkedGoal")!=null){
	   if (ent!=null && ent.getLinkedGoal()!=null){
		...
		((javax.swing.JLabel)( currentMap).get("LinkedGoal")).setText(ent.getLinkedGoal().toString());
		 ...
		 if (currentMap.get("LinkedGoal") instanceof javax.swing.text.JTextComponent)
		 ((javax.swing.text.JTextComponent)( currentMap).get("LinkedGoal")).setText(ent.getLinkedGoal().toString());
		
	      } 
	   } else  {
	     if (currentMap.get("LinkedGoal") instanceof javax.swing.JLabel)
	     ((javax.swing.JLabel)( currentMap).get("LinkedGoal")).setText("");
	     else {
		if (!(currentMap.get("LinkedGoal") instanceof ingenias.editor.rendererxml.CollectionPanel)) 
		((javax.swing.text.JTextComponent)( currentMap).get("LinkedGoal")).setText("");
	     }
         }
	  }
	...
	{
	return RenderComponentManager.retrievePanel("StateGoal",
