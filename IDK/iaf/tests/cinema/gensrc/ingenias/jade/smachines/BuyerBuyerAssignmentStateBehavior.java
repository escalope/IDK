...
public class BuyerBuyerAssignmentStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public BuyerBuyerAssignmentStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","RequestBeingAssistant");
			    
			    
			    
			      // Receiving a message    
			      smf.add("RequestBeingAssistant", "waiting for RequestBeingAssistant");
			      
			      // Next states after receiving "RequestBeingAssistant"
			      
			       smf.add("waiting for RequestBeingAssistant","RejectBecomingAssistant");
			      
			       smf.add("waiting for RequestBeingAssistant","AcceptBecomingAssistant");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("AcceptBecomingAssistant", "endAcceptBecomingAssistant");
			      
			    
			     // States involved in message deliver
			     
			      smf.add("RejectBecomingAssistant", "endRejectBecomingAssistant");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "BuyerAssignment","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("RequestBeingAssistant") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"RequestBeingAssistant",
                   "BuyerAssignment",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("RequestBeingAssistant");
      this.addState("waiting for RequestBeingAssistant");
      this.notifyStateTransitionExecuted("RequestBeingAssistant", "waiting for RequestBeingAssistant");
  } 
  ...
  if (this.isState("AcceptBecomingAssistant")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("AcceptBecomingAssistant",optionsA,this)){
           ...
           "AcceptBecomingAssistant","BuyerAssignment",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("AcceptBecomingAssistant", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("AcceptBecomingAssistant", "ABORTED");
    	  } else  {
  ...
  // Sends a message and synchronization commands
  if (this.isState("RejectBecomingAssistant")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("RejectBecomingAssistant",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "RejectBecomingAssistant","BuyerAssignment",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("RejectBecomingAssistant", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("RejectBecomingAssistant", "ABORTED");
    	  } else  {
    		  if (!getTimeout().isStarted())
    		  getTimeout().start(0);
    	  }
