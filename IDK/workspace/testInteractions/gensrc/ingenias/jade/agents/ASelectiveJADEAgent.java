...
public class ASelectiveJADEAgent
		 extends JADEAgent {         
 
		 public ASelectiveJADEAgent(){
		 super(new ASelectiveProtocol(),new ASelectiveInteractionLocks());
		 }
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
	    ...
	    if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
         ...
         if (tobject.getType().equals("Launch_conversation_selecting_partners")){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("InitialFact");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("InitialFact");
			 } else {
			    addExpectedInputs(tobject, "InitialFact","1",expectedInput);
	...
	public Vector tasksThatSatisfyGoal(String goalname){
		 ...
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new Launch_conversation_selecting_partnersTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("Launch_conversation_selecting_partners");		         
