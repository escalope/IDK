...
public class SourcesAlphaQualityInspectorRoleAlphaQualityInspectionRequestStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public SourcesAlphaQualityInspectorRoleAlphaQualityInspectionRequestStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","AlphaQualityInspectionRequestIU");
			    
			    
			    
			      // Receiving a message    
			      smf.add("AlphaQualityInspectionRequestIU", "waiting for AlphaQualityInspectionRequestIU");
			      
			      // Next states after receiving "AlphaQualityInspectionRequestIU"
			      
			       smf.add("waiting for AlphaQualityInspectionRequestIU","AlphaQualityDegreeIU");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("AlphaQualityDegreeIU", "endAlphaQualityDegreeIU");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "AlphaQualityInspectionRequest","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("AlphaQualityInspectionRequestIU") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"AlphaQualityInspectionRequestIU",
                   "AlphaQualityInspectionRequest",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("AlphaQualityInspectionRequestIU");
      this.addState("waiting for AlphaQualityInspectionRequestIU");
      this.notifyStateTransitionExecuted("AlphaQualityInspectionRequestIU", "waiting for AlphaQualityInspectionRequestIU");
  } 
  ...
  if (this.isState("AlphaQualityDegreeIU")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("AlphaQualityDegreeIU",optionsA,this)){
           ...
           "AlphaQualityDegreeIU","AlphaQualityInspectionRequest",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("AlphaQualityDegreeIU", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("AlphaQualityDegreeIU", "ABORTED");
    	  } else  {
