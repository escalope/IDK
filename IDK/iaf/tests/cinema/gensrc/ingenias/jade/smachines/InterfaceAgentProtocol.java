...
public class InterfaceAgentProtocol implements AgentProtocols{

public InterfaceAgentProtocol(){};		
	...
	public ActiveConversation initialiseProtocols(String agentName,RuntimeConversation conv, 
        ...
        // Adds a protocol to the protocol queue
		   ...
		   if (conv.getInteraction().getId().equals("Asking_for_a_ticket") && conv.getPlayedRole().equals("User_assistant")){
		   DefaultCommControl dcc=new User_assistantAsking_for_a_ticketDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new User_assistantAsking_for_a_ticketStateBehavior(
			agentName,
				msr,msu,
					conv,                           		
				   conv.getPlayedRole(),
		   ...
		   if (conv.getInteraction().getId().equals("BuyerAssignment") && conv.getPlayedRole().equals("User_assistant")){
		   DefaultCommControl dcc=new User_assistantBuyerAssignmentDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new User_assistantBuyerAssignmentStateBehavior(
			agentName,
				msr,msu,
					conv,                           		
				   conv.getPlayedRole(),
         ...
         throw new WrongInteraction("Agent "+agentName+" does not know any interaction protocol of type "+conv.getInteraction().getId()+" where the " +
	        ...
	        public AgentExternalDescription[] getInteractionActors(String interaction, YellowPages yp) throws ingenias.jade.exception.NoAgentsFound{
              AgentExternalDescription[] result=null;
               ...
               if (interaction.equals("Asking_for_a_ticket")){
                Vector<AgentExternalDescription> cols=new Vector<AgentExternalDescription>();
                 
                 try {
                 String cardinality="";
		 DFAgentDescription[] agents=yp.getAgents("Buyer");
                   if (agents==null || agents.length<=0)
                      throw new ingenias.jade.exception.NoAgentsFound("Could not find an agent playing the role Buyer");
                   if (cardinality.equals("1") || cardinality.equals(""))
                    cols.add(new AgentExternalDescription(agents[0].getName(),"Buyer"));
                    else
                	   if (cardinality.equals("1__*"))
						   for (int k=0;k<agents.length;k++)
                			  cols.add(new AgentExternalDescription(agents[k].getName(),"Buyer"));
               ...
               if (interaction.equals("BuyerAssignment")){
                Vector<AgentExternalDescription> cols=new Vector<AgentExternalDescription>();
