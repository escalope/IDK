...
public class WFTestAfterEdge extends NAryEdge {

  // Constructor with data content
  public WFTestAfterEdge(ingenias.editor.entities.WFTestAfter userObject) {
    super(userObject);
    
     this.addRole("WFTestAftersource");
     this.setArity("WFTestAftersource",true,1);
     this.setArity("WFTestAftersource",false,2147483647);
     
this.addClass("WFTestAftersource","ingenias.editor.entities.WFTestState");
     
    
     this.addRole("WFTestAftertarget");
     this.setArity("WFTestAftertarget",true,1);
     this.setArity("WFTestAftertarget",false,2147483647);
     
this.addClass("WFTestAftertarget","ingenias.editor.entities.WFTestState");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFTestAfterEdge)))
      return false;

    // n-edge to be considered.
    WFTestAfterEdge edge = new WFTestAfterEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof WFTestAfterEdge)
      edge = (WFTestAfterEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("WFTestAftersource")){
 	 edges.add( new DefaultEdge( new WFTestAftersourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("WFTestAftertarget")){
 	 edges.add( new DefaultEdge( new WFTestAftertargetRole() ) );
	 }
