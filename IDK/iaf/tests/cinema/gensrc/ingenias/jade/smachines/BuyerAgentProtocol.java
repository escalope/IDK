...
public class BuyerAgentProtocol implements AgentProtocols{

public BuyerAgentProtocol(){};		
	...
	public ActiveConversation initialiseProtocols(String agentName,RuntimeConversation conv, 
        ...
        // Adds a protocol to the protocol queue
		   ...
		   if (conv.getInteraction().getId().equals("Asking_for_a_ticket") && conv.getPlayedRole().equals("Buyer")){
		   	DefaultCommControl dcc=new BuyerAsking_for_a_ticketDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new BuyerAsking_for_a_ticketStateBehavior(agentName,
				msr, msu,
				conv,                           		
				conv.getPlayedRole(),
			   	actors,
		   ...
		   if (conv.getInteraction().getId().equals("BuyerAssignment") && conv.getPlayedRole().equals("Buyer")){
		   	DefaultCommControl dcc=new BuyerBuyerAssignmentDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new BuyerBuyerAssignmentStateBehavior(agentName,
				msr, msu,
				conv,                           		
				conv.getPlayedRole(),
			   	actors,
		   ...
		   if (conv.getInteraction().getId().equals("Buy_ticket") && conv.getPlayedRole().equals("Buyer")){
		   DefaultCommControl dcc=new BuyerBuy_ticketDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new BuyerBuy_ticketStateBehavior(
			agentName,
				msr,msu,
					conv,                           		
				   conv.getPlayedRole(),
         ...
         throw new WrongInteraction("Agent "+agentName+" does not know any interaction protocol of type "+conv.getInteraction().getId()+" where the " +
	        ...
	        public AgentExternalDescription[] getInteractionActors(String interaction, YellowPages yp) throws ingenias.jade.exception.NoAgentsFound{
              AgentExternalDescription[] result=null;
               ...
               if (interaction.equals("Buy_ticket")){
                Vector<AgentExternalDescription> cols=new Vector<AgentExternalDescription>();
                 
                 try {
                 String cardinality="";
		 DFAgentDescription[] agents=yp.getAgents("Seller");
                   if (agents==null || agents.length<=0)
                      throw new ingenias.jade.exception.NoAgentsFound("Could not find an agent playing the role Seller");
                   if (cardinality.equals("1") || cardinality.equals(""))
                    cols.add(new AgentExternalDescription(agents[0].getName(),"Seller"));
                    else
                	   if (cardinality.equals("1__*"))
						   for (int k=0;k<agents.length;k++)
                			  cols.add(new AgentExternalDescription(agents[k].getName(),"Seller"));
		...
		dfd.setName(agentID);
		 ...
		 dfd.addServices(sd);
                ...
                if (protocol.equals("Asking_for_a_ticket")){
		dfd.setName(agentID);
		 ServiceDescription sd = new ServiceDescription();
		 sd.setName(agentID.getLocalName()  + "-sub-df");
		 sd.setType("Buyer");
		 ...
		 dfd.addServices(sd);
                ...
                if (protocol.equals("BuyerAssignment")){
		dfd.setName(agentID);
		 ServiceDescription sd = new ServiceDescription();
		 sd.setName(agentID.getLocalName()  + "-sub-df");
		 sd.setType("Buyer");
