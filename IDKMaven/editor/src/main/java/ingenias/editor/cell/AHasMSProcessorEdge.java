...
public class AHasMSProcessorEdge extends NAryEdge {

  // Constructor with data content
  public AHasMSProcessorEdge(ingenias.editor.entities.AHasMSProcessor userObject) {
    super(userObject);
    
     this.addRole("AHasMSProcessorsource");
     this.setArity("AHasMSProcessorsource",true,1);
     this.setArity("AHasMSProcessorsource",false,1);
     
this.addClass("AHasMSProcessorsource","ingenias.editor.entities.Agent");
     
    
     this.addRole("AHasMSProcessortarget");
     this.setArity("AHasMSProcessortarget",true,1);
     this.setArity("AHasMSProcessortarget",false,1);
     
this.addClass("AHasMSProcessortarget","ingenias.editor.entities.MentalStateProcessor");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AHasMSProcessorEdge)))
      return false;

    // n-edge to be considered.
    AHasMSProcessorEdge edge = new AHasMSProcessorEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AHasMSProcessorEdge)
      edge = (AHasMSProcessorEdge) selectedEdge;
