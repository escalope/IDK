...
public class CtxtUpdatesEdge extends NAryEdge {

  // Constructor with data content
  public CtxtUpdatesEdge(ingenias.editor.entities.CtxtUpdates userObject) {
    super(userObject);
    
     this.addRole("CtxtUpdatessource");
     this.setArity("CtxtUpdatessource",true,1);
     this.setArity("CtxtUpdatessource",false,1);
     
this.addClass("CtxtUpdatessource","ingenias.editor.entities.Agent");
     
this.addClass("CtxtUpdatessource","ingenias.editor.entities.Role");
     
    
     this.addRole("CtxtUpdatestarget");
     this.setArity("CtxtUpdatestarget",true,1);
     this.setArity("CtxtUpdatestarget",false,2147483647);
     
this.addClass("CtxtUpdatestarget","ingenias.editor.entities.AMIContext");
     
this.addClass("CtxtUpdatestarget","ingenias.editor.entities.FAERIECtxtElement");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof CtxtUpdatesEdge)))
      return false;

    // n-edge to be considered.
    CtxtUpdatesEdge edge = new CtxtUpdatesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof CtxtUpdatesEdge)
      edge = (CtxtUpdatesEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("CtxtUpdatessource")){
 	 edges.add( new DefaultEdge( new CtxtUpdatessourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("CtxtUpdatestarget")){
 	 edges.add( new DefaultEdge( new CtxtUpdatestargetRole() ) );
	 }
