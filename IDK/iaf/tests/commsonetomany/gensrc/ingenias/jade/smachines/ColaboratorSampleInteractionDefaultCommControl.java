...
import ingenias.exception.NotFound;
  ...
  public class ColaboratorSampleInteractionDefaultCommControl extends DefaultCommControl{
  ...
  public ColaboratorSampleInteractionDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("Answer");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("InteractionUnit1") &&
		 ...
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Answer").isEmpty();
         ...
         if (allexist && true){
           ...
           sb.removeState("InteractionUnit1");
	   	   ...
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Answer");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("Answer");
	       lockProcessed("Answer");
           //MainInteractionManager.log("Removing lock Answer",this.getAgent().getLocalName()+"-"+sb.getConversationID());
