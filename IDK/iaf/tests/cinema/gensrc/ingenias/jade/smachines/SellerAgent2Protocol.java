...
public class SellerAgent2Protocol implements AgentProtocols{

public SellerAgent2Protocol(){};		
	...
	public ActiveConversation initialiseProtocols(String agentName,RuntimeConversation conv, 
        ...
        // Adds a protocol to the protocol queue
		   ...
		   if (conv.getInteraction().getId().equals("Buy_ticket") && conv.getPlayedRole().equals("Seller")){
		   	DefaultCommControl dcc=new SellerBuy_ticketDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new SellerBuy_ticketStateBehavior(agentName,
				msr, msu,
				conv,                           		
				conv.getPlayedRole(),
			   	actors,
