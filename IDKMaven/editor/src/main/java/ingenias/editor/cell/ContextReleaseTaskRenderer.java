...
public class ContextReleaseTaskRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("ContextReleaseTask",
	index,"/rendererxml/ContextReleaseTaskUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("ContextReleaseTask",
	index,"/rendererxml/ContextReleaseTaskINGENIASPanel.xml");
     ...
     */
    public ContextReleaseTaskRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("ContextReleaseTask",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("ContextReleaseTask",current);
    }
  ...
  public static void setEntity(ContextReleaseTask ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("ContextReleaseTask",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("ContextReleaseTask",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("ContextReleaseTask",current);
	}
