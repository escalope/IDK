...
public class DecisionNodeRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("DecisionNode",
	index,"/rendererxml/DecisionNodeUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("DecisionNode",
	index,"/rendererxml/DecisionNodeINGENIASPanel.xml");
     ...
     */
    public DecisionNodeRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("DecisionNode",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("DecisionNode",current);
    }
  ...
  public static void setEntity(DecisionNode ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("DecisionNode",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("DecisionNode",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("DecisionNode",current);
	}
