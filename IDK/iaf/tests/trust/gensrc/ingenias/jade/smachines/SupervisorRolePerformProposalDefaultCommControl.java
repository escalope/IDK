...
import ingenias.exception.NotFound;
  ...
  public class SupervisorRolePerformProposalDefaultCommControl extends DefaultCommControl{
  
  	  private Vector<String> previous=new Vector<String>();
  	  
  public SupervisorRolePerformProposalDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  };
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("PropuestaRechazada");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("PropuestaRechazadaIU") &&
    	 sb.isState("PropuestaRechazadaIU")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"PropuestaRechazada").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("PropuestaRechazadaIU");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"PropuestaRechazada");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLock(mfcontent);
	       lockProcessed("PropuestaRechazada");
           //MainInteractionManager.log("Removing lock PropuestaRechazada",this.getAgent().getLocalName()+"-"+sb.getConversationID());
    ...
    if (stateToEvaluate.equals("PropuestaAceptadaIU") &&
    	 sb.isState("PropuestaAceptadaIU")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         ...
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("PropuestaAceptadaIU");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for PropuestaIU")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("PropuestaIU")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for PropuestaIU");           
    	   try {
		   ...
		   futureStates.add("PropuestaRechazadaIU");
                  
		   futureStates.add("PropuestaAceptadaIU");
