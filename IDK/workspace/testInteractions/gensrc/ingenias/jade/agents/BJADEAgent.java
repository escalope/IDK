...
public class BJADEAgent
		 extends JADEAgent {         
 
		 public BJADEAgent(){
		 super(new BProtocol(),new BInteractionLocks());
		 }
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
	    ...
	    if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("SingleAnswer");
             if (expectedInput.size()==0){
				nonExistingInputs.add("SingleAnswer");
			 } else {
			    addExpectedInputs(tobject, "SingleAnswer","1",expectedInput);
             	addConsumedInput(to,"SingleAnswer",expectedInput);
			 }
	...
	private boolean initialiseConversationalTask(RuntimeConversation conversation, Task tobject) {
	   ...
	   boolean validConversationType=false;
		...
		if (validConversationType){
       	...
       	if (tobject.getType().equals("GeneratingAnswerWithinSimpleDeliver")){
	        Vector<MentalEntity> expectedInput=null;
		...
		boolean allEntitiesExist=true;
            ...
            expectedInput=this.getMSM().obtainConversationalMentalEntityByType(conversation,"SingleQuestion");
        ...
        /*     
	     ...
	     boolean alreadyExists=true;
		    ...
		    {SingleAnswer expectedOutputSingleAnswer=		    
		     new SingleAnswer(MentalStateManager.generateMentalEntityID());			
             to.add(new OutputEntity(expectedOutputSingleAnswer,TaskOperations.CreateWF));
            }
         ...
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new GeneratingAnswerWithinSimpleDeliverTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("GeneratingAnswerWithinSimpleDeliver");					
