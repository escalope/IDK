...
public class Agent0JADEAgent
		...
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
			    ...
			    addExpectedInputs(tobject, "FrameFact0","1",expectedInput);
             	addConsumedInput(to,"FrameFact0",expectedInput);
			 }
	 ...
	 /*    
	      ...
	      */	  
	      ...
	      /**/	      
		    ...
		    {FrameFact0 expectedOutputFrameFact0=		    
		     new FrameFact0(MentalStateManager.generateMentalEntityID());			
             to.add(new OutputEntity(expectedOutputFrameFact0,TaskOperations.CreateWF));
            }
	...
	private boolean initialiseConversationalTask(RuntimeConversation conversation, Task tobject) {
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		expectedInput=this.getMSM().obtainConversationalMentalEntityByType(conversation,"FrameFact0");
		if (this.getLM().canBeDeleted(expectedInput)){                          
             if (expectedInput.size()==0){
				nonExistingInputs.add("FrameFact0");
			 } else {
         ...
         _jaRole0Interaction0.addBehaviour(
           ...
           public void action() {
			 ...
			 boolean allexist=true;
			  ...
			  expectedInput=getMSM().getMentalEntityByType("FrameFact0");
