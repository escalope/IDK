...
public class FAERIECtxtRelationshipRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("FAERIECtxtRelationship",
	index,"/rendererxml/FAERIECtxtRelationshipUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("FAERIECtxtRelationship",
	index,"/rendererxml/FAERIECtxtRelationshipINGENIASPanel.xml");
     ...
     */
    public FAERIECtxtRelationshipRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("FAERIECtxtRelationship",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("FAERIECtxtRelationship",current);
    }
  ...
  public static void setEntity(FAERIECtxtRelationship ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("FAERIECtxtRelationship",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("FAERIECtxtRelationship",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("FAERIECtxtRelationship",current);
	}
