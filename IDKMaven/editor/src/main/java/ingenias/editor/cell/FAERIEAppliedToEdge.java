...
public class FAERIEAppliedToEdge extends NAryEdge {

  // Constructor with data content
  public FAERIEAppliedToEdge(ingenias.editor.entities.FAERIEAppliedTo userObject) {
    super(userObject);
    
     this.addRole("FAERIEAppliedTosource");
     this.setArity("FAERIEAppliedTosource",true,1);
     this.setArity("FAERIEAppliedTosource",false,1);
     
this.addClass("FAERIEAppliedTosource","ingenias.editor.entities.FAERIECtxtAttribute");
     
this.addClass("FAERIEAppliedTosource","ingenias.editor.entities.FAERIECtxtEntity");
     
    
     this.addRole("FAERIEAppliedTotarget");
     this.setArity("FAERIEAppliedTotarget",true,1);
     this.setArity("FAERIEAppliedTotarget",false,2147483647);
     
this.addClass("FAERIEAppliedTotarget","ingenias.editor.entities.FAERIECtxtEntity");
     
this.addClass("FAERIEAppliedTotarget","ingenias.editor.entities.FAERIECtxtAttribute");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof FAERIEAppliedToEdge)))
      return false;

    // n-edge to be considered.
    FAERIEAppliedToEdge edge = new FAERIEAppliedToEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof FAERIEAppliedToEdge)
      edge = (FAERIEAppliedToEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("FAERIEAppliedTosource")){
 	 edges.add( new DefaultEdge( new FAERIEAppliedTosourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("FAERIEAppliedTotarget")){
 	 edges.add( new DefaultEdge( new FAERIEAppliedTotargetRole() ) );
	 }
