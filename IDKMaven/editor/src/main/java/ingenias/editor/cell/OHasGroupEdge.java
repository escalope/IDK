...
public class OHasGroupEdge extends NAryEdge {

  // Constructor with data content
  public OHasGroupEdge(ingenias.editor.entities.OHasGroup userObject) {
    super(userObject);
    
     this.addRole("OHasGroupsource");
     this.setArity("OHasGroupsource",true,1);
     this.setArity("OHasGroupsource",false,1);
     
this.addClass("OHasGroupsource","ingenias.editor.entities.Organization");
     
    
     this.addRole("OHasGrouptarget");
     this.setArity("OHasGrouptarget",true,1);
     this.setArity("OHasGrouptarget",false,2147483647);
     
this.addClass("OHasGrouptarget","ingenias.editor.entities.OrganizationGroup");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof OHasGroupEdge)))
      return false;

    // n-edge to be considered.
    OHasGroupEdge edge = new OHasGroupEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof OHasGroupEdge)
      edge = (OHasGroupEdge) selectedEdge;
