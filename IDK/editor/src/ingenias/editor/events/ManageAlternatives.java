 ...
 *
 */
public class ManageAlternatives
  ...
  public ManageAlternatives(JGraph graph) {
    this.graph = graph;
  }
  ...
  private AUMLAlternativeBoxCell createAUMLBoxCell(ingenias.editor.Model model) {
    AUMLAlternativeBox box =
        ingenias.editor.ObjectManager.getInstance().createAUMLAlternativeBox(
        "box" + ingenias.editor.Editor.getNewId());

    ingenias.editor.cell.AUMLAlternativeBoxCell aumlbox =
        new ingenias.editor.cell.AUMLAlternativeBoxCell(box);
    Map pmap = new Hashtable();
    // col.addPorts(port);
    Hashtable ht = new Hashtable();
    ht.put(aumlbox, pmap);
...
// col.addPorts(port);
  ...
  public void processEdge(ingenias.editor.cell.AUMLSelectionEdge edge,
                          ingenias.editor.Model model) {
    ...
    if (edges!=null && edges.length>0){
      ...
      ingenias.editor.cell.AUMLAlternativeBoxCell asbcell =
          (AUMLAlternativeBoxCell)this.createAUMLBoxCell(model);
      ingenias.editor.entities.AUMLAlternativeBox asb =
          (AUMLAlternativeBox) asbcell.getUserObject();

      ingenias.editor.cell.AUMLAlternativeRowCell aarcell = this.
          createAUMLRowCell(model);
      ingenias.editor.entities.AUMLAlternativeRow aarc =
          (AUMLAlternativeRow) aarcell.getUserObject();

      ColumnCell ncc1 = this.createColumnCell(model);
      ColumnCell ncc2 = this.createColumnCell(model);
      ColumnCell ncunderalt1 = this.createColumnCell(model);
      Column colunderalt1=(Column)ncunderalt1.getUserObject();
      ColumnCell ncunderalt2 = this.createColumnCell(model);
      Column colunderalt2=(Column)ncunderalt2.getUserObject();
      ...
      aarc.addChildren(col1);
      aarc.addChildren(col2);

      asb.addChildren(aarc);
      asb.setParent(ll1.getParent());
      ( (AUMLContainer) ll1.getParent()).addChildren(asb);
      aarc.setParent(asb);

      ll1.addChildren(col1);
      ll1.addChildren(colunderalt1);
      ll2.addChildren(col2);
      ll2.addChildren(colunderalt2);
      ...
      Map m3 = asbcell.getAttributes();
      GraphConstants.setBounds(m3, selectionrect);

      Map m4 = aarcell.getAttributes();
      GraphConstants.setBounds(m4,
                               new Rectangle(selectionrect.x + 20,
                                             selectionrect.y + 10,
      ...
      Map m7 = portc2.getAttributes();
      GraphConstants.setBounds(m7,
                               new Rectangle( (int) (llcr1.x + (llcr1.width / 2) -
          5), y + 80, 30, 50));

      Map m8 = portc2.getAttributes();
      GraphConstants.setBounds(m8,
                               new Rectangle( (int) (llcr2.x + (llcr2.width / 2) -
          5), y + 80, 30, 50));
      ...
      ht.put(ncc1, m1);
      ht.put(ncc2, m2);
      ht.put(ncunderalt1, m6);
      ht.put(ncunderalt2, m7);
      ht.put(portc1, m5);
      ht.put(portc2, m6);
      ht.put(asbcell, m3);
      ht.put(aarcell, m4);
  ...
  public void graphChanged(org.jgraph.event.GraphModelEvent gme) {
    ...
    if (enabled && this.workingObject == null &&
      ...
      ingenias.editor.cell.AUMLSelectionEdge asel = null;
      ingenias.editor.Model model = (ingenias.editor.Model) gme.getSource();
      for (int k = 0; k < model.getRootCount() && !foundEdge; k++) {
        foundEdge = model.getRootAt(k) instanceof ingenias.editor.cell.AUMLSelectionEdge;
        if (foundEdge) {
          asel = (ingenias.editor.cell.AUMLSelectionEdge) model.getRootAt(k);
        }
      }
      if (foundEdge) {
