...
public class OrgDplmntEdge extends NAryEdge {

  // Constructor with data content
  public OrgDplmntEdge(ingenias.editor.entities.OrgDplmnt userObject) {
    super(userObject);
    
     this.addRole("OrgDplmntsource");
     this.setArity("OrgDplmntsource",true,1);
     this.setArity("OrgDplmntsource",false,1);
     
this.addClass("OrgDplmntsource","ingenias.editor.entities.OrgDeploymentUnit");
     
    
     this.addRole("OrgDplmnttarget");
     this.setArity("OrgDplmnttarget",true,1);
     this.setArity("OrgDplmnttarget",false,2147483647);
     
this.addClass("OrgDplmnttarget","ingenias.editor.entities.GroupDeploymentUnit");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof OrgDplmntEdge)))
      return false;

    // n-edge to be considered.
    OrgDplmntEdge edge = new OrgDplmntEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof OrgDplmntEdge)
      edge = (OrgDplmntEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("OrgDplmntsource")){
 	 edges.add( new DefaultEdge( new OrgDplmntsourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("OrgDplmnttarget")){
 	 edges.add( new DefaultEdge( new OrgDplmnttargetRole() ) );
	 }
