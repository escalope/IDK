...
public class GTDependsRenderer extends VertexRenderer implements CellViewRenderer{
...
private static ViewPreferences.ViewType current = ViewPreferences.ViewType.INGENIAS;
  ...
  static {
   try {
	...
	RenderComponentManager.loadRenderFile("GTDepends",
	index,"/rendererxml/GTDependsNOICONPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("GTDepends",
	index,"/rendererxml/GTDependsINGENIASPanel.xml");
	
	index=ViewPreferences.ViewType.LABEL;
	RenderComponentManager.loadRenderFile("GTDepends",
	index,"/rendererxml/GTDependsLABELPanel.xml");
     ...
     */
     public GTDependsRenderer() {
    }

    public Dimension getSize() {
    return RenderComponentManager.getSize("GTDepends",current);
    }
 ...
 public static void setEntity(ingenias.editor.entities.GTDepends ent, Map attributes){
 Map currentMap=(Map)  RenderComponentManager.retrieveIDs("GTDepends",ent.getPrefs(attributes).getView());
current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("GTDepends",this.current);
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("GTDepends",c);
	}
