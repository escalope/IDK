...
public class GTModifiesEdge extends NAryEdge {

  // Constructor with data content
  public GTModifiesEdge(ingenias.editor.entities.GTModifies userObject) {
    super(userObject);
...
this.addClass("WFConsumessource","ingenias.editor.entities.Workflow");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof GTModifiesEdge)))
      return false;

    // n-edge to be considered.
    GTModifiesEdge edge = new GTModifiesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof GTModifiesEdge)
      edge = (GTModifiesEdge) selectedEdge;
