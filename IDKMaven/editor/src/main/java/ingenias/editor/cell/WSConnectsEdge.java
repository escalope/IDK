...
public class WSConnectsEdge extends NAryEdge {

  // Constructor with data content
  public WSConnectsEdge(ingenias.editor.entities.WSConnects userObject) {
    super(userObject);
    
     this.addRole("WSConnectssource");
     this.setArity("WSConnectssource",true,1);
     this.setArity("WSConnectssource",false,1);
     
this.addClass("WSConnectssource","ingenias.editor.entities.TaskWS");
     
    
     this.addRole("WSConnectstarget");
     this.setArity("WSConnectstarget",true,1);
     this.setArity("WSConnectstarget",false,2147483647);
     
this.addClass("WSConnectstarget","ingenias.editor.entities.TaskWS");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WSConnectsEdge)))
      return false;

    // n-edge to be considered.
    WSConnectsEdge edge = new WSConnectsEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof WSConnectsEdge)
      edge = (WSConnectsEdge) selectedEdge;
