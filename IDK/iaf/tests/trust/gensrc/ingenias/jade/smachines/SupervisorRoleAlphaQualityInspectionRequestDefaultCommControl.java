...
import ingenias.exception.NotFound;
  ...
  public class SupervisorRoleAlphaQualityInspectionRequestDefaultCommControl extends DefaultCommControl{
  ...
  public SupervisorRoleAlphaQualityInspectionRequestDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("InspectQualityOfSourceInTesting");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("InspectQualityOfSourceInTesting");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("AlphaQualityInspectionRequestIU") &&
    	 sb.isState("AlphaQualityInspectionRequestIU")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"InspectQualityOfSourceInTesting").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("AlphaQualityInspectionRequestIU");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"InspectQualityOfSourceInTesting");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("InspectQualityOfSourceInTesting");
	       lockProcessed("InspectQualityOfSourceInTesting");
           //MainInteractionManager.log("Removing lock InspectQualityOfSourceInTesting",this.getAgent().getLocalName()+"-"+sb.getConversationID());
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for AlphaQualityDegreeIU")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("AlphaQualityDegreeIU")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for AlphaQualityDegreeIU");           
    	   //try {
