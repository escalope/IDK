...
public class ARoleInheritanceEdge extends NAryEdge {

  // Constructor with data content
  public ARoleInheritanceEdge(ingenias.editor.entities.ARoleInheritance userObject) {
    super(userObject);
    
     this.addRole("ARoleInheritancesource");
     this.setArity("ARoleInheritancesource",true,1);
     this.setArity("ARoleInheritancesource",false,1);
     
this.addClass("ARoleInheritancesource","ingenias.editor.entities.Role");
     
    
     this.addRole("ARoleInheritancetarget");
     this.setArity("ARoleInheritancetarget",true,1);
     this.setArity("ARoleInheritancetarget",false,1);
     
this.addClass("ARoleInheritancetarget","ingenias.editor.entities.Role");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof ARoleInheritanceEdge)))
      return false;

    // n-edge to be considered.
    ARoleInheritanceEdge edge = new ARoleInheritanceEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof ARoleInheritanceEdge)
      edge = (ARoleInheritanceEdge) selectedEdge;
