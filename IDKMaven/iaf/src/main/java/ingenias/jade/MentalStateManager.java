...
MentalStateUpdater {
	...
	private java.util.concurrent.ConcurrentLinkedQueue<Boolean> graphModificationQueue=new java.util.concurrent.ConcurrentLinkedQueue<Boolean>(); 
			...
			Runnable resize=new Runnable(){
				public void run(){
					...
					for (int k=0;k<gm.getRootCount();k++){
						...
						if (ent1!=null &&
								RenderComponentManager.retrievePanel(type,ent1.getPrefs(null).getView())!=null){
							Dimension dim1=RenderComponentManager.getSize(ent1,type,ent1.getPrefs(null).getView());
	...
	public synchronized boolean isGraphModificationQueueEmpty() {
		return graphModificationQueue.isEmpty(); 			
	}
	public synchronized void removeFirstFromGraphModificationQueue() {
		graphModificationQueue.poll(); 			
	}
	...
	public synchronized void unlockChanges() {
		if (!ownerThread.equals(Thread.currentThread())){
			throw new RuntimeException("Thread "+Thread.currentThread()+" is trying to gain access to a a resource locked by thread "+ownerThread);
		}
		changesAllowed=true;			
		notifyAll();			
	}
