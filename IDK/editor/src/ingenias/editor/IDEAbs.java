...
public abstract class IDEAbs
extends ingenias.editor.IDEGUI
implements java.io.Serializable, ClipboardOwner
{
	...
	public IDEAbs() {
		...
		ids.prop.put("IDK:extfolder",
				new ProjectProperty("IDK","extfolder", "Extension Module Folder",
						"ext",
				"Folder where the IDE will find its new modules"));
		Log.initInstance(new PrintWriter(new TextAreaOutputStream(this.moduleOutput)),
				new PrintWriter(new TextAreaOutputStream(this.logs)));
		try {
							...
							if (url.getHost().equals("app")){
								...
								if (url.getHost().equals("ent")){
									...
									if (userobject.size()==0){
										...
										/*ingenias.editor.cell.GeneralEditionFrame jf = new ingenias.editor.
							cell.GeneralEditionFrame(ids.editor, ids.om, null,
							"Edit " + entity, (Entity)userobject.elementAt(0));
		...
		/*int backupCopied=this.abackup.copyBackupFiles();
		if (backupCopied==-1){
			Log.getInstance().logERROR("There was an error while copying backup files to your backup folder. Please, check the error output in the console");
		} else {
			if (backupCopied>0)
				Log.getInstance().logWARNING("There were backup files when this editor started. I have copied them to your backup folder in the IDK distribution");
		}*/
		try {
			restorePreferences();
			...
			if (current!=null){
				...
				vfmi.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if (changes) {
							result = JOptionPane.showConfirmDialog(_ide,
	...
	public void clearProjects() {
		...
		ids = new IDEState(null,
				this.ids.om.root, this.ids.om.arbolObjetos, this.ids.gm.root,
				this.ids.gm.arbolProyecto);
		ids.editor = new Editor(ids.om);
		pprin.add(ids.editor, java.awt.BorderLayout.CENTER);
		this.validate();
		this.repaint();
	...
	void undo_actionPerformed(ActionEvent e) {
		ActionEvent e1 = new ActionEvent(this.ids.editor.graph, e.getID(),
	...
	void copy_actionPerformed(ActionEvent e) {
		ActionEvent e1 = new ActionEvent(this.ids.editor.graph, e.getID(),
				e.getActionCommand(), e.getModifiers());
		if (ids.editor.getGraph()!=null) {
			(this.ids.editor.graph.getTransferHandler().getCopyAction()).
			actionPerformed(e1);
			setChanged();
		}
	}
	...
	void paste_actionPerformed(ActionEvent e) {
		ActionEvent e1 = new ActionEvent(this.ids.editor.graph, e.getID(),
				e.getActionCommand(), e.getModifiers());
		if (ids.editor.getGraph()!=null) {
			(this.ids.editor.graph.getTransferHandler().getPasteAction()).
			actionPerformed(e1);
			setChanged();
		}	
	}
	...
	void newProject_actionPerformed(ActionEvent e) {
		...
		if (result == JOptionPane.OK_OPTION) {
			...
			this.replaceTree(this.rootObjetos,
					(DefaultMutableTreeNode) nids.om.arbolObjetos.getModel().
					getRoot());
			this.replaceTree(this.rootProject,
