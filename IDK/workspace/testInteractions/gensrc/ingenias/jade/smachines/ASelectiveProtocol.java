...
public class ASelectiveProtocol implements AgentProtocols{

public ASelectiveProtocol(){};		
	...
	public ActiveConversation initialiseProtocols(String agentName,RuntimeConversation conv, 
        ...
        // Adds a protocol to the protocol queue
          StateBehavior   sb=null;
		   ...
		   if (conv.getInteraction().getId().equals("IntExample") && conv.getPlayedRole().equals("Initiator")){
		   DefaultCommControl dcc=new InitiatorIntExampleDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new InitiatorIntExampleStateBehavior(
			agentName,
				msr,msu,
					conv,                           		
				   conv.getPlayedRole(),
         ...
         return null;
	        ...
	        public DFAgentDescription[] getInteractionActors(String interaction, YellowPages yp) throws ingenias.jade.exception.NoAgentsFound{
              DFAgentDescription[] result=null;
               ...
               if (interaction.equals("IntExample")){
                 Vector cols=new Vector();
                 
                 try {
                 String cardinality="1__*";
		 DFAgentDescription[] agents=yp.getAgents("Participant2");
                   if (agents.length<=0)
                      throw new ingenias.jade.exception.NoAgentsFound("Could not find an agent playing the role Participant2");
                 ...
                 try {
                 String cardinality="";
		 DFAgentDescription[] agents=yp.getAgents("Participant1");
                   if (agents.length<=0)
                      throw new ingenias.jade.exception.NoAgentsFound("Could not find an agent playing the role Participant1");
                   if (cardinality.equals("1") || cardinality.equals(""))
                    cols.add(agents[0]);
                    else
                	   if (cardinality.equals("1__*"))
	...
	public boolean verifyActors(String protocol, DFAgentDescription[ ] actors) {
        ...
        if (protocol.equals("IntExample")){
            ...
            HashSet<String> rolesFound=new HashSet<String>();
         	...
         	toVerify.add("Participant2");
         	
         	toVerify.add("Participant1");
