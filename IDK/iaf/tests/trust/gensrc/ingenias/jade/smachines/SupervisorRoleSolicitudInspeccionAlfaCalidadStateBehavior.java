...
public class SupervisorRoleSolicitudInspeccionAlfaCalidadStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public SupervisorRoleSolicitudInspeccionAlfaCalidadStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "SolicitudInspeccionAlfaCalidadUI");
			    
			    
			      // Receiving a message    
			      smf.add("GradoCalidadCuarentenaListoUI", "waiting for GradoCalidadCuarentenaListoUI");
			      
			      // Next states after receiving "GradoCalidadCuarentenaListoUI"
			      
			       smf.add("waiting for GradoCalidadCuarentenaListoUI","endGradoCalidadCuarentenaListoUI");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("SolicitudInspeccionAlfaCalidadUI", "GradoCalidadCuarentenaListoUI");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
  	try {  	  
      String[] options=new String[]{"SolicitudInspeccionAlfaCalidadUI"};
      AID[] actors=null;
      Vector<AID> actorsv=new Vector<AID>();
      Vector<String> rolesv=new Vector<String>();
      
      actorsv.addAll(this.getActor("SourcesAlfaQualityInspectorRole"));
      {      
       Vector<AID> receivers=this.getActor("SourcesAlfaQualityInspectorRole");      
       for (AID aid:receivers){
        rolesv.add("SourcesAlfaQualityInspectorRole");
       }
      }
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","SolicitudInspeccionAlfaCalidad",completelistInvolvedActors);
  ...
  if (this.isState("GradoCalidadCuarentenaListoUI") ) {  // State changed by other agent and upted
    //in the parent class
         
      Vector options=new Vector();
      
      options.add("endGradoCalidadCuarentenaListoUI");
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"GradoCalidadCuarentenaListoUI",
                   "SolicitudInspeccionAlfaCalidad",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("GradoCalidadCuarentenaListoUI");
      this.addState("waiting for GradoCalidadCuarentenaListoUI");
      this.notifyStateTransitionExecuted("GradoCalidadCuarentenaListoUI", "waiting for GradoCalidadCuarentenaListoUI");
  } 
  ...
  if (this.isState("SolicitudInspeccionAlfaCalidadUI")) {
     ...
     try {
      ...
      options.add("GradoCalidadCuarentenaListoUI");      
      
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("SolicitudInspeccionAlfaCalidadUI",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "SolicitudInspeccionAlfaCalidadUI","SolicitudInspeccionAlfaCalidad",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("SolicitudInspeccionAlfaCalidadUI", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("SolicitudInspeccionAlfaCalidadUI", "ABORTED");
    	  } else  {
  ...
  if (this.isState("endGradoCalidadCuarentenaListoUI")) {
