...
public class WorkflowRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("Workflow",
	index,"/rendererxml/WorkflowUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("Workflow",
	index,"/rendererxml/WorkflowINGENIASPanel.xml");
    ...
    public WorkflowRenderer() {
	  ...
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("Workflow",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("Workflow",current);
    }
  ...
  public static void setEntity(Workflow ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("Workflow",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("Workflow",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("Workflow",current);
	}
