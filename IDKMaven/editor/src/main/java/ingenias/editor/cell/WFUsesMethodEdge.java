...
public class WFUsesMethodEdge extends NAryEdge {

  // Constructor with data content
  public WFUsesMethodEdge(ingenias.editor.entities.WFUsesMethod userObject) {
    super(userObject);
    
     this.addRole("WFUsessource");
     this.setArity("WFUsessource",true,1);
     this.setArity("WFUsessource",false,1);
     
this.addClass("WFUsessource","ingenias.editor.entities.Task");
     
this.addClass("WFUsessource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("WFUsestarget");
     this.setArity("WFUsestarget",true,1);
     this.setArity("WFUsestarget",false,2147483647);
     
this.addClass("WFUsestarget","ingenias.editor.entities.Application");
     
this.addClass("WFUsestarget","ingenias.editor.entities.Resource");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFUsesMethodEdge)))
      return false;

    // n-edge to be considered.
    WFUsesMethodEdge edge = new WFUsesMethodEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof WFUsesMethodEdge)
      edge = (WFUsesMethodEdge) selectedEdge;
