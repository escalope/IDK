...
public class FAERIEGeneratedByEdge extends NAryEdge {

  // Constructor with data content
  public FAERIEGeneratedByEdge(ingenias.editor.entities.FAERIEGeneratedBy userObject) {
    super(userObject);
    
     this.addRole("FAERIEGeneratedBysource");
     this.setArity("FAERIEGeneratedBysource",true,1);
     this.setArity("FAERIEGeneratedBysource",false,1);
     
this.addClass("FAERIEGeneratedBysource","ingenias.editor.entities.FAERIECtxtValue");
     
    
     this.addRole("FAERIEGeneratedBytarget");
     this.setArity("FAERIEGeneratedBytarget",true,1);
     this.setArity("FAERIEGeneratedBytarget",false,2147483647);
     
this.addClass("FAERIEGeneratedBytarget","ingenias.editor.entities.FAERIECtxtEntity");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof FAERIEGeneratedByEdge)))
      return false;

    // n-edge to be considered.
    FAERIEGeneratedByEdge edge = new FAERIEGeneratedByEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof FAERIEGeneratedByEdge)
      edge = (FAERIEGeneratedByEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("FAERIEGeneratedBysource")){
 	 edges.add( new DefaultEdge( new FAERIEGeneratedBysourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("FAERIEGeneratedBytarget")){
 	 edges.add( new DefaultEdge( new FAERIEGeneratedBytargetRole() ) );
	 }
