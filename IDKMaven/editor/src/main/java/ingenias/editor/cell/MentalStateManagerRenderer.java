...
public class MentalStateManagerRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("MentalStateManager",
	index,"/rendererxml/MentalStateManagerUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("MentalStateManager",
	index,"/rendererxml/MentalStateManagerINGENIASPanel.xml");
     ...
     */
    public MentalStateManagerRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("MentalStateManager",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("MentalStateManager",current);
    }
  ...
  public static void setEntity(MentalStateManager ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("MentalStateManager",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
        ...
        if (currentMap.get("ManagerDescription")!=null && 
            currentMap.get("ManagerDescription") instanceof ingenias.editor.rendererxml.CollectionPanel) {
          try {
            ( (ingenias.editor.rendererxml.CollectionPanel) currentMap.get(
                "ManagerDescription")).setCollection("ManagerDescription",ent.ManagerDescription, ent.ManagerDescription.getType());
          }
	...
	{
	return RenderComponentManager.retrievePanel("MentalStateManager",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("MentalStateManager",current);
	}
