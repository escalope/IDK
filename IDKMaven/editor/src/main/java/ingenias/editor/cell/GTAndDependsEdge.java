...
public class GTAndDependsEdge extends NAryEdge {
  ...
  public GTAndDependsEdge(ingenias.editor.entities.GTAndDepends userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof GTAndDependsEdge)))
      return false;
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof GTAndDependsEdge)
      edge = (GTAndDependsEdge) selectedEdge;
