...
public class BuyerAgentJADEAgent
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
		...
		if (this.getLM().canBeDeleted("CouldNotGetTicket")){
		...
		if (this.getLM().canBeDeleted("RequestedMovie")){
             expectedInput=this.getMSM().getMentalEntityByType("RequestedMovie");
             if (expectedInput.size()==0){
				nonExistingInputs.add("RequestedMovie");
			 } else {
			    addExpectedInputs(tobject, "RequestedMovie","1",expectedInput);
             	addConsumedInput(to,"RequestedMovie",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
		} else {
		allEntitiesExist=false;
		...
		if (this.getLM().canBeDeleted("Ticket_data")){
	...
	private boolean initialiseConversationalTask(RuntimeConversation conversation, Task tobject) {
	   ...
	   boolean validConversationType=false;
		...
		if (validConversationType){
       	...
       	if (tobject.getType().equals("AbortTicketSearch")){
	        Vector<MentalEntity> expectedInput=null;
	     ...
	     tobject.addOutput(to);
     	      ...
     	      if (!allEntitiesExist){
				...
				for (int j=0;j<nonExistingInputs.size();j++){
	        	 ...
	        	 " to achieve goal FindCinema because did not have all preconditions. Missing elements are:"+nonexisting+
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	repeatedOutputs.clear();
       	if (tobject.getType().equals("Process_no_tickets_available")){
	        Vector<MentalEntity> expectedInput=null;
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	repeatedOutputs.clear();
       	if (tobject.getType().equals("ChooseCinema")){
	        Vector<MentalEntity> expectedInput=null;
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	repeatedOutputs.clear();
       	if (tobject.getType().equals("Look_for_another")){
	        Vector<MentalEntity> expectedInput=null;
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	repeatedOutputs.clear();
       	if (tobject.getType().equals("Process_ticket")){
	        Vector<MentalEntity> expectedInput=null;
	     ...
	     tobject.addOutput(to);
     	      ...
     	      if (!allEntitiesExist){
				...
				for (int j=0;j<nonExistingInputs.size();j++){
       	        ...
       	        MainInteractionManager.log("Conversational intialisation discarded task "+tobject.getType()+
	        	 " to achieve goal GetTicket because did not have all preconditions. Missing elements are:"+nonexisting+
                        ". Currently, the MS has the following elements:"+this.getMSM().getAllMentalEntities(),getLocalName()+"-"+tobject.getType());
			   }
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	repeatedOutputs.clear();
       	if (tobject.getType().equals("Process_offer")){
	        Vector<MentalEntity> expectedInput=null;
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	repeatedOutputs.clear();
       	if (tobject.getType().equals("PayTicket")){
	        Vector<MentalEntity> expectedInput=null;
         ...
         if (goalname.equals("FindCinema")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				boolean canbescheduled=false;
				for (int k=0;k<conversations.size();k++){
					tobject=new Look_for_anotherTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
						MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal FindCinema",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
