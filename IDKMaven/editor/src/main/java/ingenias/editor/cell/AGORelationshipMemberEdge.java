...
public class AGORelationshipMemberEdge extends NAryEdge {

  // Constructor with data content
  public AGORelationshipMemberEdge(ingenias.editor.entities.AGORelationshipMember userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AGORelationshipMemberEdge)))
      return false;

    // n-edge to be considered.
    AGORelationshipMemberEdge edge = new AGORelationshipMemberEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AGORelationshipMemberEdge)
      edge = (AGORelationshipMemberEdge) selectedEdge;
