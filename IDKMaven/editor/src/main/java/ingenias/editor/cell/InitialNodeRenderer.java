...
public class InitialNodeRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("InitialNode",
	index,"/rendererxml/InitialNodeUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("InitialNode",
	index,"/rendererxml/InitialNodeINGENIASPanel.xml");
     ...
     */
    public InitialNodeRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("InitialNode",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("InitialNode",current);
    }
  ...
  public static void setEntity(InitialNode ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("InitialNode",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("InitialNode",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("InitialNode",current);
	}
