...
public class ConditionalMentalStateRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("ConditionalMentalState",
	index,"/rendererxml/ConditionalMentalStateUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("ConditionalMentalState",
	index,"/rendererxml/ConditionalMentalStateINGENIASPanel.xml");
     ...
     */
    public ConditionalMentalStateRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("ConditionalMentalState",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("ConditionalMentalState",current);
    }
  ...
  public static void setEntity(ConditionalMentalState ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("ConditionalMentalState",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("ConditionalMentalState",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("ConditionalMentalState",current);
	}
