...
public class EPerceivesPollingEdge extends NAryEdge {

  // Constructor with data content
  public EPerceivesPollingEdge(ingenias.editor.entities.EPerceivesPolling userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof EPerceivesPollingEdge)))
      return false;

    // n-edge to be considered.
    EPerceivesPollingEdge edge = new EPerceivesPollingEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof EPerceivesPollingEdge)
      edge = (EPerceivesPollingEdge) selectedEdge;
