...
public class PerfectSourceQualityInspectorJADEAgent
		 extends JADEAgent {         
 
		 public PerfectSourceQualityInspectorJADEAgent(){
		 super(new PerfectSourceQualityInspectorProtocol(),new PerfectSourceQualityInspectorInteractionLocks());
		 }
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
		...
		expectedInput=this.getMSM().getMentalEntityByType("SourceQualityDegree");
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
			    ...
			    addExpectedInputs(tobject, "SourceQualityDegree","1",expectedInput);
             	addConsumedInput(to,"SourceQualityDegree",expectedInput);
			 }
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	...
  	   	boolean correctRole=conversation.getPlayedRole().equals ("PerfectSourceQualityInspectorRole");
       	...
       	if (tobject.getType().equals("PerfectProcessQualityInspectionRequestTask") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
        ...
        /*     
	     ...
	     boolean alreadyExists=true;
		    ...
		    {SourceQualityDegree expectedOutputSourceQualityDegree=		    
		     new SourceQualityDegree(MentalStateManager.generateMentalEntityID());			
             to.add(new OutputEntity(expectedOutputSourceQualityDegree,TaskOperations.CreateWF));
            }
	     ...
	     tobject.addOutput(to);
     	      ...
     	      if (!allEntitiesExist){
				 ...
				 EventManager.getInstance().conversationalInitializationOfTaskFailed(
				 			getLocalName(), "PerfectSourceQualityInspector", 
												tobject, nonexisting);
	...
	public Vector tasksThatSatisfyGoal(String goalname){
         ...
         if (goalname.equals("GivingQualityDegreesToInfoSources")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new PerfectProcessQualityInspectionRequestTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal GivingQualityDegreesToInfoSources",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
					}
         ...
         if (goalname.equals("GivingQualityDegreesToInfoSources")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				boolean canbescheduled=false;
				for (int k=0;k<conversations.size();k++){
					tobject=new ProcessQualityInspectionRequestTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal GivingQualityDegreesToInfoSources",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("PerfectProcessQualityInspectionRequestTask");					
         
         
                  
         
                   
         ttypes.add("ProcessQualityInspectionRequestTask");					
   ...
   sg= new ingenias.editor.entities.StateGoal("GivingQualityDegreesToInfoSources");
   sg.setState("pending");
      try {
   ...
   java.awt.event.ActionListener ifPressed=null;
	...
	public DFAgentDescription[ ] getDescription() {
		...
		sd = new ServiceDescription();
		sd.setName(getLocalName() + "-sub-df");
		sd.setType("PerfectSourceQualityInspectorRole");
		sd.setOwnership("JADE");
		dfd.addServices(sd);
                playedRoles.add(dfd);
