...
public class AMIContextInstantiationRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("AMIContextInstantiation",
	index,"/rendererxml/AMIContextInstantiationUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("AMIContextInstantiation",
	index,"/rendererxml/AMIContextInstantiationINGENIASPanel.xml");
     ...
     */
    public AMIContextInstantiationRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("AMIContextInstantiation",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("AMIContextInstantiation",current);
    }
  ...
  public static void setEntity(AMIContextInstantiation ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("AMIContextInstantiation",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("AMIContextInstantiation",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("AMIContextInstantiation",current);
	}
