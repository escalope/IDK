...
public class UMLSpecificationRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	index,"/rendererxml/UMLSpecificationUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("UMLSpecification",
	index,"/rendererxml/UMLSpecificationINGENIASPanel.xml");
    ...
    public UMLSpecificationRenderer() {
	  ...
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("UMLSpecification",ent.getPrefs(map).getView());
  ...
  public static void setEntity(UMLSpecification ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("UMLSpecification",ent.getPrefs(attributes).getView());
