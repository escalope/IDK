...
public class FAERIECtxtModelInstRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("FAERIECtxtModelInst",
	index,"/rendererxml/FAERIECtxtModelInstUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("FAERIECtxtModelInst",
	index,"/rendererxml/FAERIECtxtModelInstINGENIASPanel.xml");
     ...
     */
    public FAERIECtxtModelInstRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("FAERIECtxtModelInst",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("FAERIECtxtModelInst",current);
    }
  ...
  public static void setEntity(FAERIECtxtModelInst ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("FAERIECtxtModelInst",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
        ...
        if (currentMap.get("EntityInstances")!=null && 
            currentMap.get("EntityInstances") instanceof ingenias.editor.rendererxml.CollectionPanel) {
          try {
            ( (ingenias.editor.rendererxml.CollectionPanel) currentMap.get(
                "EntityInstances")).setCollection("EntityInstances",ent.EntityInstances, ent.EntityInstances.getType());
          }
        ...
        if (currentMap.get("Model")!=null && 
            currentMap.get("Model") instanceof ingenias.editor.rendererxml.CollectionPanel) {
          try {
            ( (ingenias.editor.rendererxml.CollectionPanel) currentMap.get(
                "Model")).setCollection("Model",ent.Model, ent.Model.getType());
          }
          catch (IllegalArgumentException ex) {
            ex.printStackTrace();
          }
      ...
      if (currentMap.get("EntityInstances")!=null){
	   if (ent!=null && ent.getEntityInstances()!=null){
		if (currentMap.get("EntityInstances") instanceof javax.swing.JLabel){
		((javax.swing.JLabel)( currentMap).get("EntityInstances")).setText(ent.getEntityInstances().toString());
		} else {
		 if (currentMap.get("EntityInstances") instanceof javax.swing.text.JTextComponent)
		 ((javax.swing.text.JTextComponent)( currentMap).get("EntityInstances")).setText(ent.getEntityInstances().toString());
		
	      } 
	   } else  {
	     if (currentMap.get("EntityInstances") instanceof javax.swing.JLabel)
	     ((javax.swing.JLabel)( currentMap).get("EntityInstances")).setText("");
	     else {
		if (!(currentMap.get("EntityInstances") instanceof ingenias.editor.rendererxml.CollectionPanel)) 
		((javax.swing.text.JTextComponent)( currentMap).get("EntityInstances")).setText("");
	     }
         }
	  }
	...
	{
	return RenderComponentManager.retrievePanel("FAERIECtxtModelInst",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("FAERIECtxtModelInst",current);
	}
