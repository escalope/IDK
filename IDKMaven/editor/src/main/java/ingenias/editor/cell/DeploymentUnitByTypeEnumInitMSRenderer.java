...
public class DeploymentUnitByTypeEnumInitMSRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("DeploymentUnitByTypeEnumInitMS",
	index,"/rendererxml/DeploymentUnitByTypeEnumInitMSUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("DeploymentUnitByTypeEnumInitMS",
	index,"/rendererxml/DeploymentUnitByTypeEnumInitMSINGENIASPanel.xml");
     ...
     */
    public DeploymentUnitByTypeEnumInitMSRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("DeploymentUnitByTypeEnumInitMS",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("DeploymentUnitByTypeEnumInitMS",current);
    }
  ...
  public static void setEntity(DeploymentUnitByTypeEnumInitMS ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("DeploymentUnitByTypeEnumInitMS",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
        ...
        if (currentMap.get("InitialState")!=null && 
            currentMap.get("InitialState") instanceof ingenias.editor.rendererxml.CollectionPanel) {
          try {
            ( (ingenias.editor.rendererxml.CollectionPanel) currentMap.get(
                "InitialState")).setCollection("InitialState",ent.InitialState, ent.InitialState.getType());
          }
	...
	{
	return RenderComponentManager.retrievePanel("DeploymentUnitByTypeEnumInitMS",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("DeploymentUnitByTypeEnumInitMS",current);
	}
