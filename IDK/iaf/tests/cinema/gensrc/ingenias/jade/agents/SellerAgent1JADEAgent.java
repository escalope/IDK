...
public class SellerAgent1JADEAgent
		 extends JADEAgent {         
 
		 public SellerAgent1JADEAgent(){
		 super(new SellerAgent1Protocol(),new SellerAgent1InteractionLocks());
		 }
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	...
  	   	boolean correctRole=conversation.getPlayedRole().equals ("Seller");
       	...
       	if (tobject.getType().equals("Determine_availability") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
		...
		if (validConversationType){
       	...
       	if (tobject.getType().equals("GetMoney") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
