...
import ingenias.exception.NotFound;
  ...
  public class SourcesAlphaQualityInspectorRoleAlphaQualityInspectionRequestDefaultCommControl extends DefaultCommControl{
  ...
  public SourcesAlphaQualityInspectorRoleAlphaQualityInspectionRequestDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("QualityDegreeOfSourceInTesting");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("QualityDegreeOfSourceInTesting");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("AlphaQualityDegreeIU") &&
    	 sb.isState("AlphaQualityDegreeIU")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"QualityDegreeOfSourceInTesting").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("AlphaQualityDegreeIU");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"QualityDegreeOfSourceInTesting");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("QualityDegreeOfSourceInTesting");
	       lockProcessed("QualityDegreeOfSourceInTesting");
           //MainInteractionManager.log("Removing lock QualityDegreeOfSourceInTesting",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("endAlphaQualityDegreeIU");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for AlphaQualityInspectionRequestIU")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("AlphaQualityInspectionRequestIU")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for AlphaQualityInspectionRequestIU");           
    	   //try {
		   ...
		   futureStates.add("AlphaQualityDegreeIU");
