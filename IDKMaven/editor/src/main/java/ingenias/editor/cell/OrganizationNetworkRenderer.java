...
public class OrganizationNetworkRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("OrganizationNetwork",
	index,"/rendererxml/OrganizationNetworkUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("OrganizationNetwork",
	index,"/rendererxml/OrganizationNetworkINGENIASPanel.xml");
     ...
     */
    public OrganizationNetworkRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("OrganizationNetwork",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("OrganizationNetwork",current);
    }
  ...
  public static void setEntity(OrganizationNetwork ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("OrganizationNetwork",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("OrganizationNetwork",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("OrganizationNetwork",current);
	}
