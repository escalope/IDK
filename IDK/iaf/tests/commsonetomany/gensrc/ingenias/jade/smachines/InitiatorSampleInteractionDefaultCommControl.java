...
import ingenias.exception.NotFound;
  ...
  public class InitiatorSampleInteractionDefaultCommControl extends DefaultCommControl{
  ...
  public InitiatorSampleInteractionDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("Request");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("InteractionUnit0") &&
		 ...
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Request").isEmpty();
         ...
         if (allexist && true){
           ...
           sb.removeState("InteractionUnit0");
	   	   ...
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Request");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("Request");
	       lockProcessed("Request");
           //MainInteractionManager.log("Removing lock Request",this.getAgent().getLocalName()+"-"+sb.getConversationID());
