...
private MentalStateReader msr=null;
  public Role1Interaction0StateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      // Next states after receiving "InteractionUnit0"
			       ...
			       smf.add("waiting for InteractionUnit0","InteractionUnit2");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("InteractionUnit2", "endInteractionUnit2");
  ...
  if (this.isState("InteractionUnit0") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if ("1".equals("n")){
         try{
          Vector<AID> receivers=this.getActor("Role0");    
  ...
  if (this.isState("InteractionUnit2")) {
     ...
     try {
      ...
      {      
       Vector<AID> receivers=this.getActor("Role2");      
      ...
      options.add("endInteractionUnit2");      
      
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("InteractionUnit2",optionsA,this)){
           ...
           "InteractionUnit2","Interaction0",this.getContentForNextMessage(), this.getPlayedRole());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("InteractionUnit2", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("InteractionUnit2", "ABORTED");
    	  } else  {
  ...
  if (this.isState("endInteractionUnit2")) {
