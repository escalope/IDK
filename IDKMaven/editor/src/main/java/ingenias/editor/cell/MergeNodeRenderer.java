...
public class MergeNodeRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("MergeNode",
	index,"/rendererxml/MergeNodeUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("MergeNode",
	index,"/rendererxml/MergeNodeINGENIASPanel.xml");
    ...
    public MergeNodeRenderer() {
	  ...
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("MergeNode",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("MergeNode",current);
    }
  ...
  public static void setEntity(MergeNode ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("MergeNode",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("MergeNode",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("MergeNode",current);
	}
