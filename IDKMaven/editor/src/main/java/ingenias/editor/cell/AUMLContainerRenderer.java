...
public class AUMLContainerRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("AUMLContainer",
	index,"/rendererxml/AUMLContainerUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("AUMLContainer",
	index,"/rendererxml/AUMLContainerINGENIASPanel.xml");
     ...
     */
    public AUMLContainerRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("AUMLContainer",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("AUMLContainer",current);
    }
  ...
  public static void setEntity(AUMLContainer ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("AUMLContainer",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
        ...
        if (currentMap.get("Children")!=null && 
            currentMap.get("Children") instanceof ingenias.editor.rendererxml.CollectionPanel) {
          try {
            ( (ingenias.editor.rendererxml.CollectionPanel) currentMap.get(
                "Children")).setCollection("Children",ent.Children, ent.Children.getType());
          }
	...
	{
	return RenderComponentManager.retrievePanel("AUMLContainer",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("AUMLContainer",current);
	}
