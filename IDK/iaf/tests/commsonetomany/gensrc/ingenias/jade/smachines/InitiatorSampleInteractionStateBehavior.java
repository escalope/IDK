...
public class InitiatorSampleInteractionStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public InitiatorSampleInteractionStateBehavior( String agentName,
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
      ...
      actorsv.addAll(this.getActor("Colaborator"));
      {      
       Vector<AID> receivers=this.getActor("Colaborator");      
       for (AID aid:receivers){
        rolesv.add("Colaborator");
       }
      }
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","SampleInteraction",completelistInvolvedActors);
    ...
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                   ...
                   "SampleInteraction",this.getPlayedRole(),
     ...
     try {
      ...
      if (this.getDCC().notifyMessageSent("InteractionUnit0",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "InteractionUnit0","SampleInteraction",this.getContentForNextMessage());
           getTimeout().stop();
