...
public class JoinNodeRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("JoinNode",
	index,"/rendererxml/JoinNodeUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("JoinNode",
	index,"/rendererxml/JoinNodeINGENIASPanel.xml");
     ...
     */
    public JoinNodeRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("JoinNode",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("JoinNode",current);
    }
  ...
  public static void setEntity(JoinNode ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("JoinNode",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("JoinNode",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("JoinNode",current);
	}
