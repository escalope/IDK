...
public class WFCancelsEdge extends NAryEdge {

  // Constructor with data content
  public WFCancelsEdge(ingenias.editor.entities.WFCancels userObject) {
    super(userObject);
...
this.addClass("WFUsessource","ingenias.editor.entities.MentalState");
     ...
     this.addRole("WFCancelstarget");
     this.setArity("WFCancelstarget",true,1);
     this.setArity("WFCancelstarget",false,2147483647);
     
this.addClass("WFCancelstarget","ingenias.editor.entities.Conversation");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFCancelsEdge)))
      return false;

    // n-edge to be considered.
    WFCancelsEdge edge = new WFCancelsEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof WFCancelsEdge)
      edge = (WFCancelsEdge) selectedEdge;
