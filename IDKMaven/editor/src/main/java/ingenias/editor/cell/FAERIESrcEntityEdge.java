...
public class FAERIESrcEntityEdge extends NAryEdge {

  // Constructor with data content
  public FAERIESrcEntityEdge(ingenias.editor.entities.FAERIESrcEntity userObject) {
    super(userObject);
    
     this.addRole("FAERIESrcEntitysource");
     this.setArity("FAERIESrcEntitysource",true,1);
     this.setArity("FAERIESrcEntitysource",false,1);
     
this.addClass("FAERIESrcEntitysource","ingenias.editor.entities.FAERIECtxtRelationship");
     
this.addClass("FAERIESrcEntitysource","ingenias.editor.entities.FAERIECtxtEntity");
     
    
     this.addRole("FAERIESrcEntitytarget");
     this.setArity("FAERIESrcEntitytarget",true,1);
     this.setArity("FAERIESrcEntitytarget",false,2147483647);
     
this.addClass("FAERIESrcEntitytarget","ingenias.editor.entities.FAERIECtxtEntity");
     
this.addClass("FAERIESrcEntitytarget","ingenias.editor.entities.FAERIECtxtRelationship");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof FAERIESrcEntityEdge)))
      return false;

    // n-edge to be considered.
    FAERIESrcEntityEdge edge = new FAERIESrcEntityEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof FAERIESrcEntityEdge)
      edge = (FAERIESrcEntityEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("FAERIESrcEntitysource")){
 	 edges.add( new DefaultEdge( new FAERIESrcEntitysourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("FAERIESrcEntitytarget")){
 	 edges.add( new DefaultEdge( new FAERIESrcEntitytargetRole() ) );
	 }
