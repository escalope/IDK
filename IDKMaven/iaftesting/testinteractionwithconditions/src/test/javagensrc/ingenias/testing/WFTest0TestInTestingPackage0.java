...
public class WFTest0TestInTestingPackage0 extends TestingPackage0ForWFTest0 {
  ...
  public void testDemo() throws TimeOut{		
        	...
        	// wait for Agent1_0DeploymentUnitByType1 to initialise
        	MentalStateManager msmAgent1_0DeploymentUnitByType1=MSMRepository.getInstance().waitFor("Agent1_0DeploymentUnitByType1");
			...
			ga.addStateTransition("WFTestInitialState0","Agent1_0DeploymentUnitByType1-Task1","WFTestFinalState0");
        	...
        	tev.registerTask("Agent1_0DeploymentUnitByType1","Task1");
		...
		long finishedTime=0;
		long duration=5000;
		long maxtimepercycle=5000;
		
		finishedTime=Math.max(finishedTime,5000);
		...
		while (currentTime<finishedTime){
				...
				if (currentTime%1000 ==0 && currentTime<5000)
					try{
					 msmAgent0_0DeploymentUnitByType0.addMentalEntity(new ApplicationEventSlots0(MentalStateManager.generateMentalEntityID()));
