...
public class WFDecomposesWFEdge extends NAryEdge {

  // Constructor with data content
  public WFDecomposesWFEdge(ingenias.editor.entities.WFDecomposesWF userObject) {
    super(userObject);
    
     this.addRole("WFDecomposesWFsource");
     this.setArity("WFDecomposesWFsource",true,1);
     this.setArity("WFDecomposesWFsource",false,1);
     
this.addClass("WFDecomposesWFsource","ingenias.editor.entities.Workflow");
     
    
     this.addRole("WFDecomposesWFtarget");
     this.setArity("WFDecomposesWFtarget",true,1);
     this.setArity("WFDecomposesWFtarget",false,2147483647);
     
this.addClass("WFDecomposesWFtarget","ingenias.editor.entities.Task");
     
this.addClass("WFDecomposesWFtarget","ingenias.editor.entities.Workflow");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFDecomposesWFEdge)))
      return false;

    // n-edge to be considered.
    WFDecomposesWFEdge edge = new WFDecomposesWFEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof WFDecomposesWFEdge)
      edge = (WFDecomposesWFEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("WFDecomposesWFsource")){
 	 edges.add( new DefaultEdge( new WFDecomposesWFsourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("WFDecomposesWFtarget")){
 	 edges.add( new DefaultEdge( new WFDecomposesWFtargetRole() ) );
	 }
