...
public class WFFollowsGuardedEdge extends NAryEdge {

  // Constructor with data content
  public WFFollowsGuardedEdge(ingenias.editor.entities.WFFollowsGuarded userObject) {
    super(userObject);
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFFollowsGuardedEdge)))
      return false;

    // n-edge to be considered.
    WFFollowsGuardedEdge edge = new WFFollowsGuardedEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof WFFollowsGuardedEdge)
      edge = (WFFollowsGuardedEdge) selectedEdge;
