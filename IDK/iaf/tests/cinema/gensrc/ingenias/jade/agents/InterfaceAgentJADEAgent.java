	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		boolean allEntitiesExist=true;
	      ...
	      allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
             ...
             expectedApp=(ingenias.jade.components.Application)getAM().getApplication("SimulationKernel");
             tobject.addApplication("SimulationKernel",expectedApp);
		...
		if (validConversationType){
       	...
       	if (tobject.getType().equals("Process_ticket_obtained") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
		...
		if (validConversationType){
       	...
       	if (tobject.getType().equals("Process_Buyer_Failure") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
