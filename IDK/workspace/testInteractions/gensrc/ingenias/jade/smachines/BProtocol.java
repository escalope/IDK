...
public class BProtocol implements AgentProtocols{
	...
	public ActiveConversation initialiseProtocols(String agentName,RuntimeConversation conv, 
        ...
        // Adds a protocol to the protocol queue
          StateBehavior   sb=null;
		   ...
		   if (conv.getInteraction().getId().equals("IntExample") && conv.getPlayedRole().equals("Participant1")){
		   	DefaultCommControl dcc=new Participant1IntExampleDefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new Participant1IntExampleStateBehavior(agentName,
				msr, msu,
				conv,                           		
				conv.getPlayedRole(),
			   	actors,
