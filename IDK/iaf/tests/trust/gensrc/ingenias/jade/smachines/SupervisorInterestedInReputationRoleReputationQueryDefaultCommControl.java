...
import ingenias.exception.NotFound;
  ...
  public class SupervisorInterestedInReputationRoleReputationQueryDefaultCommControl extends DefaultCommControl{
  ...
  public SupervisorInterestedInReputationRoleReputationQueryDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("AgentReputationQuery");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("AgentReputationQuery");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("ReputationQueryIU") &&
    	 sb.isState("ReputationQueryIU")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"AgentReputationQuery").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("ReputationQueryIU");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"AgentReputationQuery");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("AgentReputationQuery");
	       lockProcessed("AgentReputationQuery");
           //MainInteractionManager.log("Removing lock AgentReputationQuery",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("ReputationQueryResponse");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for ReputationQueryResponse")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("ReputationQueryResponse")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for ReputationQueryResponse");           
    	   //try {
		   ...
		   futureStates.add("endReputationQueryResponse");
