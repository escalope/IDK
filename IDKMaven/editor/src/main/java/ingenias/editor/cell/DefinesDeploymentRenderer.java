...
public class DefinesDeploymentRenderer extends VertexRenderer implements CellViewRenderer{
...
private static ViewPreferences.ViewType current = ViewPreferences.ViewType.INGENIAS;
  ...
  static {
   try {
	...
	RenderComponentManager.loadRenderFile("DefinesDeployment",
	index,"/rendererxml/DefinesDeploymentNOICONPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("DefinesDeployment",
	index,"/rendererxml/DefinesDeploymentINGENIASPanel.xml");
	
	index=ViewPreferences.ViewType.LABEL;
	RenderComponentManager.loadRenderFile("DefinesDeployment",
	index,"/rendererxml/DefinesDeploymentLABELPanel.xml");
     ...
     */
     public DefinesDeploymentRenderer() {
    }

    public Dimension getSize() {
    return RenderComponentManager.getSize("DefinesDeployment",current);
    }
 ...
 public static void setEntity(ingenias.editor.entities.DefinesDeployment ent, Map attributes){
 Map currentMap=(Map)  RenderComponentManager.retrieveIDs("DefinesDeployment",ent.getPrefs(attributes).getView());
current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("DefinesDeployment",this.current);
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("DefinesDeployment",c);
	}
