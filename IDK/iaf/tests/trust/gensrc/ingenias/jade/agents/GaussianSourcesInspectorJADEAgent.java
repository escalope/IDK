...
public class GaussianSourcesInspectorJADEAgent
		 extends JADEAgent {         
 
		 public GaussianSourcesInspectorJADEAgent(){
		 super(new GaussianSourcesInspectorProtocol(),new GaussianSourcesInspectorInteractionLocks());
		 }
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	...
  	   	boolean correctRole=conversation.getPlayedRole().equals ("GaussianSourceQualityInspectorRole");
       	...
       	if (tobject.getType().equals("GaussianProcessQualityInspectionRequestTask") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
	     ...
	     tobject.addOutput(to);
     	      ...
     	      if (!allEntitiesExist){
				 ...
				 EventManager.getInstance().conversationalInitializationOfTaskFailed(
				 			getLocalName(), "GaussianSourcesInspector", 
												tobject, nonexisting);
         ...
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new GaussianProcessQualityInspectionRequestTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("GaussianProcessQualityInspectionRequestTask");					
   ...
   java.awt.event.ActionListener ifPressed=null;
	...
	public DFAgentDescription[ ] getDescription() {
		...
		sd = new ServiceDescription();
		sd.setName(getLocalName() + "-sub-df");
		sd.setType("GaussianSourceQualityInspectorRole");
		sd.setOwnership("JADE");
		dfd.addServices(sd);
                playedRoles.add(dfd);
