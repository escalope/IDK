...
public class ODecomposesWFEdge extends NAryEdge {

  // Constructor with data content
  public ODecomposesWFEdge(ingenias.editor.entities.ODecomposesWF userObject) {
    super(userObject);
    
     this.addRole("ODecomposesWFsource");
     this.setArity("ODecomposesWFsource",true,1);
     this.setArity("ODecomposesWFsource",false,1);
     
this.addClass("ODecomposesWFsource","ingenias.editor.entities.Workflow");
     
    
     this.addRole("ODecomposesWFtarget");
     this.setArity("ODecomposesWFtarget",true,1);
     this.setArity("ODecomposesWFtarget",false,2147483647);
     
this.addClass("ODecomposesWFtarget","ingenias.editor.entities.Workflow");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof ODecomposesWFEdge)))
      return false;

    // n-edge to be considered.
    ODecomposesWFEdge edge = new ODecomposesWFEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof ODecomposesWFEdge)
      edge = (ODecomposesWFEdge) selectedEdge;
