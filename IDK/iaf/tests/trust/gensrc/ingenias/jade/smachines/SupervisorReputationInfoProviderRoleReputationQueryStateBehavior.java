...
public class SupervisorReputationInfoProviderRoleReputationQueryStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public SupervisorReputationInfoProviderRoleReputationQueryStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","ReputationQueryIU");
			    
			    
			    
			      // Receiving a message    
			      smf.add("ReputationQueryIU", "waiting for ReputationQueryIU");
			      
			      // Next states after receiving "ReputationQueryIU"
			      
			       smf.add("waiting for ReputationQueryIU","ReputationQueryResponse");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("ReputationQueryResponse", "endReputationQueryResponse");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "ReputationQuery","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("ReputationQueryIU") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"ReputationQueryIU",
                   "ReputationQuery",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("ReputationQueryIU");
      this.addState("waiting for ReputationQueryIU");
      this.notifyStateTransitionExecuted("ReputationQueryIU", "waiting for ReputationQueryIU");
  } 
  ...
  if (this.isState("ReputationQueryResponse")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("ReputationQueryResponse",optionsA,this)){
           ...
           "ReputationQueryResponse","ReputationQuery",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("ReputationQueryResponse", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("ReputationQueryResponse", "ABORTED");
    	  } else  {
