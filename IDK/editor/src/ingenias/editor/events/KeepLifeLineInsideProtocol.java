 ...
 *
 */
public class KeepLifeLineInsideProtocol
  ...
  private void movedProtocol(ProtocolCell pc,
                             org.jgraph.event.GraphModelEvent gme) {
    Map previousLoc = (Map) gme.getChange().getPreviousAttributes().
        get(pc);

    Rectangle lastpos = GraphConstants.getBounds(previousLoc).
        getBounds();
    Rectangle currentpos = GraphConstants.getBounds( (Map) gme.
        getChange().getAttributes().get(pc)).getBounds();
    Protocol prot = (Protocol) pc.getUserObject();
    Enumeration enumeration = prot.getChildrenElements();
    ingenias.editor.Model model = (ingenias.editor.Model) gme.
        getSource();
    ...
    while (enumeration.hasMoreElements()) {
      ...
      if (nextElem instanceof Lifeline) {
        ...
        Rectangle llclastpos = GraphConstants.getBounds(att).
            getBounds();
        Point newpoint = new Point(llclastpos.x -
                                   (currentpos.x - lastpos.x),
                                   llclastpos.y -
                                   (currentpos.y - lastpos.y));
        Rectangle npos = new Rectangle(newpoint, llclastpos.getSize());
        /*    System.err.println("movin cell " + ll.getId() + " from " +
                               llclastpos + " to " + npos);*/
  ...
  private void movedLifeline(LifelineCell lc,
                             org.jgraph.event.GraphModelEvent gme) {
    ingenias.editor.Model model = (ingenias.editor.Model) gme.
        getSource();
    Map protocolMap = (Map)this.locateProtocolCell( (Lifeline) lc.getUserObject(),
        (ingenias.editor.Model) gme.getSource()).getAttributes();
    Rectangle lloldpos = GraphConstants.getBounds( (Map) gme.getChange().
                                                  getAttributes().get(lc)).
        getBounds();
    Rectangle llcurrentpos = GraphConstants.getBounds(lc.getAttributes()).
        getBounds();
    Rectangle protpos = GraphConstants.getBounds(protocolMap).getBounds();
//        System.err.println("detected lifeline "+llcurrentpos+" :"+lloldpos+" :prot "+protpos+ " "+protpos.union(llcurrentpos));
    if (!protpos.union(llcurrentpos).equals(protpos)) {
      ...
      m.put(lc, nmap);
      model.edit(m, null, null, null);
      this.moveLifeline(lc, llcurrentpos.getLocation(), lloldpos.getLocation(),
                        model);
    }
  }

  private void moveLifeline(LifelineCell lc, Point dest, Point orig,
                            ingenias.editor.Model model) {
    ...
    while (enumeration.hasMoreElements()) {
      ...
      if (nextElement instanceof Column) {
        ...
        npos.x = npos.x + despx;
        npos.y = npos.y + despy;
        /*        Rectangle npos = new Rectangle(bounds.x +
                                               (bounds.width / 2) - 15,
                                               previousy,
                                               0, 0);*/
        npos.width = 30;
        ...
        while (ports.hasMoreElements()) {
          ...
          if (portcell == null) {
            ...
            Rectangle nppos = new Rectangle(oldpos.x + despx,
                                            oldpos.y + despy,
                                            30, 10);
            //npos.height = npos.height + 30;
            GraphConstants.setBounds(portmap, nppos);
            elems.put(portcell, portmap);
          }
        }
//        previousy = previousy + npos.height;
    ...
    if (enabled && this.workingObject == null &&
      ...
      if (objs != null) {
        for (int k = 0; k < objs.length; k++) {
          if (objs[k] != null) {
            if (objs[k] instanceof ProtocolCell &&
                gme.getChange().getPreviousAttributes() != null &&
                gme.getChange().getPreviousAttributes().get(objs[k]) != null) {
              this.movedProtocol( (ProtocolCell) objs[k], gme);
              movedProtocols.add(objs[k]);
            }
            if (objs[k] instanceof LifelineCell &&
                gme.getChange().getPreviousAttributes() != null &&
                gme.getChange().getPreviousAttributes().get(objs[k]) != null) {
              ProtocolCell pc = this.locateProtocolCell( (Lifeline) ( (
                  LifelineCell) objs[k]).getUserObject(),
                  (ingenias.editor.Model) gme.getSource());
