...
public class RemoteProcedureCallRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("RemoteProcedureCall",
	index,"/rendererxml/RemoteProcedureCallUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("RemoteProcedureCall",
	index,"/rendererxml/RemoteProcedureCallINGENIASPanel.xml");
     ...
     */
    public RemoteProcedureCallRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("RemoteProcedureCall",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("RemoteProcedureCall",current);
    }
  ...
  public static void setEntity(RemoteProcedureCall ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("RemoteProcedureCall",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("RemoteProcedureCall",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("RemoteProcedureCall",current);
	}
