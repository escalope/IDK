...
public class SupervisorInterestedInReputationRoleReputationQueryStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public SupervisorInterestedInReputationRoleReputationQueryStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "ReputationQueryIU");
			    
			    
			      // Receiving a message    
			      smf.add("ReputationQueryResponse", "waiting for ReputationQueryResponse");
			      
			      // Next states after receiving "ReputationQueryResponse"
			      
			       smf.add("waiting for ReputationQueryResponse","endReputationQueryResponse");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("ReputationQueryIU", "ReputationQueryResponse");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
  	try {  	  
      String[] options=new String[]{"ReputationQueryIU"};
      AID[] actors=null;
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","ReputationQuery",completelistInvolvedActors);
  ...
  if (this.isState("ReputationQueryResponse") ) {  // State changed by other agent and upted
    //in the parent class
         
      Vector options=new Vector();
      
      options.add("endReputationQueryResponse");
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"ReputationQueryResponse",
                   "ReputationQuery",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("ReputationQueryResponse");
      this.addState("waiting for ReputationQueryResponse");
      this.notifyStateTransitionExecuted("ReputationQueryResponse", "waiting for ReputationQueryResponse");
  } 
  ...
  if (this.isState("ReputationQueryIU")) {
     ...
     try {
      ...
      options.add("ReputationQueryResponse");      
      
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("ReputationQueryIU",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "ReputationQueryIU","ReputationQuery",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("ReputationQueryIU", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("ReputationQueryIU", "ABORTED");
    	  } else  {
  ...
  if (this.isState("endReputationQueryResponse")) {
