...
public class BasicGraphUI extends GraphUI implements Serializable {
	...
	public static final boolean DNDPREVIEW = System.getProperty("java.version")
			...
			private Vector<DefaultGraphCell> highlighted = new Vector<DefaultGraphCell>();
			...
			public void paintCell(Graphics g, CellView view, Rectangle2D bounds,
				...
				if (view != null && bounds != null) {
					...
					Component component = view.getRendererComponent(graph, sel, bfocus,
							preview);
					Stroke oldstr = ((Graphics2D) g).getStroke();
					Stroke currentStroke = oldstr;
					boolean found = false;
					Vector<DefaultGraphCell> toPaint = new Vector<DefaultGraphCell>();
					for (Object selected : graph.getSelectionCells()) {
						if (selected instanceof DefaultGraphCell)
							toPaint.add((DefaultGraphCell) selected);
					}
					toPaint.addAll(highlighted);

					for (int k = 0; k < toPaint.size(); k++) {
						if (view.getCell() instanceof DefaultEdge) {
							found = found
									|| ((DefaultPort) (((DefaultEdge) view.getCell())
											.getTarget())).getParent().equals(
													toPaint.elementAt(k))
													|| ((DefaultPort) (((DefaultEdge) view.getCell())
															.getSource())).getParent().equals(
																	toPaint.elementAt(k));
							if (!found) {

								if (((DefaultPort) (((DefaultEdge) view.getCell())
										.getTarget())).getParent() != null
										&& nAryEdge
										.isAssignableFrom(((DefaultPort) (((DefaultEdge) view
												.getCell()).getTarget()))
												.getParent().getClass())) {

									DefaultGraphCell dgcevaluatewith = (DefaultGraphCell) ((DefaultPort) (((DefaultEdge) view
											.getCell()).getTarget())).getParent();
									found = found
											|| evaluateIfDefaultGraphCellHasToBeHighlighted(
													dgcevaluatewith, found,
													toPaint.elementAt(k));
								} else {
									if (((DefaultPort) (((DefaultEdge) view.getCell())
							...
							if (view.getCell() instanceof DefaultGraphCell) {
								found = evaluateIfDefaultGraphCellHasToBeHighlighted(
										(DefaultGraphCell) (view.getCell()), found,
										toPaint.elementAt(k));
							}
						}

						found = found || toPaint.elementAt(k).equals(view.getCell());
					}
					...
					if (found) {
						...
						// ((Graphics2D)g).setFont(oldfont.deriveFont(20));
						// setFontRecursive(new Component[]{component},
						// oldfont.deriveFont(Font.BOLD));

					}

					rendererPane.paintComponent(g, component, graph,
							(int) bounds.getX(), (int) bounds.getY(),
							(int) bounds.getWidth(), (int) bounds.getHeight(), true);

					// restore stroke and font
					((Graphics2D) g).setStroke(oldstr);
					// ((Graphics2D)g).setFont(oldfont);
					// component.setFont(oldfont);
					setFontRecursive(new Component[] { component }, oldfont);
					// continue painting
					Object cell = view.getCell();
					if (cell instanceof DefaultGraphCell) {
						DefaultGraphCell currentCell = (DefaultGraphCell) cell;
						Hashtable<String, Rectangle> subcomponentBounds = new Hashtable<String, Rectangle>();
						obtainSubComponentBounds(subcomponentBounds, component);
						if (this.graph.getModel().getAttributes(currentCell) != null) {
							this.graph.getModel().getAttributes(currentCell)
							.put("subcomponentbounds", subcomponentBounds);
						}
					}
				}
				// Then Paint Children
			...
			private boolean evaluateIfDefaultGraphCellHasToBeHighlighted(
					DefaultGraphCell dgc, boolean found, DefaultGraphCell toPaint) {
				if (dgc.getChildCount() > 0) {
					for (Object objectport : dgc.getChildren()) {
						if (objectport instanceof DefaultPort) {
							DefaultPort port = (DefaultPort) objectport;
							Iterator<DefaultEdge> edges = graph.getModel().edges(port);
							while (edges.hasNext() && !found) {
								DefaultEdge next = edges.next();
								found = found
										|| ((DefaultPort) (next.getTarget()))
										.getParent().equals(toPaint)
										|| ((DefaultPort) (next.getSource()))
										.getParent().equals(toPaint);
							}
						}
					}
			...
			private void obtainSubComponentBounds(
					Hashtable<String, Rectangle> subcomponentBounds, Component component) {
				if (component.getName() != null)
					subcomponentBounds.put(component.getName(), component.getBounds());
				if (component instanceof Container) {
					Container componentPanel = (Container) component;
					for (Component subcomp : componentPanel.getComponents()) {
						obtainSubComponentBounds(subcomponentBounds, subcomp);
					}
				}
			...
			protected void paintBackgroundComponent(Graphics g, Component component) {
				try {
					...
					rendererPane.paintComponent(g, component, graph, 0, 0,
							(int) dim.getWidth(), (int) dim.getHeight(), true);
				} catch (Exception e) {
				} catch (Error e) {
				}
			}
				...
				if (ports != null) {
					...
					for (int i = 0; i < ports.length; i++) {
						if (ports[i] != null) {
							...
							Component parentRenderer = ports[i].getParentView()
									.getRendererComponent(graph, false, false, false);
							boolean withoutports = false;
							try {
								if (parentRenderer != null
										&& defaultPanel.isAssignableFrom(parentRenderer
												.getClass())
												&& withoutPortsPanel
												.isAssignableFrom(parentRenderer
														.getClass()
														.getMethod("getPanelInside",
																new Class[] {})
																.invoke(parentRenderer,
																		new Object[] {})
																		.getClass()))
									withoutports = true;
							...
							Rectangle2D bounds = new Rectangle2D.Double(tmp.getX(),
			...
			protected void updateCachedPreferredSize() {
				...
				Point2D min = (d != null) ? graph
						...
						preferredSize = new Dimension(
								(int) Math.max(min.getX(), scaled.getX()), (int) Math.max(
										min.getY(), scaled.getY()));
						...
						if (image != null) {
							...
							preferredSize = new Dimension((int) Math.max(
									preferredSize.getWidth(), imageSize.getX()),
									(int) Math.max(preferredSize.getHeight(), imageSize.getY()));
						}
			...
			protected Point2D getEditorLocation(Object cell, Dimension2D editorSize,
				...
				if (view instanceof EdgeView) {
					...
					if (renderer instanceof EdgeRenderer) {
						...
						pt.setLocation(
								Math.max(0, pt.getX() - editorSize.getWidth() / 2),
			...
			public class GraphModelHandler implements GraphModelListener, Serializable {
				...
				public GraphModelHandler(JGraph graph) {
					this.graph=graph;
					graphLayoutCache=this.graph.getGraphLayoutCache();
				}
					...
					try {
						...
						if (changed != null && changed.length > 0) {
							for (int i = 0; i < changed.length; i++) {
								if (graph != null && graphLayoutCache != null)
									graph.updateAutoSize(graphLayoutCache.getMapping(
											changed[i], false));
							}
						}
						if (dirtyRegion == null) {
							Rectangle2D newDirtyRegion = graph.getClipRectangle(e
									.getChange());
							dirtyRegion = RectUtils.union(oldDirty, newDirtyRegion);
							e.getChange().setDirtyRegion(dirtyRegion);
						}

						if (dirtyRegion != null && graph != null) {
			...
			Serializable {
				...
				public GraphLayoutCacheHandler(JGraph graph){
					this.graph=graph;
				 ...
				 * 
				 * @see
				 * org.jgraph.event.GraphLayoutCacheListener#graphLayoutCacheChanged
			...
			public class KeyHandler extends KeyAdapter implements Serializable {
				...
				public void keyPressed(KeyEvent e) {
					if (graph != null && graph.hasFocus() && graph.isEnabled()) {
						KeyStroke keyStroke = KeyStroke.getKeyStroke(e.getKeyCode(),
								e.getModifiers());
							...
							if (activeHandle != null) {
								...
								if (event.isControlDown() && graph.isCloneable()) { // Clone
									...
									cells = graphLayoutCache.insertClones(cells,
											graph.cloneCells(cells), attributes, cs,
											pm, 0, 0);
