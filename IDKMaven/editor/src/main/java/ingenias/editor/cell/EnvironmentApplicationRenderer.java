...
public class EnvironmentApplicationRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("EnvironmentApplication",
	index,"/rendererxml/EnvironmentApplicationUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("EnvironmentApplication",
	index,"/rendererxml/EnvironmentApplicationINGENIASPanel.xml");
     ...
     */
    public EnvironmentApplicationRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("EnvironmentApplication",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("EnvironmentApplication",current);
    }
  ...
  public static void setEntity(EnvironmentApplication ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("EnvironmentApplication",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("EnvironmentApplication",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("EnvironmentApplication",current);
	}
