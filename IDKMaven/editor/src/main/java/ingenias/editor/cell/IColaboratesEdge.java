...
public class IColaboratesEdge extends NAryEdge {

  // Constructor with data content
  public IColaboratesEdge(ingenias.editor.entities.IColaborates userObject) {
    super(userObject);
    
     this.addRole("IColaboratessource");
     this.setArity("IColaboratessource",true,1);
     this.setArity("IColaboratessource",false,1);
     
this.addClass("IColaboratessource","ingenias.editor.entities.Interaction");
     
    
     this.addRole("IColaboratestarget");
     this.setArity("IColaboratestarget",true,1);
     this.setArity("IColaboratestarget",false,2147483647);
     
this.addClass("IColaboratestarget","ingenias.editor.entities.Agent");
     
this.addClass("IColaboratestarget","ingenias.editor.entities.Role");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof IColaboratesEdge)))
      return false;

    // n-edge to be considered.
    IColaboratesEdge edge = new IColaboratesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof IColaboratesEdge)
      edge = (IColaboratesEdge) selectedEdge;
