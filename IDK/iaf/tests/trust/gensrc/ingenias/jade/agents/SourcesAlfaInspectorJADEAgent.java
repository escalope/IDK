...
public class SourcesAlfaInspectorJADEAgent
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
		...
		expectedInput=this.getMSM().getMentalEntityByType("GradoCalidadFuenteCuarentena");
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
			    ...
			    addExpectedInputs(tobject, "GradoCalidadFuenteCuarentena","1",expectedInput);
             	addConsumedInput(to,"GradoCalidadFuenteCuarentena",expectedInput);
			 }
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	...
  	   	boolean correctRole=conversation.getPlayedRole().equals ("SourcesAlfaQualityInspectorRole");
       	...
       	if (tobject.getType().equals("ProcesarSolicitudInspeccionAlfaFuenteTask") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
        ...
        /*     
	     ...
	     boolean alreadyExists=true;
		    ...
		    {GradoCalidadFuenteCuarentena expectedOutputGradoCalidadFuenteCuarentena=		    
		     new GradoCalidadFuenteCuarentena(MentalStateManager.generateMentalEntityID());			
             to.add(new OutputEntity(expectedOutputGradoCalidadFuenteCuarentena,TaskOperations.CreateWF));
            }
	...
	public Vector tasksThatSatisfyGoal(String goalname){
         ...
         if (goalname.equals("DarServicioCalificacionAlfaCalidadFuentes")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new ProcesarSolicitudInspeccionAlfaFuenteTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal DarServicioCalificacionAlfaCalidadFuentes",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
					}
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("ProcesarSolicitudInspeccionAlfaFuenteTask");					
   ...
   sg= new ingenias.editor.entities.StateGoal("DarServicioCalificacionAlfaCalidadFuentes");
   sg.setState("pending");
      try {
   ...
   java.awt.event.ActionListener ifPressed=null;
	...
	public DFAgentDescription[ ] getDescription() {
		...
		sd.setType("SourcesAlfaQualityInspectorRole");
