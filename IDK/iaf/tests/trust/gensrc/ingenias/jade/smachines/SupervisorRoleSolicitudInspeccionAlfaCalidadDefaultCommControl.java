...
import ingenias.exception.NotFound;
  ...
  public class SupervisorRoleSolicitudInspeccionAlfaCalidadDefaultCommControl extends DefaultCommControl{
  
  	  private Vector<String> previous=new Vector<String>();
  	  
  public SupervisorRoleSolicitudInspeccionAlfaCalidadDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  };
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("InspeccionarCalidadFuenteCuarentena");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("InspeccionarCalidadFuenteCuarentena");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("SolicitudInspeccionAlfaCalidadUI") &&
    	 sb.isState("SolicitudInspeccionAlfaCalidadUI")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"InspeccionarCalidadFuenteCuarentena").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("SolicitudInspeccionAlfaCalidadUI");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"InspeccionarCalidadFuenteCuarentena");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLock(mfcontent);
	       lockProcessed("InspeccionarCalidadFuenteCuarentena");
           //MainInteractionManager.log("Removing lock InspeccionarCalidadFuenteCuarentena",this.getAgent().getLocalName()+"-"+sb.getConversationID());
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for GradoCalidadCuarentenaListoUI")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("GradoCalidadCuarentenaListoUI")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for GradoCalidadCuarentenaListoUI");           
    	   try {
