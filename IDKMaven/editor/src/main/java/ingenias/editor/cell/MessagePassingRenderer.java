...
public class MessagePassingRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("MessagePassing",
	index,"/rendererxml/MessagePassingUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("MessagePassing",
	index,"/rendererxml/MessagePassingINGENIASPanel.xml");
     ...
     */
    public MessagePassingRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("MessagePassing",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("MessagePassing",current);
    }
  ...
  public static void setEntity(MessagePassing ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("MessagePassing",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("MessagePassing",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("MessagePassing",current);
	}
