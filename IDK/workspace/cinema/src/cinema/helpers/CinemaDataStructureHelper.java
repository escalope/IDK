...
public class CinemaDataStructureHelper {
	...
	public static Vector<String> createVectorString(String string){
		Tree parent=new Tree();
	...
	private static Hashtable<String, Hashtable<String, Vector<String>>> createTable(
	...
	private static Hashtable<String, Vector<String>> createSubtable(
			Vector<Tree> children) {
	...
	private static int createTree(int lastIndex,String string, Tree parent) {
		...
		while (lastIndex<string.length() && 
			...
			default:{
				...
				for (;lastIndex<string.length() && 
				(Character.isDigit(string.charAt(lastIndex))||
						Character.isLetter(string.charAt(lastIndex)))	;lastIndex++);
				if (lastIndex<string.length())
					nt.root=string.substring(initial,lastIndex);			
			}
			}
	...
	public static void main(String args[]){
		...
		System.err.println(createVectorString("(1,2,3,4,5)"));
