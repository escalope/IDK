...
public class TestEventInjectionEdge extends NAryEdge {

  // Constructor with data content
  public TestEventInjectionEdge(ingenias.editor.entities.TestEventInjection userObject) {
    super(userObject);
    
     this.addRole("TestEventInjectionsource");
     this.setArity("TestEventInjectionsource",true,1);
     this.setArity("TestEventInjectionsource",false,2147483647);
     
this.addClass("TestEventInjectionsource","ingenias.editor.entities.SimulationEvent");
     
    
     this.addRole("TestEventInjectiontarget");
     this.setArity("TestEventInjectiontarget",true,1);
     this.setArity("TestEventInjectiontarget",false,1);
     
this.addClass("TestEventInjectiontarget","ingenias.editor.entities.TestingPackage");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof TestEventInjectionEdge)))
      return false;

    // n-edge to be considered.
    TestEventInjectionEdge edge = new TestEventInjectionEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof TestEventInjectionEdge)
      edge = (TestEventInjectionEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("TestEventInjectionsource")){
 	 edges.add( new DefaultEdge( new TestEventInjectionsourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("TestEventInjectiontarget")){
 	 edges.add( new DefaultEdge( new TestEventInjectiontargetRole() ) );
	 }
