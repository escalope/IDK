...
public class ResearcherAssistantJADEAgent
		 extends JADEAgent {         
 
		 public ResearcherAssistantJADEAgent(){
		 super(new ResearcherAssistantProtocol(),new ResearcherAssistantInteractionLocks());
		 }
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
		...
		expectedInput=this.getMSM().getMentalEntityByType("PropuestaFuente");
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
			    ...
			    addExpectedInputs(tobject, "PropuestaFuente","1",expectedInput);
             	addConsumedInput(to,"PropuestaFuente",expectedInput);
			 }
         ...
         if (tobject.getType().equals("GenerarPropuestaTask") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("NuevaPropuestaPorEnviar");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("NuevaPropuestaPorEnviar");
			 } else {
			    addExpectedInputs(tobject, "NuevaPropuestaPorEnviar","1",expectedInput);
	 ...
	 /*    
	      ...
	      */	  
	      ...
	      /**/	      
		    ...
		    {PropuestaFuente expectedOutputPropuestaFuente=		    
		     new PropuestaFuente(MentalStateManager.generateMentalEntityID());			
             to.add(new OutputEntity(expectedOutputPropuestaFuente,TaskOperations.CreateWF));
            }
		...
		return initialised;	       
         ...
         nonExistingInputs.clear();
  		 repeatedOutputs.clear();
         if (tobject.getType().equals("ProcesarPropuestaDeHumanoTask") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("NuevaPropuestaIntroducidaEvento");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("NuevaPropuestaIntroducidaEvento");
			 } else {
			    addExpectedInputs(tobject, "NuevaPropuestaIntroducidaEvento","1",expectedInput);
             	addConsumedInput(to,"1",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
	 ...
	 /*    
	      ...
	      */	  
	      ...
	      /**/	      
		    ...
		    {NuevaPropuestaPorEnviar expectedOutputNuevaPropuestaPorEnviar=		    
		     new NuevaPropuestaPorEnviar(MentalStateManager.generateMentalEntityID());			
             to.add(new OutputEntity(expectedOutputNuevaPropuestaPorEnviar,TaskOperations.CreateWF));
            }
		...
		return initialised;	       
         ...
         nonExistingInputs.clear();
  		 repeatedOutputs.clear();
         if (tobject.getType().equals("InicializarVisualizacionTask") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("PrimeraEjecucion");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("PrimeraEjecucion");
			 } else {
			    addExpectedInputs(tobject, "PrimeraEjecucion","1",expectedInput);
             	addConsumedInput(to,"1",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist&& expectedInput.size()!=0;
	     ...
	     expectedApp=(ingenias.jade.components.Application)getAM().getApplication("VisualizacionApp");
             tobject.addApplication("VisualizacionApp",expectedApp);
	...
	private boolean initialiseConversationalTask(RuntimeConversation conversation, Task tobject) {
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		if (validConversationType){
	   	...
	   	nonExistingInputs.clear();
  	   	repeatedOutputs.clear();
  	   	boolean correctRole=conversation.getPlayedRole().equals ("ColaboratorRole");
  	   	...
  	   	correctRole=correctRole|| 
  	   	 conversation.getPlayedRole().equals ("ColaboratorRole");
  	   	
       	if (tobject.getType().equals("ProcesarRespuestaPropuestaRechazadaTask") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
	     ...
	     tobject.addOutput(to);
     	      ...
     	      if (!allEntitiesExist){
				 ...
				 EventManager.getInstance().conversationalInitializationOfTaskFailed(
				 			getLocalName(), "ResearcherAssistant", 
												tobject, nonexisting);
		...
		if (validConversationType){
       	...
       	if (tobject.getType().equals("ProcesarRespuestaPropuestaAceptadaTask") && (false ||
       		correctRole)){
	        Vector<MentalEntity> expectedInput=null;
	...
	public Vector tasksThatSatisfyGoal(String goalname){
         ...
         if (goalname.equals("SistemaEnriquecidoConBuenasFuentes")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				...
				for (int k=0;k<conversations.size();k++){
					tobject=new ProcesarRespuestaPropuestaRechazadaTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal SistemaEnriquecidoConBuenasFuentes",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
					}
         ...
         if (goalname.equals("SistemaEnriquecidoConBuenasFuentes")){
         
          {
			...
			Vector<RuntimeConversation>  conversations=getCM().getCurrentActiveConversations(typesOfConversation);
				boolean canbescheduled=false;
				for (int k=0;k<conversations.size();k++){
					tobject=new ProcesarRespuestaPropuestaAceptadaTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
					canbescheduled=initialiseConversationalTask(conversations.elementAt(k),tobject);
					if (canbescheduled){
					//	MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal SistemaEnriquecidoConBuenasFuentes",getLocalName()+"-"+tobject.getType());
						tasks.add(tobject);
		 ...
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new GenerarPropuestaTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
		 ...
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new ProcesarPropuestaDeHumanoTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
				 canbescheduled=initialiseNonConversationalTask(tobject);
			 	 if (canbescheduled){
					//MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal SistemaEnriquecidoConBuenasFuentes",getLocalName()+"-"+tobject.getType());
					tasks.add(tobject);
         ...
         if (goalname.equals("AsistirAlInvestigador")){
         
         {
         boolean canbescheduled=false;
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new InicializarVisualizacionTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
				 canbescheduled=initialiseNonConversationalTask(tobject);
			 	 if (canbescheduled){
					//MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal AsistirAlInvestigador",getLocalName()+"-"+tobject.getType());
					tasks.add(tobject);
				 } 			
	     }
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("ProcesarRespuestaPropuestaRechazadaTask");					
         
         
                  
         
                   
         ttypes.add("ProcesarRespuestaPropuestaAceptadaTask");					
         
         
         
         
         
         ttypes.add("GenerarPropuestaTask");		         
                
         
         
         ttypes.add("ProcesarPropuestaDeHumanoTask");		         
                
         
         
         ttypes.add("InicializarVisualizacionTask");		         
   ...
   sg= new ingenias.editor.entities.StateGoal("SistemaEnriquecidoConBuenasFuentes");
   sg.setState("pending");
      try {
   ...
   sg= new ingenias.editor.entities.StateGoal("AsistirAlInvestigador");
   sg.setState("pending");
      try {
	   this.getMSM().addMentalEntity(sg);
   } catch (InvalidEntity e1) {
   ...
   /* */
	...
	Vector evetns=null;
     ...
     app=VisualizacionAppInit.getInstance(this);
     //app.registerOwner(this);
	
     	    
     this.getAM().addApplication("VisualizacionApp",app);        
	 events=new Vector();
	 actions=new Vector();
		
	 event= new NuevaPropuestaIntroducidaEvento();
	 /*
	 slot=new Slot("228");
	 slot.setName("data");
	 ...
	 */ 
	 events.add(event);
	 actions.add(generateActionListener(NuevaPropuestaIntroducidaEvento.class));		

	 if (getGraphics()!=null)
	  getGraphics().addApplication("VisualizacionApp", events,actions);    
   ...
   java.awt.event.ActionListener ifPressed=null;
     ...
     final JADEAgent _jaColaboratorRolePerformProposal=this;
     ifPressed=new java.awt.event.ActionListener() {
       public void actionPerformed(ActionEvent e) {
         _jaColaboratorRolePerformProposal.addBehaviour(
           ...
           public void action() {
			 ...
			 if (allexist){
              ...
              if (getGraphics()!=null)
                getGraphics().runtimeWarning(" Mental conditions required for starting ColaboratorRolePerformProposal "+
     ...
     if (getGraphics()!=null)
      getGraphics().addInteraction(this.getName(), "Role: ColaboratorRole - Int: PerformProposal", ifPressed);
     getCM().addInitiatorRoles("PerformProposal","ColaboratorRole");
	...
	public DFAgentDescription[ ] getDescription() {
		...
		sd.setType("ColaboradorHumanoRole");
