...
public class OrganizationGroupRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("OrganizationGroup",
	index,"/rendererxml/OrganizationGroupUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("OrganizationGroup",
	index,"/rendererxml/OrganizationGroupINGENIASPanel.xml");
     ...
     */
    public OrganizationGroupRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("OrganizationGroup",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("OrganizationGroup",current);
    }
  ...
  public static void setEntity(OrganizationGroup ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("OrganizationGroup",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("OrganizationGroup",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("OrganizationGroup",current);
	}
