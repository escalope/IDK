...
public class AUMLSendSimpleEdge extends NAryEdge {

  // Constructor with data content
  public AUMLSendSimpleEdge(ingenias.editor.entities.AUMLSendSimple userObject) {
    super(userObject);
    
     this.addRole("AUMLSendSimplesource");
     this.setArity("AUMLSendSimplesource",true,1);
     this.setArity("AUMLSendSimplesource",false,1);
     
this.addClass("AUMLSendSimplesource","ingenias.editor.entities.AUMLPort");
     
    
     this.addRole("AUMLSendSimpletarget");
     this.setArity("AUMLSendSimpletarget",true,1);
     this.setArity("AUMLSendSimpletarget",false,1);
     
this.addClass("AUMLSendSimpletarget","ingenias.editor.entities.AUMLPort");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AUMLSendSimpleEdge)))
      return false;

    // n-edge to be considered.
    AUMLSendSimpleEdge edge = new AUMLSendSimpleEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AUMLSendSimpleEdge)
      edge = (AUMLSendSimpleEdge) selectedEdge;
