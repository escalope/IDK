...
public class GRASIASpecificationRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("GRASIASpecification",
	index,"/rendererxml/GRASIASpecificationUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("GRASIASpecification",
	index,"/rendererxml/GRASIASpecificationINGENIASPanel.xml");
     ...
     */
    public GRASIASpecificationRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("GRASIASpecification",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("GRASIASpecification",current);
    }
  ...
  public static void setEntity(GRASIASpecification ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("GRASIASpecification",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("GRASIASpecification",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("GRASIASpecification",current);
	}
