...
private MentalStateReader msr=null;
  public SupervisorRoleQualityInspectionRequestStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "QualityInspectionRequestIU");
			    
			    
			      // Receiving a message    
			      smf.add("QualityDegreeIU", "waiting for QualityDegreeIU");
			      
			      // Next states after receiving "QualityDegreeIU"
			      
			       smf.add("waiting for QualityDegreeIU","endQualityDegreeIU");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("QualityInspectionRequestIU", "QualityDegreeIU");
  ...
  if (this.isState("QualityDegreeIU") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"QualityDegreeIU",
                   "QualityInspectionRequest",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("QualityDegreeIU");
      this.addState("waiting for QualityDegreeIU");
      this.notifyStateTransitionExecuted("QualityDegreeIU", "waiting for QualityDegreeIU");
  } 
  ...
  if (this.isState("QualityInspectionRequestIU")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("QualityInspectionRequestIU",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "QualityInspectionRequestIU","QualityInspectionRequest",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("QualityInspectionRequestIU", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("QualityInspectionRequestIU", "ABORTED");
    	  } else  {
