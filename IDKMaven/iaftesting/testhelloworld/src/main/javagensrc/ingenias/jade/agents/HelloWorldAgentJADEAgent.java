...
public class HelloWorldAgentJADEAgent
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
		...
		if (this.getLM().canBeDeleted(expectedInput)){             
         ...
         if (tobject.getType().equals("GreetingsTask") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
			...
			to=new TaskOutput("default");
             ...
             expectedInput=this.getMSM().getMentalEntityByType("DontForgetToGreet");
             if (expectedInput.size()==0 && !("1".equals("0..n"))){
				nonExistingInputs.add("DontForgetToGreet");
			 } else {
			    addExpectedInputs(tobject, "DontForgetToGreet","1",expectedInput);
	...
	public Vector tasksThatSatisfyGoal(String goalname){
         ...
         if (goalname.equals("BePolite")){
         
         {
         boolean canbescheduled=false;
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new GreetingsTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
				 canbescheduled=initialiseNonConversationalTask(tobject);
			 	 if (canbescheduled){
					//MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal BePolite",getLocalName()+"-"+tobject.getType());
					tasks.add(tobject);
				 } 			
	     }
   ...
   sg= new ingenias.editor.entities.StateGoal("BePolite");
   sg.setState("pending");
      try {
