...
public class ListenerContainer implements GraphModelListener {
	...
	public static boolean isContainer(DefaultGraphCell object, JGraph jg){
		boolean found=false;
	...
	public void setParent(DefaultGraphCell principal, DefaultGraphCell nested1) {
	...
	public void fromXML(Hashtable<String, DefaultGraphCell> nodeids,
			Node parentshipnode) {
		NodeList children = parentshipnode.getChildNodes();
		for (int k = 0; k < children.getLength(); k++) {
			String childtype = (String) children.item(k).getNodeName();
			if (childtype.equals("child")) {
				String childid = children.item(k).getAttributes()
				.getNamedItem("id").getNodeValue();
				String parentid = children.item(k).getAttributes()
