...
import ingenias.exception.NotFound;
  ...
  public class BuyerAsking_for_a_ticketDefaultCommControl extends DefaultCommControl{
  ...
  public BuyerAsking_for_a_ticketDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("CouldNotGetTicket");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("Ticket_data");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("ImpossibleToGiveTicket") &&
    	 sb.isState("ImpossibleToGiveTicket")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"CouldNotGetTicket").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("ImpossibleToGiveTicket");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"CouldNotGetTicket");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("CouldNotGetTicket");
	       lockProcessed("CouldNotGetTicket");
           //MainInteractionManager.log("Removing lock CouldNotGetTicket",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("endImpossibleToGiveTicket");
    ...
    if (stateToEvaluate.equals("GiveTicket") &&
    	 sb.isState("GiveTicket")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Ticket_data").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("GiveTicket");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Ticket_data");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("Ticket_data");
	       lockProcessed("Ticket_data");
           //MainInteractionManager.log("Removing lock Ticket_data",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("endGiveTicket");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for RequestOrder")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("RequestOrder")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for RequestOrder");           
    	   //try {
		   ...
		   futureStates.add("GiveTicket");
                  
		   futureStates.add("ImpossibleToGiveTicket");
