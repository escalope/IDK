...
private MentalStateReader msr=null;
  public SupervisorRoleRequestToAddSourceStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "AddingSourceRequestIU");
			    
			    
			      // Receiving a message    
			      smf.add("AddingSourceResponseIU", "waiting for AddingSourceResponseIU");
			      
			      // Next states after receiving "AddingSourceResponseIU"
			      
			       smf.add("waiting for AddingSourceResponseIU","endAddingSourceResponseIU");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("AddingSourceRequestIU", "AddingSourceResponseIU");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
  	try {  	  
      String[] options=new String[]{"AddingSourceRequestIU"};
      AID[] actors=null;
      Vector<AID> actorsv=new Vector<AID>();
      Vector<String> rolesv=new Vector<String>();
      
      actorsv.addAll(this.getActor("SourcesManagerRole"));
      {      
       Vector<AID> receivers=this.getActor("SourcesManagerRole");      
       for (AID aid:receivers){
        rolesv.add("SourcesManagerRole");
       }
      }
  ...
  if (this.isState("AddingSourceResponseIU") ) {  // State changed by other agent and upted
    //in the parent class
         
      Vector options=new Vector();
      
      options.add("endAddingSourceResponseIU");
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"AddingSourceResponseIU",
                   "RequestToAddSource",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("AddingSourceResponseIU");
      this.addState("waiting for AddingSourceResponseIU");
      this.notifyStateTransitionExecuted("AddingSourceResponseIU", "waiting for AddingSourceResponseIU");
  } 
  ...
  if (this.isState("AddingSourceRequestIU")) {
     ...
     try {
      ...
      options.add("AddingSourceResponseIU");      
      
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("AddingSourceRequestIU",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "AddingSourceRequestIU","RequestToAddSource",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("AddingSourceRequestIU", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("AddingSourceRequestIU", "ABORTED");
    	  } else  {
  ...
  if (this.isState("endAddingSourceResponseIU")) {
