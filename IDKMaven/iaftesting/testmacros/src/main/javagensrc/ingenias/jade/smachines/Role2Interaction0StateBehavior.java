...
private MentalStateReader msr=null;
  public Role2Interaction0StateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","InteractionUnit2");
			    
			    
			    
			      // Receiving a message    
			      smf.add("InteractionUnit2", "waiting for InteractionUnit2");
			      
			      // Next states after receiving "InteractionUnit2"
			      
			       smf.add("waiting for InteractionUnit2","InteractionUnit1");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     String[] options=new String[]{"InteractionUnit2"};
  ...
  if (this.isState("InteractionUnit2") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if ("1".equals("n")){
         try{
          Vector<AID> receivers=this.getActor("Role1");    
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"InteractionUnit2",
                   "Interaction0",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("InteractionUnit2");
      this.addState("waiting for InteractionUnit2");
      this.notifyStateTransitionExecuted("InteractionUnit2", "waiting for InteractionUnit2");
  } 
