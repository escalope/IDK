...
public class EPerceivesNotificationEdge extends NAryEdge {

  // Constructor with data content
  public EPerceivesNotificationEdge(ingenias.editor.entities.EPerceivesNotification userObject) {
    super(userObject);
    
     this.addRole("EPerceivessource");
     this.setArity("EPerceivessource",true,1);
     this.setArity("EPerceivessource",false,1);
     
this.addClass("EPerceivessource","ingenias.editor.entities.Agent");
     
    
     this.addRole("EPerceivestarget");
     this.setArity("EPerceivestarget",true,1);
     this.setArity("EPerceivestarget",false,2147483647);
     
this.addClass("EPerceivestarget","ingenias.editor.entities.Application");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof EPerceivesNotificationEdge)))
      return false;

    // n-edge to be considered.
    EPerceivesNotificationEdge edge = new EPerceivesNotificationEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof EPerceivesNotificationEdge)
      edge = (EPerceivesNotificationEdge) selectedEdge;
