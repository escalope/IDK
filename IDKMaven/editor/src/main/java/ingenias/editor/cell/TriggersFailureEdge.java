...
public class TriggersFailureEdge extends NAryEdge {

  // Constructor with data content
  public TriggersFailureEdge(ingenias.editor.entities.TriggersFailure userObject) {
    super(userObject);
    
     this.addRole("TriggersFailuresource");
     this.setArity("TriggersFailuresource",true,1);
     this.setArity("TriggersFailuresource",false,1);
     
this.addClass("TriggersFailuresource","ingenias.editor.entities.InteractionUnit");
     
    
     this.addRole("TriggersFailuretarget");
     this.setArity("TriggersFailuretarget",true,1);
     this.setArity("TriggersFailuretarget",false,1);
     
this.addClass("TriggersFailuretarget","ingenias.editor.entities.RuntimeCommFailure");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof TriggersFailureEdge)))
      return false;

    // n-edge to be considered.
    TriggersFailureEdge edge = new TriggersFailureEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof TriggersFailureEdge)
      edge = (TriggersFailureEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("TriggersFailuresource")){
 	 edges.add( new DefaultEdge( new TriggersFailuresourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("TriggersFailuretarget")){
 	 edges.add( new DefaultEdge( new TriggersFailuretargetRole() ) );
	 }
