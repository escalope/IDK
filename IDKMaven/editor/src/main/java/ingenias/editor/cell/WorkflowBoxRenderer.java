...
public class WorkflowBoxRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("WorkflowBox",
	index,"/rendererxml/WorkflowBoxUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("WorkflowBox",
	index,"/rendererxml/WorkflowBoxINGENIASPanel.xml");
     ...
     */
    public WorkflowBoxRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("WorkflowBox",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("WorkflowBox",current);
    }
  ...
  public static void setEntity(WorkflowBox ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("WorkflowBox",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("WorkflowBox",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("WorkflowBox",current);
	}
