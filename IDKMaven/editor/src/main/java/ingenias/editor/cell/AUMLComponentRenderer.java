...
public class AUMLComponentRenderer extends CompositeRenderer implements CellViewRenderer, Serializable {
  ...
  static {
	...
	RenderComponentManager.loadRenderFile("AUMLComponent",
	index,"/rendererxml/AUMLComponentUMLPanel.xml");
	
	index=ViewPreferences.ViewType.INGENIAS;
	RenderComponentManager.loadRenderFile("AUMLComponent",
	index,"/rendererxml/AUMLComponentINGENIASPanel.xml");
     ...
     */
    public AUMLComponentRenderer() {
    }
    
      public JComponent getConcreteSubComponent(String fieldname, Entity ent, Map map) {
	  Map<String,JComponent> currentMap=(Map)  RenderComponentManager.retrieveIDs("AUMLComponent",ent.getPrefs(map).getView());
	  return currentMap.get(fieldname);
  		}

    public Dimension getSize() {
    return RenderComponentManager.getSize("AUMLComponent",current);
    }
  ...
  public static void setEntity(AUMLComponent ent, Map attributes){
  Map currentMap=(Map)  RenderComponentManager.retrieveIDs("AUMLComponent",ent.getPrefs(attributes).getView());
  current=ent.getPrefs(attributes).getView();
	...
	{
	return RenderComponentManager.retrievePanel("AUMLComponent",
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("AUMLComponent",current);
	}
