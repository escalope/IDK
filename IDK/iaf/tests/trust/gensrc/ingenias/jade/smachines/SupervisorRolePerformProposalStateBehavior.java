...
public class SupervisorRolePerformProposalStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public SupervisorRolePerformProposalStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","ProposalIU");
			    
			    
			    
			      // Receiving a message    
			      smf.add("ProposalIU", "waiting for ProposalIU");
			      
			      // Next states after receiving "ProposalIU"
			      
			       smf.add("waiting for ProposalIU","DeniedProposalIU");
			      
			       smf.add("waiting for ProposalIU","AcceptedProposalIU");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("DeniedProposalIU", "endDeniedProposalIU");
			      
			    
			     // States involved in message deliver
			     
			      smf.add("AcceptedProposalIU", "endAcceptedProposalIU");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "PerformProposal","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("ProposalIU") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if ("1".equals("n")){
         try{
          Vector<AID> receivers=this.getActor("CollaboratorRole");    
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"ProposalIU",
                   "PerformProposal",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("ProposalIU");
      this.addState("waiting for ProposalIU");
      this.notifyStateTransitionExecuted("ProposalIU", "waiting for ProposalIU");
  } 
  ...
  if (this.isState("DeniedProposalIU")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("DeniedProposalIU",optionsA,this)){
           ...
           "DeniedProposalIU","PerformProposal",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("DeniedProposalIU", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("DeniedProposalIU", "ABORTED");
    	  } else  {
  ...
  // Sends a message and synchronization commands
  if (this.isState("AcceptedProposalIU")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("AcceptedProposalIU",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "AcceptedProposalIU","PerformProposal",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("AcceptedProposalIU", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("AcceptedProposalIU", "ABORTED");
    	  } else  {
    		  if (!getTimeout().isStarted())
    		  getTimeout().start(0);
    	  }
