...
public class IAccessesEdge extends NAryEdge {

  // Constructor with data content
  public IAccessesEdge(ingenias.editor.entities.IAccesses userObject) {
    super(userObject);
    
     this.addRole("IAccessessource");
     this.setArity("IAccessessource",true,1);
     this.setArity("IAccessessource",false,1);
     
this.addClass("IAccessessource","ingenias.editor.entities.Task");
     
    
     this.addRole("IAccessestarget");
     this.setArity("IAccessestarget",true,1);
     this.setArity("IAccessestarget",false,1);
     
this.addClass("IAccessestarget","ingenias.editor.entities.Interaction");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof IAccessesEdge)))
      return false;

    // n-edge to be considered.
    IAccessesEdge edge = new IAccessesEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof IAccessesEdge)
      edge = (IAccessesEdge) selectedEdge;
